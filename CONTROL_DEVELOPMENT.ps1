...
$executionPath = 'config/control/harness/executions/E2026-08-11-H0-0-R1'
$selectedModes = @($Status, $Plan, $Resume) | Where-Object { $_ }
if ($Execute -or $selectedModes.Count -ne 1) {
...
try {
    ...
    if ($null -eq $pythonCommand) {
        Write-Output '{"schema":"distributed_world_simulator.control_development_output.v1","command":"UNKNOWN","ok":false,"error":{"code":"CONTRACT_OR_DEPENDENCY_INVALID","detail":"PYTHON_3_REQUIRED"}}'
        exit 3
    }
