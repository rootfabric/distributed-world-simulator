...
Текущий этап: подготовка M5 на `main @ 2879fdb`.

```text
A0 → H0 → A1 → S0 → T1 → B0 → M0 → S1 accepted
H1 → H2 → H3 → A2 → M1 → M2 → M3 → M4 accepted
M5 preparation in progress
M5 → M6 → A3 planned
...
Полный M5 ещё должен доказать UI-driven действия двух graphical clients,
...
Исторически M3 подготовил presentation/ownership vertical, а принятый M4 добавил canonical Item Graph, authoritative item commands и двухклиентский contention. Текущая работа — подготовительная UI/replica boundary для M5; она не открывает новый gameplay path.
