...
if (-not (Test-Path -LiteralPath $Garment -PathType Leaf)) {
    throw "Male_Peasant.gltf is missing. CH9.6 reuses the accepted CH8 Quaternius garment asset."
}
...
Write-Host "CH9.6 - Playable Network Equipment Lab" -ForegroundColor Cyan
Write-Host "Path: Inventory UI -> network equipment controller -> ITEM/ENet -> persistent server -> replicated Item Graph -> Quaternius presenter" -ForegroundColor Cyan
Write-Host "Port: $Port" -ForegroundColor Cyan
if ($ResetState) {
    Write-Host "Persistence state will be reset before server bootstrap." -ForegroundColor Yellow
} else {
    Write-Host "Persistence state is preserved. Reopen the lab to verify recovered equipment." -ForegroundColor Cyan
}
Write-Host "Inventory opens after network JOIN. Drag wearable items into head/back/upper/lower/feet slots." -ForegroundColor Cyan
Write-Host "Close with Tab to walk/jump/crouch and inspect the equipped character." -ForegroundColor Cyan
