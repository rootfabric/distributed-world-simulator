...
SERVER_NETWORK_PROFILE="${M7_SERVER_NETWORK_PROFILE:-LOCAL}"
CLIENT_NETWORK_PROFILE="${M7_CLIENT_NETWORK_PROFILE:-LOCAL}"
NETWORK_PRESETS_FILE="${M7_NETWORK_PRESETS_FILE:-res://config/network/network-condition-presets.v1.json}"
...
NETWORK_SESSION_TOKEN="${NX0_NETWORK_SESSION_TOKEN:-session-id/m7-$RUN_ID}"
...
start_isolated "$RUN_ROOT/profiles/server" --headless --path "$ROOT" --log-file "$RUN_ROOT/server.log" -- \
  ...
  --network-session-token="$NETWORK_SESSION_TOKEN" \
  --node-id=m7-playground-server --instance-id=m7-playground \
  --server-address="$HOST" --server-port="$PORT" --m7-result-file="$SERVER_RESULT" \
  --network-profile="$SERVER_NETWORK_PROFILE" --network-presets-file="$NETWORK_PRESETS_FILE" \
...
for ((i=0;i<CLIENT_COUNT;i++)); do
  ...
  start_isolated "$RUN_ROOT/profiles/client-$id" --path "$ROOT" --rendering-method gl_compatibility \
    ...
    --network-profile="$CLIENT_NETWORK_PROFILE" --network-presets-file="$NETWORK_PRESETS_FILE" \
    --print-runtime-descriptor
  CLIENT_PIDS+=("$STARTED_PID")
  sleep .35
done
python3 - "$ACTIVE" "$RUN_ROOT/session.json" "$ROOT" "$GODOT_BIN" "$HOST" "$PORT" "$SERVER_PID" "$RUN_ROOT" "$PERSISTENCE_ROOT" "$NETWORK_SESSION_TOKEN" "$SERVER_NETWORK_PROFILE" "$CLIENT_NETWORK_PROFILE" "$NETWORK_PRESETS_FILE" "${CLIENT_PIDS[*]}" <<'PY'
import json,sys,datetime
active,copy,root,godot,host,port,server,run,persist,token,server_profile,client_profile,presets,clients=sys.argv[1:]
d={"schema":"planet_simulator.m7_playable_networked_playground_session.v1","checkpoint":"v16.11.0-network-nx1-deterministic-condition-simulator","started_at_utc":datetime.datetime.now(datetime.timezone.utc).isoformat(),"project_root":root,"godot":godot,"server_address":host,"server_port":int(port),"server_pid":int(server),"client_pids":[int(x) for x in clients.split()],"run_root":run,"persistence_root":persist,"network_session_token":token,"server_network_profile":server_profile,"client_network_profile":client_profile,"network_presets_file":presets}
...
echo "Logs: $RUN_ROOT"
echo "Network profiles: server=$SERVER_NETWORK_PROFILE client=$CLIENT_NETWORK_PROFILE"
if [[ "$SERVER_NETWORK_PROFILE" != "LOCAL" && "$CLIENT_NETWORK_PROFILE" != "LOCAL" ]]; then
  echo "Note: endpoint profiles are additive; applying the same preset at both ends roughly doubles path impairment."
fi
