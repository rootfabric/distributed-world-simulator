# Planetary World v16.3.7-ui-i2-fix2 — поиск и большие хранилища

## UI-I0 + UI-I1 feature branch
...
UI-I1 fix1 устраняет частичный quick-transfer в `SLOTS`: команда переноса всего стака сначала ищет слот, который принимает всё количество, и сообщает фактически перемещённое количество.

UI-I1 добавляет:

- `Shift+ЛКМ` для быстрого переноса между рюкзаком и открытым контейнером;
- ПКМ-контекстное меню;
- явный split dialog с `1/Половина/Максимум`;
- tooltip с массой, объёмом и тегами;
- локальные причины отказа и toast-уведомления;
- назначение предмета в конкретный hotbar slot;
- явный выброс одного предмета или всего стака через контекстное меню.
...
## Поиск и большие хранилища

- Строка поиска фильтрует BULK-контейнеры по названию, definition ID и тегам.
- Категории: ресурсы, инструменты, контейнеры, аккумуляторы, монтируемые и строительство.
- Сортировка является только визуальной: по имени, типу, количеству, массе, объёму и последовательности успешной операции. Item Graph и порядок `container.item_ids` не меняются.
- Кеш сортировки по недавним операциям инвалидируется по transient generation ledger, поэтому загрузка другого набора записей с тем же `next_sequence` немедленно меняет порядок. Generation не сериализуется.
- ЛКМ по предмету открывает постоянный инспектор с relation, компонентами, массой, объёмом и revision.
- Для любого BULK-результата больше 96 агрегатов активируется page-window по 96 карточек. Пул жёстко ограничен 96 карточками для BULK и уменьшается до этого предела после более крупного SLOTS-представления.
- Поиск, фильтр, сортировка и видимость инспектора сохраняются отдельно в `user://planet_simulator/ui/`; эти настройки не входят в authoritative item snapshot.

## Управление стаками

- ЛКМ-перетаскивание переносит весь стак.
- `Shift+ЛКМ` быстро переносит стак между рюкзаком и открытым контейнером.
- ПКМ без движения открывает меню переноса, разделения, hotbar, inspect и drop.
