# Planetary World v16.3.5-ui-i2 — поиск и большие хранилища
...
- Сортировка является только визуальной: по имени, типу, количеству, массе, объёму и revision. Item Graph и порядок `container.item_ids` не меняются.
- ЛКМ по предмету открывает постоянный инспектор с relation, компонентами, массой, объёмом и revision.
- Для хранилищ более 120 агрегатов активируется page-window по 96 карточек. Карточки переиспользуются через pool и не создаются заново при каждом обновлении.
