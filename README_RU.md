# Planetary World v15.4.1 — Луна, Земля, предметы и взаимодействие

Проект для Godot 4.7.1, собранного с `precision=double`.

Основная ветка объединяет рабочую Луну, процедурную Землю, атмосферу,
постоянные маяки с first-person взаимодействием и лабораторию предметного
домена. Точка входа — `scripts/app/planetary_app.gd`.

## Предметы

`F5` открывает лабораторию предметов из лунного или межпланетного режима.
В ней проверяются переходы одного экземпляра предмета между миром,
контейнером и сокетом сборки. `F5` или `Esc` возвращает в общий мир.
