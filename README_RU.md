# Planetary World v15.5 — общее ядро и подключаемые миры
...
Глобальными остаются `~`, `F1`, `Tab` и небольшой набор игровых хоткеев:

```text
F2    телепортировать игрока к позиции спектатора
F3    переключить игрока и спектатор
F4    включить или выключить визуализацию LOD
Q/E   крен спектатора влево/вправо
```
...
показывает команда `input.bindings`. Земля и Луна остаются в одной абсолютной
системе координат; каждый генератор выполняет собственный body-local floating
origin только перед рендерингом.
...
F2         телепортировать игрока к спектатору
F3         персонаж / спектатор
F4         показать границы LOD
