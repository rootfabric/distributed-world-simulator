...
run_checked() {
  local name="$1"; shift
  local log="$REPORT_ROOT/a1-${name}.log"
  set +e
...
run_checked contracts --headless --path "$PROJECT_ROOT" --script res://tests/simulation/test_a1_generic_aggregate_contracts.gd
run_checked integration --headless --path "$PROJECT_ROOT" --script res://tests/simulation/test_a1_generic_aggregate_integration.gd
echo "A1 generic aggregate tests: PASS"
