...
$ReportPath = Join-Path $ReportDirectory "c10-parametric-members-summary.json"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "C10 editor parse failed" }
    $Steps += [ordered]@{
    ...
    foreach ($Test in $Tests) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "C10 test failed: $Test" }
        $Steps += [ordered]@{
    ...
    [ordered]@{
        schema = "planet_simulator.c10_parametric_members_summary.v1"
        checkpoint = "C10_PARAMETRIC_MEMBERS"
        passed = $Passed
...
Write-Host "C10 Parametric Members profile passed."
