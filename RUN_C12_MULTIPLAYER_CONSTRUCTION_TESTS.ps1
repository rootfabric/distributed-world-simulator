...
$ReportPath = Join-Path $ReportDirectory "c12-multiplayer-construction-summary.json"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "C12 editor parse failed" }
    $Steps += [ordered]@{
    ...
    foreach ($Test in $Tests) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "C12 test failed: $Test" }
        $Steps += [ordered]@{
    ...
    [ordered]@{
        schema = "planet_simulator.c12_multiplayer_construction_summary.v1"
        checkpoint = "C12_MULTIPLAYER_CONSTRUCTION_ACCEPTANCE"
        passed = $Passed
...
Write-Host "C12 Multiplayer Construction Acceptance profile passed."
