...
$ReportPath = Join-Path $ReportDirectory "c3-build-plan-summary.json"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "C3 editor parse failed" }
    $Steps += [ordered]@{
    ...
    foreach ($Test in $Tests) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "C3 test failed: $Test" }
        $Steps += [ordered]@{
    ...
    [ordered]@{
        schema = "planet_simulator.c3_build_plan_summary.v1"
        checkpoint = "C3_BUILD_PLAN_AND_GHOST_CONSTRUCTION"
        passed = $Passed
...
Write-Host "C3 BuildPlan and ghost construction profile passed."
