...
$ReportPath = Join-Path $ReportDirectory "c7-spatial-construct-summary.json"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "C7 editor parse failed" }
    $Steps += [ordered]@{
    ...
    foreach ($Test in $Tests) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "C7 test failed: $Test" }
        $Steps += [ordered]@{
    ...
    [ordered]@{
        schema = "planet_simulator.c7_spatial_construct_summary.v1"
        checkpoint = "C7_SPATIAL_CONSTRUCT"
        passed = $Passed
...
Write-Host "C7 Spatial Construct profile passed."
