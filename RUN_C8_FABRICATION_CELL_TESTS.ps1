...
$ReportPath = Join-Path $ReportDirectory "c8-fabrication-cell-summary.json"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "C8 editor parse failed" }
    $Steps += [ordered]@{
    ...
    foreach ($Test in $Tests) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "C8 test failed: $Test" }
        $Steps += [ordered]@{
    ...
    [ordered]@{
        schema = "planet_simulator.c8_fabrication_cell_summary.v1"
        checkpoint = "C8_FABRICATION_CELL"
        passed = $Passed
...
Write-Host "C8 Fabrication Cell profile passed."
