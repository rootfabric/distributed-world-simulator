...
$ReportPath = Join-Path $ReportDirectory "c9-damage-split-repair-summary.json"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "C9 editor parse failed" }
    $Steps += [ordered]@{
    ...
    foreach ($Test in $Tests) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "C9 test failed: $Test" }
        $Steps += [ordered]@{
    ...
    [ordered]@{
        schema = "planet_simulator.c9_damage_split_repair_summary.v1"
        checkpoint = "C9_DAMAGE_SPLIT_REPAIR"
        passed = $Passed
...
Write-Host "C9 Damage, Split, Repair profile passed."
