...
Write-Host "CH9.1: authoritative backpack/container <-> equipment operations" -ForegroundColor Cyan
Write-Host "Canonical mutation owner: existing ItemTransferService" -ForegroundColor Cyan
Write-Host "Single replacement: atomic swap_items; multi-item replacement: fail closed" -ForegroundColor Cyan

Write-Host ""
Write-Host "[editor_import]" -ForegroundColor Cyan
& $Godot --headless --editor --path $Root --quit
if ($LASTEXITCODE -ne 0) { throw "CH9.1 editor import/parse failed with exit code $LASTEXITCODE" }
...
Write-Host ""
Write-Host "CH9.1 Authoritative Equipment Operations candidate runner: PASS" -ForegroundColor Green
exit 0
