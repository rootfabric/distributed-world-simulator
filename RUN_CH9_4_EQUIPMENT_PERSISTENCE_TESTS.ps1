...
Write-Host "CH9.4: equipment durability through existing M6 checkpoint/replay/recovery" -ForegroundColor Cyan
Write-Host "No character-private persistence format or recovery manager is introduced." -ForegroundColor Cyan
...
if ($ImportExit -ne 0 -or $ImportText -match '(?m)(^ERROR:|SCRIPT ERROR:|Parse Error:|Compile Error:)') {
    throw "CH9.4 editor import/parse failed with exit code $ImportExit"
}
...
Invoke-Godot-Test "ch9_3_replica_projection" "res://tests/characters/test_ch9_3_multiplayer_equipment_replica_projection.gd"
Invoke-Godot-Test "ch9_3_character_presentation" "res://tests/characters/test_ch9_3_replicated_equipment_character_presentation.gd"
...
Write-Host ""
Write-Host "CH9.4 Equipment Persistence / Late Reconnect candidate runner: PASS" -ForegroundColor Green
exit 0
