param(
    ...
    [string]$EngineLogFile = $env:ECO_LS40_ENGINE_LOG
)
...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 LS4.0 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
# LS4 changes the physical environment between generations, therefore its
# transitive gate includes LS3.5/LS3.6 observability in addition to the full
# STREAM1 predecessor chain.
$Tests = @(
    ...
    'res://tests/ecology/eco_evo7_ls35_emergent_biome_acceptance.gd',
    'res://tests/ecology/eco_evo7_ls36_rule_workbench_acceptance.gd',
    ...
    'res://tests/ecology/eco_evo7_ls40_seasonal_succession_acceptance.gd',
...
$LogDir = Join-Path $Root 'artifacts/ls40_gate_logs'
New-Item -ItemType Directory -Force -Path $LogDir | Out-Null

$ErrorActionPreference = 'Continue'
foreach ($Test in $Tests) {
    Write-Host "=== LS4.0 gate: $Test ==="
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "LS4.0 gate failed: $Test (exit=$LASTEXITCODE, log=$PerTestLog)"
        exit $LASTEXITCODE
    }
}

Write-Host "LS4.0 logs: $LogDir"
Write-Host 'ECO.EVO7 LS4.0 transitive acceptance: PASS'
exit 0
