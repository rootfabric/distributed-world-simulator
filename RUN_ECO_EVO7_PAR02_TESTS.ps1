...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 PAR0.2 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
# PAR0.2 R1: inherited gates run FIRST and WITHOUT any PAR0.2 activation —
# they prove serial-default isolation (the executor is injection-only, so
# these fresh processes can never spawn a pool). Then the PAR0 gates, then
# the focused PAR0.2 acceptance (dual verification, negative divergence,
# stale response, pool lifecycle, telemetry exclusion).
$Tests = @(
