...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 PAR2 PLAY0 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
# PAR2 PLAY0: AUTO evolution >=3 minutes with the PAR2 parallel-only
# executor (PROCESS_POOL + bounded audits) injected through the seam.
$ErrorActionPreference = 'Continue'
& $GodotBin --path $Root --script res://tests/ecology/eco_evo7_par2_play0_parallel_only.gd 2>$null
exit $LASTEXITCODE
