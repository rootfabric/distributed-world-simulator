...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 PAR3 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
# PAR3 R2: includes empty-slice parity, public candidate facade, and single-kernel validation repairs.
# PAR3 R1 transitive regression: everything PAR2 runs (PAR0.2 inherited,
# PAR1, PAR2, VIS3, PLAY0) plus the focused PAR3 acceptance (single kernel
# identity, parallel candidate parity >=108 comparisons, audit schedule,
# fail-closed injections, performance gate). The >=5-minute combined
# graphical run is RUN_ECO_EVO7_PAR3_PLAY0.ps1 (run separately).
$Tests = @(
