...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS4.4 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS4.3 predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS4_3_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.3 predecessor regression failed"
    }

    Write-Host "=== ECO PLAY0 regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_PLAY0_TESTS.ps1") -GodotBin $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "PLAY0 regression failed"
    }

    Write-Host "=== ECO VIS4.4 PLAY0.MORPH focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis4_4_play0_morph_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.4 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS4.4 PLAY0.MORPH candidate: PASS"
