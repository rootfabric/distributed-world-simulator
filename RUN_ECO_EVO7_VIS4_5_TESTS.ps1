...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS4.5 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS4.4 predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS4_4_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.4 predecessor regression failed"
    }

    Write-Host "=== ECO VIS4.5 deterministic individuality focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis4_5_deterministic_individuality_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.5 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS4.5 Deterministic Individuality candidate: PASS"
