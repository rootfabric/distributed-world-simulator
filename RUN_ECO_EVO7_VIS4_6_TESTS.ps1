...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS4.6 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS4.5 predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS4_5_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.5 predecessor regression failed"
    }

    Write-Host "=== ECO VIS4.6 grid appearance boundary focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis4_6_grid_appearance_boundary_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.6 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS4.6 Grid Appearance Boundary candidate: PASS"
