...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS4.7 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS4.6 predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS4_6_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.6 predecessor regression failed"
    }

    Write-Host "=== ECO VIS4.7 morphology inspector focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis4_7_morphology_inspector_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.7 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS4.7 Morphology Inspector candidate: PASS"
