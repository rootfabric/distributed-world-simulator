...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS4.9 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS4.8 predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS4_8_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.8 predecessor regression failed"
    }

    Write-Host "=== ECO VIS4.9 performance / LOD focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis4_9_performance_lod_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.9 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS4.9 Performance / LOD candidate: PASS"
