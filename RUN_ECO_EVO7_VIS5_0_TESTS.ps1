...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS5.0 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS4.9 closed predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS4_9_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS4.9 predecessor regression failed"
    }

    Write-Host "=== ECO VIS5.0 terrain / ecosystem composition contract audit ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis5_0_terrain_ecosystem_composition_contract_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS5.0 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS5.0 Terrain / Ecosystem Composition Contract Audit candidate: PASS"
