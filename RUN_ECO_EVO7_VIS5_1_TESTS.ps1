...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS5.1 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS5.0 closed predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS5_0_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS5.0 predecessor regression failed"
    }

    Write-Host "=== ECO VIS5.1 terrain surface frame adapter focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis5_1_terrain_surface_frame_adapter_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS5.1 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS5.1 Terrain Surface Frame Adapter candidate: PASS"
