...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS5.2 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS5.1 closed predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS5_1_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS5.1 predecessor regression failed"
    }

    Write-Host "=== ECO VIS5.2 noncanonical ground-cover bridge focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis5_2_noncanonical_ground_cover_bridge_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS5.2 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS5.2 Noncanonical Ground-Cover Presentation Bridge candidate: PASS"
