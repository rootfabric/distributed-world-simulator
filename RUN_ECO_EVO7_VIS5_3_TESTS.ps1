...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS5.3 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS5.2 closed predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS5_2_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS5.2 predecessor regression failed"
    }

    Write-Host "=== ECO VIS5.3 mixed-strata composition focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis5_3_mixed_strata_composition_lab_acceptance.gd"
    if ($LASTEXITCODE -ne 0) {
        throw "VIS5.3 focused acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
...
Write-Host "ECO.EVO7 VIS5.3 Mixed-Strata Composition Lab candidate: PASS"
