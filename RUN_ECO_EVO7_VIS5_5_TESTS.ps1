...
if ($Actual -ne $Expected) {
    throw "ECO.EVO7 VIS5.5 BLOCKED: expected Godot '$Expected', got '$Actual'"
}
...
try {
    ...
    Write-Host "=== ECO VIS5.4 closed predecessor regression ==="
    & (Join-Path $RootDir "RUN_ECO_EVO7_VIS5_4_TESTS.ps1") -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) { throw "VIS5.4 predecessor regression failed" }
    Write-Host "=== ECO VIS5.5 visual evidence / PLAY1 handoff focused acceptance ==="
    & $GodotPath --headless --path $RootDir --script "res://tests/ecology/eco_evo7_vis5_5_visual_evidence_play1_handoff_acceptance.gd"
    if ($LASTEXITCODE -ne 0) { throw "VIS5.5 focused acceptance failed with exit code $LASTEXITCODE" }
}
finally {
