...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-1-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "=== ECO VIS1.1 isolated headless gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS1.1 Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) {
        throw "ECO VIS1.1 requires exact Godot $ExpectedGodotVersion"
    }
    Write-Host "ECO.VIS1.1 exact Godot identity: PASS"
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.1 Isolated Gate"
...
"@
    ...
    $copies = @(
        ...
        @("tests\research\ecology\test_eco_vis1_1_environment_proving_ground.gd", "tests\research\ecology\test_eco_vis1_1_environment_proving_ground.gd")
    )
    ...
    Write-Host "ECO.VIS1.1 isolated project without gameplay/MCP autoloads: PASS"

    Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis1_1_environment_proving_ground.gd") `
        -Label "ECO VIS1.1 parser preflight" | Out-Null
    Write-Host "ECO.VIS1.1 parser preflight: PASS"

    $smokeOutput = Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_1_environment_proving_ground.gd") `
        -Label "ECO VIS1.1 headless scene smoke"
    if ($smokeOutput -notmatch 'ECO\.VIS1\.1 headless scene smoke: PASS') {
        throw "ECO VIS1.1 PASS marker missing"
    }

    Write-Host "ECO.VIS1.1 automated gate: PASS"
}
finally {
