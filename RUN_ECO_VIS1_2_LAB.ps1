...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-2-lab-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.2 Spatial Ecology Projection"
run/main_scene="res://scenes/labs/ecology/eco_vis1_2_spatial_projection.tscn"
...
"@
    ...
    Write-Host "=== ECO VIS1.2 Spatial Ecology Projection ==="
    Write-Host "polygon=500x500m environment_seed=73191"
    Write-Host "source=canonical ECO OBS1 spatial demo timeline, frame=5"
    Write-Host "colored discs=lab patch projection"
    Write-Host "colored columns=final biomass per plant/lineage diagnostic"
    Write-Host "white links=dispersal graph"
    Write-Host "IMPORTANT: glyphs are diagnostics; PH5 plant geometry starts in VIS1.3"
    ...
    $process = Start-Process `
        -FilePath $GodotPath `
        -ArgumentList @("--path", $TempRoot, "res://scenes/labs/ecology/eco_vis1_2_spatial_projection.tscn") `
        -PassThru `
        -Wait

    if ($process.ExitCode -ne 0) {
        throw "ECO VIS1.2 graphical lab exited with code $($process.ExitCode)"
    }
}
finally {
