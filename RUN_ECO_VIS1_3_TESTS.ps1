...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-3-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "=== ECO VIS1.3 isolated PH5 population gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS1.3 Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) {
        throw "ECO VIS1.3 requires exact Godot $ExpectedGodotVersion"
    }
    Write-Host "ECO.VIS1.3 exact Godot identity: PASS"
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.3 Isolated Gate"
...
"@
    ...
    Write-Host "ECO.VIS1.3 isolated project with VIS1.2 + canonical PH5 dependencies: PASS"

    Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis1_3_ph5_population_materialization.gd") `
        -Label "ECO VIS1.3 parser preflight" | Out-Null
    Write-Host "ECO.VIS1.3 parser preflight: PASS"
    ...
    $vis13Output = Invoke-GodotProcess `
        ...
        -Label "ECO VIS1.3 headless scene smoke"
    if ($vis13Output -notmatch 'ECO\.VIS1\.3 headless scene smoke: PASS') {
        throw "ECO VIS1.3 PASS marker missing"
    }

    Write-Host "ECO.VIS1.3 automated gate: PASS"
}
finally {
