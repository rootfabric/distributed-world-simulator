...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-4-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "=== ECO VIS1.4 isolated readable population field gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS1.4 Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) {
        throw "ECO VIS1.4 requires exact Godot $ExpectedGodotVersion"
    }
    Write-Host "ECO.VIS1.4 exact Godot identity: PASS"
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.4 Isolated Gate"
...
"@
    ...
    Write-Host "ECO.VIS1.4 isolated project with VIS1.2 + VIS1.3 + canonical PH5 dependencies: PASS"

    Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis1_4_population_visual_field.gd") `
        -Label "ECO VIS1.4 parser preflight" | Out-Null
    Write-Host "ECO.VIS1.4 parser preflight: PASS"

    $vis12Output = Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_2_spatial_projection.gd") `
        -Label "ECO VIS1.2 regression smoke"
    if ($vis12Output -notmatch 'ECO\.VIS1\.2 headless scene smoke: PASS') {
        throw "ECO VIS1.2 regression PASS marker missing"
    }
    Write-Host "ECO.VIS1.2 regression gate: PASS"

    $vis13Output = Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_3_ph5_population_materialization.gd") `
        -Label "ECO VIS1.3 regression smoke"
    if ($vis13Output -notmatch 'ECO\.VIS1\.3 headless scene smoke: PASS') {
        throw "ECO VIS1.3 regression PASS marker missing"
    }
    Write-Host "ECO.VIS1.3 regression gate: PASS"

    $vis14Output = Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_4_population_visual_field.gd") `
        -Label "ECO VIS1.4 headless scene smoke"
    if ($vis14Output -notmatch 'ECO\.VIS1\.4 headless scene smoke: PASS') {
        throw "ECO VIS1.4 PASS marker missing"
    }

    Write-Host "ECO.VIS1.4 automated gate: PASS"
}
finally {
