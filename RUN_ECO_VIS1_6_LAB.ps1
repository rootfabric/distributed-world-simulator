...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-6-lab-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.6 Lineage Genome Field"
run/main_scene="res://scenes/labs/ecology/eco_vis1_6_lineage_genome_field.tscn"
[display]
...
"@
    ...
    Write-Host "=== ECO VIS1.6 Lineage / Genome Field ==="
    ...
    Write-Host "environment=VIS1.5 local EnvironmentSample coupling retained"
    Write-Host "lineage=PlantMutationLineageKernel + PlantLineageRecord, 3 selected generations per representative"
    Write-Host "genome_to_development=VIS1.6 lab-derived bridge; canonical alpha/beta genome truth remains OFF"
    Write-Host "geometry=GrowthGraph -> PH5 RenderDescription -> Plant3DMaterializer"
    ...
    $process = Start-Process -FilePath $GodotPath -ArgumentList @("--path", $TempRoot, "res://scenes/labs/ecology/eco_vis1_6_lineage_genome_field.tscn") -PassThru -Wait
    if ($process.ExitCode -ne 0) { throw "ECO VIS1.6 graphical lab exited with code $($process.ExitCode)" }
}
finally {
