...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-6-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "=== ECO VIS1.6 isolated lineage/genome field gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"
    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS1.6 Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) { throw "ECO VIS1.6 requires exact Godot $ExpectedGodotVersion" }
    Write-Host "ECO.VIS1.6 exact Godot identity: PASS"
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.6 Isolated Gate"
[display]
...
"@
    ...
    foreach ($relative in $copies) {
        ...
        if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "ECO VIS1.6 required file missing: $source" }
        $targetDir = Split-Path -Parent $target
        New-Item -ItemType Directory -Path $targetDir -Force | Out-Null
        Copy-Item -LiteralPath $source -Destination $target -Force
    }
    Write-Host "ECO.VIS1.6 isolated project with VIS1.2-VIS1.5 + lineage/mutation + canonical PH5/development dependencies: PASS"

    Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis1_6_lineage_genome_field.gd") -Label "ECO VIS1.6 parser preflight" | Out-Null
    Write-Host "ECO.VIS1.6 parser preflight: PASS"

    foreach ($stage in @("2","3","4","5")) {
        $testNames = @{"2"="spatial_projection";"3"="ph5_population_materialization";"4"="population_visual_field";"5"="environment_phenotype_field"}
        $testPath = "res://tests/research/ecology/test_eco_vis1_${stage}_$($testNames[$stage]).gd"
    ...
    $vis16Output = Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_6_lineage_genome_field.gd") -Label "ECO VIS1.6 headless scene smoke"
    if ($vis16Output -notmatch 'ECO\.VIS1\.6 headless scene smoke: PASS') { throw "ECO VIS1.6 PASS marker missing" }
    Write-Host "ECO.VIS1.6 automated gate: PASS"
}
finally {
