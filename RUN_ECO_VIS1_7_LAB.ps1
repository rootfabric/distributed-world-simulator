...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-7-lab-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.7 Temporal Evolution Scrubber"
run/main_scene="res://scenes/labs/ecology/eco_vis1_7_temporal_evolution_field.tscn"
[display]
...
"@
    ...
    Write-Host "=== ECO VIS1.7 Temporal Evolution Scrubber ==="
    Write-Host "polygon=500x500m environment_seed=73191"
    Write-Host "ecology_source=canonical VIS1.2 spatial snapshot, read-only"
    Write-Host "lineage_source=VIS1.6 deterministic lineage/genome selection"
    Write-Host "timeline=lab-derived generation scrubber 0..12; canonical ecology timeline truth remains OFF"
    Write-Host "positions_and_biomass=fixed across generations; only derived genome/phenotype/PH5 geometry changes"
    Write-Host "controls=Left/Right generation | Space play/pause | R generation 0 | WASD/QE camera | F1-F5 diagnostics"
    Write-Host "Close the Godot window to return to PowerShell."

    $process = Start-Process -FilePath $GodotPath -ArgumentList @("--path", $TempRoot, "res://scenes/labs/ecology/eco_vis1_7_temporal_evolution_field.tscn") -PassThru -Wait
    if ($process.ExitCode -ne 0) { throw "ECO VIS1.7 graphical lab exited with code $($process.ExitCode)" }
}
finally {
