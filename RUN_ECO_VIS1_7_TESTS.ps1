...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-7-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "=== ECO VIS1.7 isolated temporal evolution scrubber gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"
    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS1.7 Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) { throw "ECO VIS1.7 requires exact Godot $ExpectedGodotVersion" }
    Write-Host "ECO.VIS1.7 exact Godot identity: PASS"
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.7 Isolated Gate"
[display]
...
"@
    ...
    foreach ($relative in $copies) {
        ...
        if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "ECO VIS1.7 required file missing: $source" }
        $targetDir = Split-Path -Parent $target
        New-Item -ItemType Directory -Path $targetDir -Force | Out-Null
        Copy-Item -LiteralPath $source -Destination $target -Force
    }
    Write-Host "ECO.VIS1.7 isolated project with VIS1.2-VIS1.6 + temporal lineage + canonical PH5/development dependencies: PASS"

    Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis1_7_temporal_evolution_bridge.gd") -Label "ECO VIS1.7 bridge parser preflight" | Out-Null
    Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis1_7_temporal_evolution_field.gd") -Label "ECO VIS1.7 field parser preflight" | Out-Null
    Write-Host "ECO.VIS1.7 parser preflight: PASS"
    ...
    $bridgeOutput = Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_7_temporal_evolution_bridge.gd") -Label "ECO VIS1.7 temporal bridge"
    if ($bridgeOutput -notmatch 'ECO\.VIS1\.7 temporal bridge: PASS') { throw "ECO VIS1.7 temporal bridge PASS marker missing" }
    Write-Host "ECO.VIS1.7 temporal bridge gate: PASS"

    $vis17Output = Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_7_temporal_evolution_field.gd") -Label "ECO VIS1.7 headless scene smoke"
    if ($vis17Output -notmatch 'ECO\.VIS1\.7 headless scene smoke: PASS') { throw "ECO VIS1.7 PASS marker missing" }
    Write-Host "ECO.VIS1.7 automated gate: PASS"
}
finally {
