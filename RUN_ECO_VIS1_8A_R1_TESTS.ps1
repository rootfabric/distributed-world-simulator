...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-8a-r1-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "=== ECO VIS1.8A-R1 realtime presentation gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS1.8A-R1 Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) { throw "ECO VIS1.8A-R1 requires exact Godot $ExpectedGodotVersion" }
    Write-Host "ECO.VIS1.8A-R1 exact Godot identity: PASS"
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.8A-R1 Isolated Gate"
[display]
...
"@
    ...
    foreach ($relative in $copies) {
        ...
        if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "ECO VIS1.8A-R1 required file missing: $source" }
        $targetDir = Split-Path -Parent $target
        New-Item -ItemType Directory -Path $targetDir -Force | Out-Null
        Copy-Item -LiteralPath $source -Destination $target -Force
    }
    Write-Host "ECO.VIS1.8A-R1 isolated project with VIS1.2-VIS1.7 + turnover bridge + realtime presentation: PASS"

    Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis1_8a_realtime_turnover_field.gd") -Label "ECO VIS1.8A-R1 parser preflight" | Out-Null
    Write-Host "ECO.VIS1.8A-R1 parser preflight: PASS"

    $output = Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_8a_realtime_turnover_field.gd") -Label "ECO VIS1.8A-R1 headless realtime smoke"
    if ($output -notmatch 'ECO\.VIS1\.8A-R1 headless scene smoke: PASS') { throw "ECO VIS1.8A-R1 PASS marker missing" }
    Write-Host "ECO.VIS1.8A-R1 automated gate: PASS"
}
finally {
