...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis1-8b-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "=== ECO VIS1.8B continuous population evolution gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS1.8B Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) { throw "ECO VIS1.8B requires exact Godot $ExpectedGodotVersion" }
    Write-Host "ECO.VIS1.8B exact Godot identity: PASS"
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS1.8B Isolated Gate"
[display]
...
"@
    ...
    Write-Host "ECO.VIS1.8B isolated project with validated VIS1.8A-R1 + continuous rolling model: PASS"

    Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis1_8b_continuous_population_field.gd") -Label "ECO VIS1.8B parser preflight" | Out-Null
    Write-Host "ECO.VIS1.8B parser preflight: PASS"
    ...
    $output = Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_8b_continuous_population_field.gd") -Label "ECO VIS1.8B headless continuous smoke"
    if ($output -notmatch 'ECO\.VIS1\.8B headless scene smoke: PASS') { throw "ECO VIS1.8B PASS marker missing" }
    Write-Host "ECO.VIS1.8B automated gate: PASS"
}
finally {
