...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis2-0-" + [Guid]::NewGuid().ToString("N"))
...
function Invoke-GodotProcess {
    param([string[]]$Arguments, [string]$Label, [int]$TimeoutSeconds = 240)
    Write-Host ">> $Label"
...
try {
    ...
    Write-Host "=== ECO VIS2.0 Evolution Experiment Lab gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS2.0 Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) { throw "ECO VIS2.0 requires exact Godot $ExpectedGodotVersion" }
    Write-Host "ECO.VIS2.0 exact Godot identity: PASS"
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS2.0 Isolated Gate"
[display]
...
"@
    ...
    Write-Host "ECO.VIS2.0 isolated project with validated VIS1.9 + experiment dependencies: PASS"

    Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis2_0_evolution_experiment_lab.gd") -Label "ECO VIS2.0 parser preflight" | Out-Null
    Write-Host "ECO.VIS2.0 parser preflight: PASS"
    ...
    $vis19Output = Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis1_9_evolution_observatory.gd") -Label "ECO VIS1.9 regression smoke"
    if ($vis19Output -notmatch 'ECO\.VIS1\.9 headless scene smoke: PASS') { throw "ECO VIS1.9 regression PASS marker missing" }
    Write-Host "ECO.VIS1.9 regression gate: PASS"

    $output = Invoke-GodotProcess -Arguments @("--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis2_0_evolution_experiment_lab.gd") -Label "ECO VIS2.0 experiment smoke"
    if ($output -notmatch 'ECO\.VIS2\.0 headless scene smoke: PASS') { throw "ECO VIS2.0 PASS marker missing" }
    Write-Host "ECO.VIS2.0 automated gate: PASS"
}
finally {
