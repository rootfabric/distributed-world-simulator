...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis2-2a-" + [Guid]::NewGuid().ToString("N"))

# Keep the VIS2.2-A process gate at least as strict as the independently accepted
# VIS2.1-V gate (accepted code-under-test 3546520b744f3bf6e6dffc1a7fe7ef7d500e6f70).
...
function Test-GodotShutdownLeakOutput {
    ...
    foreach ($leakPattern in $script:GodotShutdownLeakPatterns) {
        if ($Output -match $leakPattern) { return $true }
    }
    return $false
}
...
function Assert-GodotShutdownLeakMatcherCoverage {
    ...
    foreach ($fixture in $leakingFixtures) {
        if (-not (Test-GodotShutdownLeakOutput -Output $fixture)) {
            throw "ECO.VIS2.2-A shutdown leak matcher missed fixture: $fixture"
        }
    }
    $benignFixtures = @(
        'ECO.VIS2.2-A replicated causal runner set: PASS (100 assertions)',
    ...
    foreach ($fixture in $benignFixtures) {
        if (Test-GodotShutdownLeakOutput -Output $fixture) {
            throw "ECO.VIS2.2-A shutdown leak matcher false-positive fixture: $fixture"
        }
    }
    Write-Host "ECO.VIS2.2-A shutdown leak matcher coverage: PASS"
}
...
try {
    ...
    Write-Host "=== ECO VIS2.2-A Replicated Causal Runner Set gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    Assert-GodotShutdownLeakMatcherCoverage

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS2.2-A Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) {
        throw "ECO VIS2.2-A requires exact Godot $ExpectedGodotVersion"
    }
    Write-Host "ECO.VIS2.2-A exact Godot identity: PASS"

    $vis21vRunner = Join-Path $RepoRoot "RUN_ECO_VIS2_1V_TESTS.ps1"
    if (-not (Test-Path -LiteralPath $vis21vRunner -PathType Leaf)) {
        throw "Accepted VIS2.1-V runner missing: $vis21vRunner"
    }
    Write-Host ">> ECO VIS2.1-V independently accepted regression gate"
    & $vis21vRunner -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS2.1-V regression runner failed with exit code $LASTEXITCODE"
    }
    Write-Host "ECO.VIS2.1-V accepted regression gate: PASS"

    $projectConfig = @"
[application]
config/name="ECO VIS2.2-A Replicated Causal Runner Set Isolated Gate"
[rendering]
...
"@
    ...
    foreach ($relativeDir in @("scripts\labs\ecology", "scripts\research\ecology", "scenes\labs\ecology")) {
        ...
        if (-not (Test-Path -LiteralPath $sourceDir -PathType Container)) {
            throw "Required ecology directory missing: $sourceDir"
        }
        New-Item -ItemType Directory -Path (Split-Path -Parent $targetDir) -Force | Out-Null
        Copy-Item -LiteralPath $sourceDir -Destination $targetDir -Recurse -Force
    }

    $testRelative = "tests\research\ecology\test_eco_vis2_2_replicate_pair_set.gd"
    ...
    if (-not (Test-Path -LiteralPath $testSource -PathType Leaf)) {
        throw "VIS2.2-A test missing: $testSource"
    }
    ...
    Write-Host "ECO.VIS2.2-A isolated ecology dependency graph: PASS"

    Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis2_2_replicate_pair_set.gd") `
        -Label "ECO VIS2.2-A parser preflight" `
        -TimeoutSeconds 240 | Out-Null
    Write-Host "ECO.VIS2.2-A parser preflight: PASS"

    Write-Host "ECO.VIS2.2-A shutdown leak gate: STRICT (ObjectDB + RID + resources + StringName + verbose smoke)"
    $output = Invoke-GodotProcess `
        -Arguments @("--verbose", "--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis2_2_replicate_pair_set.gd") `
        -Label "ECO VIS2.2-A replicated causal runner smoke" `
        -TimeoutSeconds 360 `
        -PassMarker 'ECO\.VIS2\.2-A replicated causal runner set: PASS'

    Write-Host "ECO.VIS2.2-A automated gate: PASS"
}
finally {
