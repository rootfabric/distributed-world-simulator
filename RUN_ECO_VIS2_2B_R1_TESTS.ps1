...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis2-2b-r1-" + [Guid]::NewGuid().ToString("N"))
...
function Assert-GodotShutdownLeakMatcherCoverage {
    ...
    foreach ($fixture in $leakingFixtures) {
        if (-not (Test-GodotShutdownLeakOutput -Output $fixture)) {
            throw "ECO.VIS2.2-B R1 shutdown leak matcher missed fixture: $fixture"
        }
    }
    $benignFixtures = @(
        'ECO.VIS2.2-B R1 canonical aggregate effect model: PASS (100 assertions)',
    ...
    foreach ($fixture in $benignFixtures) {
        if (Test-GodotShutdownLeakOutput -Output $fixture) {
            throw "ECO.VIS2.2-B R1 shutdown leak matcher false-positive fixture: $fixture"
        }
    }
    Write-Host "ECO.VIS2.2-B R1 shutdown leak matcher coverage: PASS"
}
...
try {
    ...
    Write-Host "=== ECO VIS2.2-B R1 Canonical Aggregate Repair gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    Assert-GodotShutdownLeakMatcherCoverage

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS2.2-B R1 Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) {
        throw "ECO VIS2.2-B R1 requires exact Godot $ExpectedGodotVersion"
    }
    Write-Host "ECO.VIS2.2-B R1 exact Godot identity: PASS"

    $vis22aRunner = Join-Path $RepoRoot "RUN_ECO_VIS2_2A_TESTS.ps1"
    if (-not (Test-Path -LiteralPath $vis22aRunner -PathType Leaf)) {
        throw "Windows-validated VIS2.2-A runner missing: $vis22aRunner"
    }
    ...
    $projectConfig = @"
[application]
config/name="ECO VIS2.2-B R1 Canonical Aggregate Repair Isolated Gate"
[rendering]
...
"@
    ...
    $testRelative = "tests\research\ecology\test_eco_vis2_2_aggregate_effect_model_r1.gd"
    ...
    if (-not (Test-Path -LiteralPath $testSource -PathType Leaf)) {
        throw "VIS2.2-B R1 test missing: $testSource"
    }
    ...
    Write-Host "ECO.VIS2.2-B R1 isolated ecology dependency graph: PASS"

    Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis2_2_aggregate_effect_model_r1.gd") `
        -Label "ECO VIS2.2-B R1 parser preflight" `
        -TimeoutSeconds 240 | Out-Null
    Write-Host "ECO.VIS2.2-B R1 parser preflight: PASS"

    Write-Host "ECO.VIS2.2-B R1 shutdown leak gate: STRICT (ObjectDB + RID + resources + StringName + verbose smoke)"
    $output = Invoke-GodotProcess `
        -Arguments @("--verbose", "--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis2_2_aggregate_effect_model_r1.gd") `
        -Label "ECO VIS2.2-B R1 canonical aggregate smoke" `
        -TimeoutSeconds 420 `
        -PassMarker 'ECO\.VIS2\.2-B R1 canonical aggregate effect model: PASS'

    Write-Host "ECO.VIS2.2-B R1 automated gate: PASS"
}
finally {
