...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis2-2b-" + [Guid]::NewGuid().ToString("N"))

# Exact semantic shutdown matcher carried forward from the Windows-validated
# VIS2.2-A / independently accepted VIS2.1-V acceptance lineage.
...
function Assert-GodotShutdownLeakMatcherCoverage {
    ...
    foreach ($fixture in $leakingFixtures) {
        if (-not (Test-GodotShutdownLeakOutput -Output $fixture)) {
            throw "ECO.VIS2.2-B shutdown leak matcher missed fixture: $fixture"
        }
    }
    $benignFixtures = @(
        'ECO.VIS2.2-B aggregate effect model: PASS (100 assertions)',
    ...
    foreach ($fixture in $benignFixtures) {
        if (Test-GodotShutdownLeakOutput -Output $fixture) {
            throw "ECO.VIS2.2-B shutdown leak matcher false-positive fixture: $fixture"
        }
    }
    Write-Host "ECO.VIS2.2-B shutdown leak matcher coverage: PASS"
}
...
try {
    ...
    Write-Host "=== ECO VIS2.2-B Aggregate Effect Model gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    Assert-GodotShutdownLeakMatcherCoverage

    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS2.2-B Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) {
        throw "ECO VIS2.2-B requires exact Godot $ExpectedGodotVersion"
    }
    Write-Host "ECO.VIS2.2-B exact Godot identity: PASS"

    $vis22aRunner = Join-Path $RepoRoot "RUN_ECO_VIS2_2A_TESTS.ps1"
    if (-not (Test-Path -LiteralPath $vis22aRunner -PathType Leaf)) {
        throw "Validated VIS2.2-A runner missing: $vis22aRunner"
    }
    Write-Host ">> ECO VIS2.2-A Windows-validated regression gate"
    & $vis22aRunner -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) {
        throw "VIS2.2-A regression runner failed with exit code $LASTEXITCODE"
    }
    Write-Host "ECO.VIS2.2-A validated regression gate: PASS"

    $projectConfig = @"
[application]
config/name="ECO VIS2.2-B Aggregate Effect Model Isolated Gate"
[rendering]
...
"@
    ...
    $testRelative = "tests\research\ecology\test_eco_vis2_2_aggregate_effect_model.gd"
    ...
    if (-not (Test-Path -LiteralPath $testSource -PathType Leaf)) {
        throw "VIS2.2-B test missing: $testSource"
    }
    ...
    Write-Host "ECO.VIS2.2-B isolated ecology dependency graph: PASS"

    Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis2_2_aggregate_effect_model.gd") `
        -Label "ECO VIS2.2-B parser preflight" `
        -TimeoutSeconds 240 | Out-Null
    Write-Host "ECO.VIS2.2-B parser preflight: PASS"

    Write-Host "ECO.VIS2.2-B shutdown leak gate: STRICT (ObjectDB + RID + resources + StringName + verbose smoke)"
    $output = Invoke-GodotProcess `
        -Arguments @("--verbose", "--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis2_2_aggregate_effect_model.gd") `
        -Label "ECO VIS2.2-B aggregate effect model smoke" `
        -TimeoutSeconds 420 `
        -PassMarker 'ECO\.VIS2\.2-B aggregate effect model: PASS'

    Write-Host "ECO.VIS2.2-B automated gate: PASS"
}
finally {
