...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis2-2c-" + [Guid]::NewGuid().ToString("N"))
...
function Test-GodotShutdownLeakOutput {
    param([string]$Output)
    foreach ($pattern in $script:GodotShutdownLeakPatterns) {
...
function Assert-GodotShutdownLeakMatcherCoverage {
    ...
    foreach ($fixture in $leakingFixtures) {
        if (-not (Test-GodotShutdownLeakOutput -Output $fixture)) {
            throw "ECO.VIS2.2-C shutdown matcher missed fixture: $fixture"
        }
    }
    $benignFixtures = @(
        'ECO.VIS2.2-C observatory panel: PASS (40 assertions)',
    ...
    foreach ($fixture in $benignFixtures) {
        if (Test-GodotShutdownLeakOutput -Output $fixture) {
            throw "ECO.VIS2.2-C shutdown matcher false-positive fixture: $fixture"
        }
    }
    Write-Host "ECO.VIS2.2-C shutdown leak matcher coverage: PASS"
}
...
function Invoke-GodotProcess {
    ...
    try {
        ...
        if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
            try { $process.Kill() } catch { Write-Warning "$Label Kill() reported: $($_.Exception.Message)" }
            $process.WaitForExit()
...
try {
    ...
    Write-Host "=== ECO VIS2.2-C Replicated Observatory Panel gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    Assert-GodotShutdownLeakMatcherCoverage
    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS2.2-C Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) { throw "ECO.VIS2.2-C requires exact Godot $ExpectedGodotVersion" }
    Write-Host "ECO.VIS2.2-C exact Godot identity: PASS"

    $vis22bRunner = Join-Path $RepoRoot "RUN_ECO_VIS2_2B_R2_TESTS.ps1"
    if (-not (Test-Path -LiteralPath $vis22bRunner -PathType Leaf)) { throw "VIS2.2-B R2 runner missing: $vis22bRunner" }
    Write-Host ">> ECO VIS2.2-B R2 canonical aggregate regression gate"
    & $vis22bRunner -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) { throw "VIS2.2-B R2 regression runner failed with exit code $LASTEXITCODE" }
    Write-Host "ECO.VIS2.2-B R2 validated regression gate: PASS"

    $projectConfig = @"
[application]
config/name="ECO VIS2.2-C Replicated Observatory Panel Isolated Gate"
[display]
...
"@
    ...
    $panelRelative = "scripts\labs\ecology\eco_vis2_2_observatory_panel.gd"
    $panelSource = Join-Path $RepoRoot $panelRelative
    $panelTarget = Join-Path $TempRoot $panelRelative
    if (-not (Test-Path -LiteralPath $panelSource -PathType Leaf)) { throw "VIS2.2-C panel missing: $panelSource" }
    New-Item -ItemType Directory -Path (Split-Path -Parent $panelTarget) -Force | Out-Null
    Copy-Item -LiteralPath $panelSource -Destination $panelTarget -Force

    $testRelative = "tests\research\ecology\test_eco_vis2_2_observatory_panel.gd"
    $testSource = Join-Path $RepoRoot $testRelative
    $testTarget = Join-Path $TempRoot $testRelative
    if (-not (Test-Path -LiteralPath $testSource -PathType Leaf)) { throw "VIS2.2-C test missing: $testSource" }
    ...
    Write-Host "ECO.VIS2.2-C isolated presentation dependency graph: PASS"

    Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis2_2_observatory_panel.gd") `
        -Label "ECO VIS2.2-C parser preflight" `
        -TimeoutSeconds 180 | Out-Null
    Write-Host "ECO.VIS2.2-C parser preflight: PASS"

    Write-Host "ECO.VIS2.2-C shutdown leak gate: STRICT (ObjectDB + RID + resources + StringName + verbose smoke)"
    Invoke-GodotProcess `
        -Arguments @("--verbose", "--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis2_2_observatory_panel.gd") `
        -Label "ECO VIS2.2-C observatory panel smoke" `
        -TimeoutSeconds 240 `
        -PassMarker 'ECO\.VIS2\.2-C observatory panel: PASS' | Out-Null

    Write-Host "ECO.VIS2.2-C automated gate: PASS"
}
finally {
