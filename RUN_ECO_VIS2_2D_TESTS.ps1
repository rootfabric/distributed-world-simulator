...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dws-eco-vis2-2d-" + [Guid]::NewGuid().ToString("N"))
...
function Assert-GodotShutdownLeakMatcherCoverage {
    ...
    foreach ($fixture in $leakingFixtures) {
        if (-not (Test-GodotShutdownLeakOutput -Output $fixture)) {
            throw "ECO.VIS2.2-D shutdown matcher missed fixture: $fixture"
        }
    }
    $benignFixtures = @(
        'ECO.VIS2.2-D integrated observatory lab: PASS (50 assertions)',
    ...
    foreach ($fixture in $benignFixtures) {
        if (Test-GodotShutdownLeakOutput -Output $fixture) {
            throw "ECO.VIS2.2-D shutdown matcher false-positive fixture: $fixture"
        }
    }
    Write-Host "ECO.VIS2.2-D shutdown leak matcher coverage: PASS"
}
...
try {
    ...
    Write-Host "=== ECO VIS2.2-D Integrated Replicated Observatory gate ==="
    Write-Host "repo_root=$RepoRoot"
    Write-Host "godot=$GodotPath"

    Assert-GodotShutdownLeakMatcherCoverage
    $versionOutput = Invoke-GodotProcess -Arguments @("--version") -Label "ECO VIS2.2-D Godot version check"
    if ($versionOutput -notmatch [Regex]::Escape($ExpectedGodotVersion)) { throw "ECO.VIS2.2-D requires exact Godot $ExpectedGodotVersion" }
    Write-Host "ECO.VIS2.2-D exact Godot identity: PASS"

    $vis22cRunner = Join-Path $RepoRoot "RUN_ECO_VIS2_2C_TESTS.ps1"
    if (-not (Test-Path -LiteralPath $vis22cRunner -PathType Leaf)) { throw "VIS2.2-C runner missing: $vis22cRunner" }
    Write-Host ">> ECO VIS2.2-C Windows-validated regression gate"
    & $vis22cRunner -GodotPath $GodotPath
    if ($LASTEXITCODE -ne 0) { throw "VIS2.2-C regression runner failed with exit code $LASTEXITCODE" }
    Write-Host "ECO.VIS2.2-C validated regression gate: PASS"

    $projectConfig = @"
[application]
config/name="ECO VIS2.2-D Integrated Replicated Observatory Isolated Gate"
[display]
...
"@
    ...
    $testRelative = "tests\research\ecology\test_eco_vis2_2_integrated_observatory_lab.gd"
    $testSource = Join-Path $RepoRoot $testRelative
    $testTarget = Join-Path $TempRoot $testRelative
    if (-not (Test-Path -LiteralPath $testSource -PathType Leaf)) { throw "VIS2.2-D test missing: $testSource" }
    ...
    Write-Host "ECO.VIS2.2-D isolated ecology dependency graph: PASS"

    Invoke-GodotProcess `
        -Arguments @("--headless", "--path", $TempRoot, "--check-only", "--script", "res://tests/research/ecology/test_eco_vis2_2_integrated_observatory_lab.gd") `
        -Label "ECO VIS2.2-D parser preflight" `
        -TimeoutSeconds 300 | Out-Null
    Write-Host "ECO.VIS2.2-D parser preflight: PASS"

    Write-Host "ECO.VIS2.2-D shutdown leak gate: STRICT (ObjectDB + RID + resources + StringName + verbose smoke)"
    Invoke-GodotProcess `
        -Arguments @("--verbose", "--headless", "--path", $TempRoot, "--script", "res://tests/research/ecology/test_eco_vis2_2_integrated_observatory_lab.gd") `
        -Label "ECO VIS2.2-D integrated observatory smoke" `
        -TimeoutSeconds 480 `
        -PassMarker 'ECO\.VIS2\.2-D integrated observatory lab: PASS' | Out-Null

    Write-Host "ECO.VIS2.2-D automated gate: PASS"
}
finally {
