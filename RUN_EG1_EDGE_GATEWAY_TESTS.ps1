...
$ReportPath = Join-Path $ReportDirectory "eg1-edge-gateway-summary.json"
...
$Tests = @(
    "res://tests/network/test_eg1_gateway_contracts.gd",
    "res://tests/network/test_eg1_gateway_forwarder.gd",
    "res://tests/network/test_eg1_gateway_session_control.gd",
    "res://tests/network/test_eg1_zero_write_fence.gd",
    "res://tests/network/test_eg1_dual_role_enet_spike.gd",
    "res://tests/network/test_eg1_gateway_loopback_equivalence.gd",
    "res://tests/network/test_eg1_gateway_processes.gd"
)

# Failure markers any EG1 test may print on an assertion breach.
$FailureMarkers = @("][FAIL]", '"verdict": "FAIL"', "SCRIPT ERROR:", "PREDICATE_NOT_DEMONSTRATED")

$Summary = [ordered]@{
    schema = "planet_simulator.eg1_edge_gateway_suite_summary.v1"
    stage = "EG1_SINGLE_GATEWAY_PASS_THROUGH"
    godot = $Godot
...
try {
    ...
    Write-Host "Stage: EG1 SINGLE_GATEWAY_PASS_THROUGH"
    ...
    $Summary.passed = $true
    Save-Summary
    Write-Host "EG1 single-gateway pass-through suite: PASS" -ForegroundColor Green
