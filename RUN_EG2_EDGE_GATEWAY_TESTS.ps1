...
$ReportPath = Join-Path $ReportDirectory "eg2-edge-gateway-summary.json"
...
$Tests = @(
    "res://tests/network/test_eg2_auth_session_service.gd",
    "res://tests/network/test_eg2_world_directory.gd",
    "res://tests/network/test_eg2_placement_flow.gd",
    "res://tests/network/test_eg2_endpoint_disclosure_fence.gd",
    "res://tests/network/test_eg2_gateway_processes.gd"
)

# Failure markers any EG2 test may print on an assertion breach.
$FailureMarkers = @("][FAIL]", '"verdict": "FAIL"', "SCRIPT ERROR:", "PREDICATE_NOT_DEMONSTRATED")

$Summary = [ordered]@{
    schema = "planet_simulator.eg2_edge_gateway_suite_summary.v1"
    stage = "EG2_AUTH_SESSION_PLACEMENT"
    godot = $Godot
...
try {
    ...
    Write-Host "Stage: EG2 AUTH_SESSION_PLACEMENT"
    ...
    $Summary.passed = $true
    Save-Summary
    Write-Host "EG2 auth/session placement suite: PASS" -ForegroundColor Green
