...
$ReportPath = Join-Path $ReportDirectory "eg3-edge-gateway-summary.json"
...
$Tests = @(
    "res://tests/network/test_eg3_backend_multiplexer.gd",
    "res://tests/network/test_eg3_multiplexed_link.gd",
    "res://tests/network/test_eg3_gateway_processes.gd"
)

# Failure markers any EG3 test may print on an assertion breach.
$FailureMarkers = @("][FAIL]", '"verdict": "FAIL"', "SCRIPT ERROR:", "PREDICATE_NOT_DEMONSTRATED")

$Summary = [ordered]@{
    schema = "planet_simulator.eg3_edge_gateway_suite_summary.v1"
    stage = "EG3_SHARED_MULTIPLEXED_BACKEND_TUNNEL"
    godot = $Godot
...
try {
    ...
    Write-Host "Stage: EG3 SHARED_MULTIPLEXED_BACKEND_TUNNEL"
    ...
    $Summary.passed = $true
    Save-Summary
    Write-Host "EG3 shared multiplexed backend tunnel suite: PASS" -ForegroundColor Green
