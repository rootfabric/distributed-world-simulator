...
if (-not (Test-Path (Join-Path $repoRoot '.godot'))) {
    Write-Host ('[eg5-runner] running mandatory headless import pass before suites...')
...
$summary = [ordered]@{
    schema       = 'planet_simulator.eg5_edge_gateway_suite_summary.v1'
    started_at   = (Get-Date).ToString('o')
    runner       = 'RUN_EG5_EDGE_GATEWAY_TESTS.ps1'
    passed       = $true
    failure_markers = @()
    suites       = @()
    exit_code    = 0
...
foreach ($t in $tests) {
    Write-Host ('[eg5-runner] running ' + $t)
...
$tmpJson = Join-Path $artifactsDir ('.eg5-edge-gateway-summary.' + [guid]::NewGuid().ToString('N') + '.json')
$json = $summary | ConvertTo-Json -Depth 6
$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText($tmpJson, $json, $utf8NoBom)
$finalJson = Join-Path $artifactsDir 'eg5-edge-gateway-summary.json'
...
if (-not $summary.passed) {
    ...
    Write-Host ('[eg5-runner] FAILED: see ' + $finalJson)
    exit 1
}
Write-Host ('[eg5-runner] PASS: ' + $finalJson)
exit 0
