...
if [[ -x "$repo_root/RUN_FABRIC_BAKE_B0_3_TESTS.sh" ]]; then
    GODOT_BIN="$godot_bin" "$repo_root/RUN_FABRIC_BAKE_B0_3_TESTS.sh"
fi
...
run_script() {
    ...
    if grep -Eq 'SCRIPT ERROR:|ERROR: Failed to load script' "$log"; then
        echo "B0.4-A fatal Godot script marker detected: $script" >&2
        rm -f "$log"
        exit 4
    fi
    rm -f "$log"
...
run_script res://tests/research/fabric_bake0/fabric_bake_b0_4_a_acceptance.gd
run_script res://tests/research/fabric_bake0/fabric_bake_b0_4_a_playground.gd
