...
$ExpectedBranch = "research/fabric-bake0-5-hybrid-bake-preflight-r1"
...
Write-Host "FABRIC-BAKE B0.5-P0 EXACT WINDOWS VERIFICATION"
...
try {
    ...
    $output = @(& $GodotPath --headless --path $RootDir --script res://tests/research/fabric_bake0/fabric_bake_b0_5_p0_acceptance.gd 2>&1)
    $exitCode = $LASTEXITCODE
    $output | ForEach-Object { Write-Host $_ }
    if ($exitCode -ne 0) { throw "B0.5-P0 acceptance failed: exit $exitCode" }
    $joined = $output -join "`n"
    if ($joined -match "SCRIPT ERROR:|ERROR: Failed to load script") {
        throw "B0.5-P0 emitted fatal script marker"
    }
    if ($joined -notmatch "FABRIC-BAKE B0\.5-P0 Acceptance: PASS \([0-9]+ assertions\)") {
        throw "B0.5-P0 PASS banner missing"
    }
}
finally {
...
Write-Host "FABRIC-BAKE B0.5-P0: PASS"
