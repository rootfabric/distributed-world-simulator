...
if [[ ! -x "$GODOT_BIN" ]]; then
  echo "BRIDGE-2-C: canonical double Godot not executable: $GODOT_BIN" >&2
  exit 2
fi
version="$("$GODOT_BIN" --version | head -n1 | tr -d '\r')"
if [[ "$version" != "$EXPECTED_GODOT_VERSION" ]]; then
  echo "BRIDGE-2-C: wrong Godot version: $version" >&2
  exit 3
fi
...
run_script() {
  ...
  if grep -Eq 'SCRIPT ERROR:|ERROR: Failed to load script' "$log"; then
    echo "BRIDGE-2-C: fatal Godot script marker in $script" >&2
    rm -f "$log"
    return 4
  fi
  if ! grep -Fq "$marker" "$log"; then
    echo "BRIDGE-2-C: PASS marker missing in $script" >&2
    rm -f "$log"
    return 5
  fi
  rm -f "$log"
...
run_script \
  res://tests/research/fabric_bake0/fabric_bake_bridge2_c_acceptance.gd \
  "FABRIC-BAKE BRIDGE-2-C Cross-Representation Event Routing Acceptance: PASS"

run_script \
  res://tests/research/fabric_bake0/fabric_bake_bridge2_c_playground.gd \
  "FABRIC-BAKE BRIDGE-2-C Playground: PASS"

echo "FABRIC-BAKE BRIDGE-2-C: PASS"
