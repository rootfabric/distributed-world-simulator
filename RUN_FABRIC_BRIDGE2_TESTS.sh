...
trap 'rm -f "$log"' EXIT
set +e
"$godot_bin" --headless --path "$repo_root" --script res://tests/research/fabric_bake0/fabric_bridge2_mixed_generic_machine_r1_acceptance.gd 2>&1 | tee "$log"
status=${PIPESTATUS[0]}
set -e
if [[ "$status" -ne 0 ]]; then exit "$status"; fi
if grep -Eq 'SCRIPT ERROR:|ERROR: Failed to load script' "$log"; then
  echo "BRIDGE-2 fatal script marker detected" >&2
  exit 4
fi
grep -Eq 'FABRIC BRIDGE-2 Mixed Generic Machine R1 Acceptance: PASS \([0-9]+ assertions\)' "$log"
