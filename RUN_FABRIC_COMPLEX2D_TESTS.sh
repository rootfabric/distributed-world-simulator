...
TIMEOUT_SECONDS="${COMPLEX2D_SCRIPT_TIMEOUT_SECONDS:-240}"
test -x "$GODOT_BIN"
...
timeout "${TIMEOUT_SECONDS}s" env \
  ...
  "$GODOT_BIN" --headless --path "$PROJECT" --script res://tests/research/fabric_bake0/fabric_bake_complex2d_independent_structural_failure_acceptance.gd >"$LOG" 2>&1
cat "$LOG"
! grep -Eq 'SCRIPT ERROR:|ERROR: Failed to load script' "$LOG"
grep -Fq "FABRIC COMPLEX2-D Independent Structural Failure Acceptance: PASS" "$LOG"
grep -Eq '^COMPLEX2D_EXPERIMENT_HASH=[0-9a-f]{64}$' "$LOG"
echo "FABRIC COMPLEX2-D INDEPENDENT STRUCTURAL FAILURE TESTS: PASS"
