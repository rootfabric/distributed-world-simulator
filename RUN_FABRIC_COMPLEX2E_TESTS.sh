...
TIMEOUT_SECONDS="${COMPLEX2E_SCRIPT_TIMEOUT_SECONDS:-300}"
test -x "$GODOT_BIN"
...
timeout "${TIMEOUT_SECONDS}s" env \
  ...
  "$GODOT_BIN" --headless --path "$PROJECT" --script res://tests/research/fabric_bake0/fabric_bake_complex2e_settle_rebake_reimpact_acceptance.gd >"$LOG" 2>&1
cat "$LOG"
! grep -Eq 'SCRIPT ERROR:|ERROR: Failed to load script' "$LOG"
grep -Fq "FABRIC COMPLEX2-E Settle Rebake Re-impact Acceptance: PASS" "$LOG"
grep -Eq '^COMPLEX2E_EXPERIMENT_HASH=[0-9a-f]{64}$' "$LOG"
echo "FABRIC COMPLEX2-E SETTLE REBAKE RE-IMPACT TESTS: PASS"
