...
OUT="${T13_EVIDENCE_DIR:-$ROOT/artifacts/fabric-r5-2-t13}"
mkdir -p "$OUT"
...
for i in 1 2 3; do
  ...
  grep -F 'FABRIC R5.2 T13 SHARED INSTANCES: PASS' "$OUT/sample-$i.log"
done
python3 scripts/research/fabric_bake0/collect_r5_2_t13_evidence.py --root "$OUT" --out "$OUT/evidence.json"
timeout --kill-after=5s 180s "$GODOT_BIN" --headless --path "$ROOT" \
  --script res://tests/research/fabric_bake0/fabric_r5_2_t12_ship_acceptance.gd \
  >"$OUT/t12-regression.log" 2>&1
grep -F 'FABRIC R5.2 T12 SHIP MATRYOSHKA: PASS' "$OUT/t12-regression.log"
test -z "$(git status --porcelain=v1 --untracked-files=no)"
