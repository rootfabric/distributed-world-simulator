...
timeout --kill-after=3s 30s \
  ...
  --script res://tests/research/fabric_bake0/fabric_r5_2_t7_gearbox_acceptance.gd \
  -- --preflight >"$preflight" 2>&1
preflight_rc=$?
set -e
cat "$preflight"
test "$preflight_rc" -eq 0
grep -F 'FABRIC_R5_2_T7_PREFLIGHT=PASS' "$preflight"
if grep -Eiq 'SCRIPT ERROR|Parse Error|Compile Error|Invalid call|Invalid access|ERROR:' "$preflight"; then
  echo "R5.2 T7 preflight detected script/runtime error" >&2
  exit 1
fi
