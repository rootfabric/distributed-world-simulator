...
foreach ($RequiredPath in @($G4Runner, $G5Runner, $WorldRunner)) {
    if (-not (Test-Path -LiteralPath $RequiredPath -PathType Leaf)) { throw "Required G5 acceptance runner is missing: $RequiredPath" }
}
...
try {
    ...
    Write-Host "=== G4 accepted dependency focused gate ==="
    & $G4Runner
    if (-not $?) { throw "G4 dependency focused gate failed." }

    Write-Host "=== G5 World Feature Graph ==="
    & $G5Runner
    if (-not $?) { throw "G5 focused runner failed." }
    ...
    $BaseRef = "origin/feature/g4-provider-composition-replacement"
    & git -C $RootDir rev-parse --verify $BaseRef *> $null
    if ($LASTEXITCODE -ne 0) { $BaseRef = "feature/g4-provider-composition-replacement" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Unable to enumerate G5 changed files against $BaseRef" }
    $FrozenPaths = @(
    ...
    if ($FrozenChanges.Count -gt 0) { throw "G5 modified frozen G0-G4 architecture paths: $($FrozenChanges -join ', ')" }
    ...
    if ($ProductionChanges.Count -gt 0) { throw "G5 unexpectedly modified production/runtime paths: $($ProductionChanges -join ', ')" }

    Write-Host "G5 full acceptance gate: PASS"
}
finally {
