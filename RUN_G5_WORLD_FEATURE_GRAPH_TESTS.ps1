...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "G5 headless editor import failed" }
    & $GodotExecutable --headless --path $RootDir --script "res://tests/procedural/features/g5_world_feature_graph_acceptance.gd"
    if ($LASTEXITCODE -ne 0) { throw "G5 World Feature Graph acceptance failed" }
    & $GodotExecutable --headless --path $RootDir --script "res://tests/procedural/features/g5_feature_cell_identity_acceptance.gd"
    if ($LASTEXITCODE -ne 0) { throw "G5 feature/cell identity acceptance failed" }
    & $GodotExecutable --headless --path $RootDir --scene "res://scenes/labs/procedural/g5_world_feature_graph_lab.tscn" --quit-after 2
    if ($LASTEXITCODE -ne 0) { throw "G5 visual lab headless launch failed" }
}
finally {
