...
function Test-G70AllowedPath {
    param([Parameter(Mandatory = $true)][string]$Path)
    if ($Path -in @(
        "RUN_G7_0_SEMANTIC_FIELD_CONTRACTS_TESTS.ps1",
        "RUN_G7_0_SEMANTIC_FIELD_CONTRACTS_TESTS.sh",
    ...
    if ($Path -match '^scripts/simulation/procedural/semantic_fields/semantic_field_registry_v1\.gd(\.uid)?$') { return $true }
    if ($Path -match '^tests/procedural/semantic_fields/g7_0_semantic_field_contracts_acceptance\.gd(\.uid)?$') { return $true }
    return $false
}

if ($null -eq (Get-Command git -ErrorAction SilentlyContinue)) {
    throw "git is required for G7.0 full acceptance"
}
...
Write-Host "=== G7.0 FULL ACCEPTANCE: repository / P0 preflight ==="
$StatusBefore = Invoke-GitText @("status", "--porcelain")
if (-not [string]::IsNullOrWhiteSpace($StatusBefore)) {
    throw "G7.0 full acceptance requires a clean working tree:`n$StatusBefore"
}
foreach ($Ref in @($MainRef, $G6Ref)) {
    & git -C $RootDir rev-parse --verify --quiet $Ref | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw "Missing required ref $Ref. Run git fetch origin first."
    }
}
...
Assert-Equal $LocalGlobalBlob $MainGlobalBlob "G7.0 global config differs from main"
Assert-Equal $LocalGlobalBlob $G6GlobalBlob "G7.0 global config differs from G6"
...
if ($LASTEXITCODE -ne 0) {
    throw "Current G7.0 head is not synchronized on top of current G6"
}

$G6Record = Get-Content -LiteralPath (Join-Path $RootDir "validation/g6-full-acceptance-validation.json") -Raw | ConvertFrom-Json
if ([string]$G6Record.decision -ne "SOURCE_ACCEPTED") {
    throw "G6 source-acceptance record is missing or stale"
}

Write-Host "=== G7.0 FULL ACCEPTANCE: scope / hygiene ==="
...
if ($ChangedFiles.Count -eq 0) { throw "G7.0 diff is empty" }
$Unexpected = @($ChangedFiles | Where-Object { -not (Test-G70AllowedPath $_) })
if ($Unexpected.Count -gt 0) {
    throw "G7.0 changed files outside the contract-first allowlist:`n$($Unexpected -join "`n")"
}
& git -C $RootDir diff --check "$G6Ref...HEAD"
if ($LASTEXITCODE -ne 0) { throw "git diff --check failed for G6...G7.0" }
Write-Host "G7.0 changed-file scope: PASS ($($ChangedFiles.Count) files)"
...
try {
    ...
    Write-Host "=== G7.0 FULL ACCEPTANCE: focused contracts ==="
    Invoke-PowerShellChild (Join-Path $RootDir "RUN_G7_0_SEMANTIC_FIELD_CONTRACTS_TESTS.ps1") @("-GodotPath", $GodotPath)

    Write-Host "=== G7.0 FULL ACCEPTANCE: full world/core regression ==="
...
Write-Host "=== G7.0 FULL ACCEPTANCE: final hygiene ==="
$StatusAfter = Invoke-GitText @("status", "--porcelain")
if (-not [string]::IsNullOrWhiteSpace($StatusAfter)) {
    throw "G7.0 full acceptance left tracked/untracked changes:`n$StatusAfter"
}
& git -C $RootDir diff --check "$G6Ref...HEAD"
if ($LASTEXITCODE -ne 0) { throw "Final git diff --check failed" }

Write-Host "G7.0 FULL ACCEPTANCE: PASS"
Write-Host "Global revision: $GlobalRevision"
Write-Host "G6 SOURCE_ACCEPTED ancestor: PASS"
Write-Host "G7.0 contract-first scope: PASS"
Write-Host "G7.0 focused contracts: PASS"
