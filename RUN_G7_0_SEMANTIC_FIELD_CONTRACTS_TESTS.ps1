...
try {
    ...
    Write-Host "=== G7.0 editor import / parse ==="
    & $GodotExecutable --headless --editor --path $RootDir --quit
    if ($LASTEXITCODE -ne 0) { throw "G7.0 headless editor import failed" }

    Write-Host "=== G7.0 Semantic Field Contracts + Registry Vocabulary ==="
    ...
    if ($LASTEXITCODE -ne 0) { throw "G7.0 semantic field contracts acceptance failed" }

    Write-Host "G7.0 Semantic Field Contracts focused gate passed."
}
finally {
