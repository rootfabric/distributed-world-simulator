...
try {
    ...
    Write-Host "=== G7.1 editor import / parse ==="
    & $GodotExecutable --headless --editor --path $RootDir --quit
    if ($LASTEXITCODE -ne 0) { throw "G7.1 headless editor import failed" }
    ...
    Write-Host "=== G7.1 G3/G5/G6 Upstream Semantic Field Adapters ==="
    ...
    if ($LASTEXITCODE -ne 0) { throw "G7.1 upstream semantic field adapter acceptance failed" }

    Write-Host "G7.1 Upstream Semantic Field Adapters focused gate passed."
}
finally {
