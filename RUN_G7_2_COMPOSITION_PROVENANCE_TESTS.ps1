...
try {
    ...
    Write-Host "=== G7.2 editor import / parse ==="
    & $GodotExecutable --headless --editor --path $RootDir --quit
    if ($LASTEXITCODE -ne 0) { throw "G7.2 headless editor import failed" }

    Write-Host "=== G7.0 accepted contracts regression ==="
    & $GodotExecutable --headless --path $RootDir --script "res://tests/procedural/semantic_fields/g7_0_semantic_field_contracts_acceptance.gd"
    if ($LASTEXITCODE -ne 0) { throw "G7.0 semantic field contract regression failed" }

    Write-Host "=== G7.1 accepted adapters regression ==="
    & $GodotExecutable --headless --path $RootDir --script "res://tests/procedural/semantic_fields/g7_1_upstream_semantic_field_adapters_acceptance.gd"
    if ($LASTEXITCODE -ne 0) { throw "G7.1 upstream adapter regression failed" }

    Write-Host "=== G7.2 Composition / Provenance ==="
    & $GodotExecutable --headless --path $RootDir --script "res://tests/procedural/semantic_fields/g7_2_composition_provenance_acceptance.gd"
    if ($LASTEXITCODE -ne 0) { throw "G7.2 composition/provenance acceptance failed" }

    Write-Host "G7.2 Composition / Provenance focused gate passed."
}
finally {
