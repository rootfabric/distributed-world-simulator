...
try {
    ...
    Write-Host "=== G8.2 editor import / parse ==="
    & $GodotExecutable --headless --editor --path $RootDir --quit
    if ($LASTEXITCODE -ne 0) { throw "G8.2 headless editor import failed" }
    ...
    Write-Host "=== G8.2 River Channel Incision ==="
    ...
    if ($LASTEXITCODE -ne 0) { throw "G8.2 river channel acceptance failed" }

    Write-Host "G8.2 River Channel Incision focused gate passed."
}
finally {
