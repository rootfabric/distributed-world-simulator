...
try {
    ...
    Write-Host "=== G8.3 editor import / parse ==="
    & $GodotExecutable --headless --editor --path $RootDir --quit
    if ($LASTEXITCODE -ne 0) { throw "G8.3 headless editor import failed" }
    ...
    Write-Host "=== G8.3 Banks and Floodplain Shaping ==="
    & $GodotExecutable --headless --path $RootDir --script "res://tests/procedural/geomorphology/g8_3_banks_floodplain_acceptance.gd"
    if ($LASTEXITCODE -ne 0) { throw "G8.3 banks/floodplain acceptance failed" }

    Write-Host "G8.3 Banks and Floodplain focused gate passed."
}
finally {
