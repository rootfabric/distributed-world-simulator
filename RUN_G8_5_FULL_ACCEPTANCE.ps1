...
$MainRef = "origin/main"
$G84AcceptedMetadataCommit = "d42b8ecb1dcf09f4317c0ef212e46df295c0ebdd"
...
function Test-G85AllowedPath {
    param([Parameter(Mandatory = $true)][string]$Path)
    if ($Path -in @(
        "RUN_G8_5_CROSS_CELL_CROSS_LOD_INVARIANCE_TESTS.ps1",
        "RUN_G8_5_FULL_ACCEPTANCE.ps1",
        "config/control/branches/feature__g8-geomorphology.v1.json",
        "config/procedural/g7-g13-p0-aligned-roadmap.v1.json",
        "config/procedural/g8-5-cross-cell-cross-lod-geomorphology-invariance.v1.json",
        "docs/checkpoints/G8_5_CROSS_CELL_CROSS_LOD_INVARIANCE_CANDIDATE_RU.md",
        "validation/g8-5-cross-cell-cross-lod-geomorphology-invariance-validation.json"
    )) { return $true }
    if ($Path -match '^tests/procedural/geomorphology/g8_5_cross_cell_cross_lod_invariance_acceptance\.gd(\.uid)?$') { return $true }
    return $false
}

if ($null -eq (Get-Command git -ErrorAction SilentlyContinue)) { throw "git is required for G8.5 full acceptance" }
...
Write-Host "=== G8.5 FULL ACCEPTANCE: repository / architecture / PC0 preflight ==="
Remove-SafeWindowsProfileTransient -Phase "stale-preflight"
$StatusBefore = Invoke-GitText @("status", "--porcelain")
if (-not [string]::IsNullOrWhiteSpace($StatusBefore)) { throw "G8.5 full acceptance requires a clean working tree:`n$StatusBefore" }
foreach ($Ref in @($MainRef, $G84AcceptedMetadataCommit)) {
...
Assert-Equal ([string]$ProjectRegistry.programs.G.current_stage) "G8.5 Cross-Cell / Cross-LOD Geomorphology Invariance" "PC0 does not declare G8.5"
...
Assert-Equal ([string]$BranchPassport.current_stage) "G8.5 Cross-Cell / Cross-LOD Geomorphology Invariance" "G8 passport stage mismatch"
Assert-Equal ([string]$BranchPassport.last_accepted_checkpoint) "G8.4 Erosion / Deposition Baseline" "G8.4 is not the accepted G8.5 parent"

& git -C $RootDir merge-base --is-ancestor $G84AcceptedMetadataCommit HEAD
if ($LASTEXITCODE -ne 0) { throw "Accepted G8.4 metadata head is not an ancestor of G8.5" }
$G84Record = Get-Content -LiteralPath (Join-Path $RootDir "validation/g8-4-erosion-deposition-validation.json") -Raw | ConvertFrom-Json
Assert-Equal ([string]$G84Record.decision) "ACCEPTED" "G8.4 validation record is not ACCEPTED"
Assert-Equal ([string]$G84Record.automated_evidence.tested_head) "82cc31429f2bf2ea419bf1b50159838eea51c727" "G8.4 accepted tested head mismatch"
$G85Manifest = Get-Content -LiteralPath (Join-Path $RootDir "config/procedural/g8-5-cross-cell-cross-lod-geomorphology-invariance.v1.json") -Raw | ConvertFrom-Json
Assert-Equal ([string]$G85Manifest.status) "IMPLEMENTED_CANDIDATE" "G8.5 manifest is not an implemented candidate"

Write-Host "=== G8.5 FULL ACCEPTANCE: scope / hygiene ==="
$ChangedFilesText = Invoke-GitText @("diff", "--name-only", "$G84AcceptedMetadataCommit...HEAD")
$ChangedFiles = @($ChangedFilesText -split "`r?`n" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
if ($ChangedFiles.Count -eq 0) { throw "G8.5 diff is empty" }
$Unexpected = @($ChangedFiles | Where-Object { -not (Test-G85AllowedPath $_) })
if ($Unexpected.Count -gt 0) { throw "G8.5 changed files outside invariance/control allowlist:`n$($Unexpected -join "`n")" }
& git -C $RootDir diff --check "$G84AcceptedMetadataCommit...HEAD"
if ($LASTEXITCODE -ne 0) { throw "git diff --check failed for accepted G8.4...G8.5" }
Write-Host "G8.5 changed-file scope: PASS ($($ChangedFiles.Count) files)"

Write-Host "=== G8.5 FULL ACCEPTANCE: Project Control audit ==="
...
Assert-Equal ([string]$GControl.current_stage) "G8.5 Cross-Cell / Cross-LOD Geomorphology Invariance" "PC0 audited G8.5 stage mismatch"
if ([string]$GControl.health -eq "RED") {
    $FindingText = @($GControl.findings | ForEach-Object { "$($_.code): $($_.detail)" }) -join "`n"
    throw "PC0 blocks G8.5 acceptance because G health is RED:`n$FindingText"
}
...
try {
    ...
    Write-Host "=== G8.5 FULL ACCEPTANCE: focused invariance proof ==="
    Invoke-PowerShellChild (Join-Path $RootDir "RUN_G8_5_CROSS_CELL_CROSS_LOD_INVARIANCE_TESTS.ps1") @("-GodotPath", $GodotPath)
    Write-Host "=== G8.5 FULL ACCEPTANCE: world/core regression ==="
...
Write-Host "=== G8.5 FULL ACCEPTANCE: final hygiene ==="
$StatusAfter = Invoke-GitText @("status", "--porcelain")
if (-not [string]::IsNullOrWhiteSpace($StatusAfter)) { throw "G8.5 full acceptance left tracked/untracked changes:`n$StatusAfter" }
& git -C $RootDir diff --check "$G84AcceptedMetadataCommit...HEAD"
if ($LASTEXITCODE -ne 0) { throw "Final G8.5 git diff --check failed" }

Write-Host "G8.5 FULL ACCEPTANCE: PASS"
Write-Host "Architecture revision: $ExpectedGlobalRevision"
Write-Host "Project Control revision: $ExpectedControlRevision"
Write-Host "G8.4 parent ACCEPTED: PASS"
Write-Host "G7.3 semantic cross-cell/cross-LOD parent proof: PASS"
Write-Host "G8.5 geomorphology cross-cell/cross-LOD invariance: PASS"
Write-Host "SurfaceCellKey / cube face / LOD excluded from canonical deformation identity: PASS"
Write-Host "World/core regression: PASS"
Write-Host "Working tree: CLEAN"
Write-Host "NEXT: G8.6 GEOMORPHOLOGY VISUAL LAB"
