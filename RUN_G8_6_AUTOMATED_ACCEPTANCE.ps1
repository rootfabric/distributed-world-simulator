...
$MainRef = "origin/main"
$G85AcceptedBoundaryCommit = "7d32e508c35ee0b29dc2f1fdb1a5c4ab12c9a699"
...
if ($null -eq (Get-Command git -ErrorAction SilentlyContinue)) { throw "git is required for G8.6 automated acceptance" }
...
Write-Host "=== G8.6 AUTOMATED ACCEPTANCE: repository / architecture / PC0 preflight ==="
Remove-SafeWindowsProfileTransient -Phase "stale-preflight"
$StatusBefore = Invoke-GitText @("status", "--porcelain")
if (-not [string]::IsNullOrWhiteSpace($StatusBefore)) { throw "G8.6 automated acceptance requires a clean working tree:`n$StatusBefore" }
foreach ($Ref in @($MainRef, $G85AcceptedBoundaryCommit)) {
...
Assert-Equal ([string]$ProjectRegistry.programs.G.current_stage) "G8.6 Geomorphology Visual Lab" "PC0 does not declare G8.6"
...
Assert-Equal ([string]$BranchPassport.current_stage) "G8.6 Geomorphology Visual Lab" "G8 passport stage mismatch"
Assert-Equal ([string]$BranchPassport.last_accepted_checkpoint) "G8.5 Cross-Cell / Cross-LOD Geomorphology Invariance" "G8.5 is not the accepted G8.6 parent"

& git -C $RootDir merge-base --is-ancestor $G85AcceptedBoundaryCommit HEAD
if ($LASTEXITCODE -ne 0) { throw "Accepted G8.5/G8.6 declaration boundary is not an ancestor of G8.6" }
$G85Record = Get-Content -LiteralPath (Join-Path $RootDir "validation/g8-5-cross-cell-cross-lod-geomorphology-invariance-validation.json") -Raw | ConvertFrom-Json
Assert-Equal ([string]$G85Record.decision) "ACCEPTED" "G8.5 validation record is not ACCEPTED"
Assert-Equal ([string]$G85Record.automated_evidence.tested_head) "6cc0c2b5ff1bc21a5b488a8492ef8cce28fa4736" "G8.5 accepted tested head mismatch"
...
Assert-Equal ([string]$G86Manifest.status) "IMPLEMENTED_CANDIDATE" "G8.6 manifest is not an implemented candidate"

Write-Host "=== G8.6 AUTOMATED ACCEPTANCE: scope / hygiene ==="
$ChangedFilesText = Invoke-GitText @("diff", "--name-only", "$G85AcceptedBoundaryCommit...HEAD")
$ChangedFiles = @($ChangedFilesText -split "`r?`n" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
if ($ChangedFiles.Count -eq 0) { throw "G8.6 diff is empty" }
$Unexpected = @($ChangedFiles | Where-Object { -not (Test-G86AllowedPath $_) })
if ($Unexpected.Count -gt 0) { throw "G8.6 changed files outside visual-lab/control allowlist:`n$($Unexpected -join "`n")" }
& git -C $RootDir diff --check "$G85AcceptedBoundaryCommit...HEAD"
if ($LASTEXITCODE -ne 0) { throw "git diff --check failed for G8.6 visual lab" }
Write-Host "G8.6 changed-file scope: PASS ($($ChangedFiles.Count) files)"

Write-Host "=== G8.6 AUTOMATED ACCEPTANCE: Project Control audit ==="
...
Assert-Equal ([string]$GControl.current_stage) "G8.6 Geomorphology Visual Lab" "PC0 audited G8.6 stage mismatch"
if ([string]$GControl.health -eq "RED") {
    $FindingText = @($GControl.findings | ForEach-Object { "$($_.code): $($_.detail)" }) -join "`n"
    throw "PC0 blocks G8.6 automated acceptance because G health is RED:`n$FindingText"
}
...
try {
    ...
    Write-Host "=== G8.6 AUTOMATED ACCEPTANCE: focused/headless visual lab ==="
    Invoke-PowerShellChild (Join-Path $RootDir "RUN_G8_6_GEOMORPHOLOGY_VISUAL_LAB_TESTS.ps1") @("-GodotPath", $GodotPath)
    Write-Host "=== G8.6 AUTOMATED ACCEPTANCE: world/core regression ==="
...
Write-Host "=== G8.6 AUTOMATED ACCEPTANCE: final hygiene ==="
$StatusAfter = Invoke-GitText @("status", "--porcelain")
if (-not [string]::IsNullOrWhiteSpace($StatusAfter)) { throw "G8.6 automated acceptance left tracked/untracked changes:`n$StatusAfter" }
& git -C $RootDir diff --check "$G85AcceptedBoundaryCommit...HEAD"
if ($LASTEXITCODE -ne 0) { throw "Final G8.6 git diff --check failed" }

Write-Host "G8.6 AUTOMATED ACCEPTANCE: PASS"
Write-Host "Architecture revision: $ExpectedGlobalRevision"
Write-Host "Project Control revision: $ExpectedControlRevision"
Write-Host "G8.5 parent ACCEPTED: PASS"
Write-Host "G8.6 headless local corridor lab: PASS"
Write-Host "Derived presentation LOD / truth separation: PASS"
Write-Host "PX/PZ seam diagnostic corridor: PASS"
Write-Host "World/core regression: PASS"
Write-Host "Working tree: CLEAN"
Write-Host "MANUAL GRAPHICAL ACCEPTANCE: REQUIRED"
Write-Host "NEXT AFTER MANUAL PASS: G8 FULL ACCEPTANCE"
