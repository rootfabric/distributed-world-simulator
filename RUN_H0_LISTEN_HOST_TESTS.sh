...
run_checked() {
  local name="$1"; shift
  local log="$REPORT_ROOT/h0-${name}.log"
  set +e
...
run_checked contracts --headless --path "$PROJECT_ROOT" --script res://tests/runtime/test_h0_listen_host_contracts.gd
run_checked processes --headless --path "$PROJECT_ROOT" --script res://tests/runtime/test_h0_listen_host_processes.gd
echo "H0 listen-host tests: PASS"
