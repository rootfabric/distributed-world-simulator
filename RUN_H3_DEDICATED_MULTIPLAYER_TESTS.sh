...
REPORT_PATH="$REPORT_ROOT/h3-dedicated-multiplayer-runner-summary.json"
mkdir -p "$REPORT_ROOT"
...
run_checked() {
  ...
  local log="$REPORT_ROOT/h3-${name}.log"
...
PY
  ...
  python3 - "$STEPS_FILE" "$name" "$target" "$exit_code" "$duration" <<'PY'
import json,sys
path,name,target,code,duration=sys.argv[1:]
with open(path,'a',encoding='utf-8') as f:
    f.write(json.dumps({
        'name':name,'target':target,'exit_code':int(code),
        'duration_seconds':float(duration),'passed':int(code)==0
    },ensure_ascii=False)+'\n')
PY
...
try:
    ...
    pass
report={
  'schema':'planet_simulator.h3_dedicated_multiplayer_runner_summary.v1',
...
for test_script in "${TESTS[@]}"; do
  ...
  run_checked "$test_name" "$test_script" --headless --path "$PROJECT_ROOT" --script "$test_script"
done
write_summary true

echo "H3 dedicated multiplayer profile: PASS"
