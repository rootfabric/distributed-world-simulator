...
$ReportPath = Join-Path $ReportDirectory "m0-aggregate-transaction-summary.json"
...
$Summary = [ordered]@{
    schema = "planet_simulator.m0_aggregate_transaction_runner_summary.v1"
    checkpoint = "v16.8.5-domain-m0-aggregate-transactions"
    build_id = "m0-multi-aggregate-transactions-outbox"
...
try {
    ...
    Write-Host "Checkpoint: v16.8.5-domain-m0-aggregate-transactions"
    ...
    $Summary.passed = $true
    Save-Summary
    Write-Host "M0 aggregate transaction tests passed." -ForegroundColor Green
