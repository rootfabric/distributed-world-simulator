...
REPORT_PATH="$REPORT_ROOT/m1-unified-networked-gameplay-summary.json"
TESTS=(
...
run() {
  ...
  set +e; "$GODOT_BIN" "$@" 2>&1 | tee "$REPORT_ROOT/m1-${name}.log"; local code=${PIPESTATUS[0]}; set -e
...
obj={'schema':'planet_simulator.m1_unified_networked_gameplay_summary.v1','checkpoint':'v16.10.0-runtime-m1-unified-networked-gameplay-core','build_id':'m1-unified-networked-gameplay-core','decision':'UNIFIED_NETWORKED_GAMEPLAY_CORE','finished_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'godot':godot,'project_root':root,'declared_test_count':int(count),'passed':passed=='true','steps':json.loads(steps)}
...
write_summary true
echo "M1 unified networked gameplay: PASS"
