...
$ProfileRoot = Join-Path $Root "artifacts/test-results/pre-m5-profile-$PID"
New-Item -ItemType Directory -Force -Path $ProfileRoot | Out-Null
$OriginalEnvironment = @{}
foreach ($Name in @("HOME","APPDATA","LOCALAPPDATA","XDG_DATA_HOME","XDG_CONFIG_HOME","XDG_CACHE_HOME","BREAKPOINT_RUNTIME_DISABLED","BREAKPOINT_RUNTIME_PORT")) {
...
function Invoke-GodotSafe {
    ...
    if ($ExitCode -ne 0 -or $Text -match "(: FAIL|SCRIPT ERROR:|Parse Error:|Compile Error:)") {
        throw "Pre-M5 preparation step failed: $Name"
    }
}
...
Write-Host ""
Write-Host "Pre-M5 graphical acceptance preparation: PASS ($($Tests.Count)/$($Tests.Count))" -ForegroundColor Green
