...
$ProfileRoot = Join-Path $Root "artifacts/test-results/m6-focused-profile-$PID"
...
function Invoke-GodotSafe {
    ...
    $Text = $Output -join "`n"
    # PowerShell -match is case-insensitive and would classify a successful
    # assertion such as "PASS: Failed durable restore..." as a failure.
    if ($ExitCode -ne 0 -or $Text -cmatch "(: FAIL|SCRIPT ERROR:|Parse Error:|Compile Error:)") {
        throw "M6 dedicated recovery step failed: $Name"
    }
}
...
Write-Host ""
Write-Host "M6 dedicated persistence and recovery: PASS ($($Tests.Count)/$($Tests.Count))" -ForegroundColor Green
