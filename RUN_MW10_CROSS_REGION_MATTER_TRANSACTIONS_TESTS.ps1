...
$Suites = @(
    [pscustomobject]@{
        ...
        Script = "res://tests/matter/transactions/test_mw10_cross_region_transactions.gd"
        Marker = "MW10 cross-region Matter transactions: PASS (184 assertions)"
    },
    [pscustomobject]@{
        Name = "multi-process"
        Script = "res://tests/matter/transactions/test_mw10_cross_region_processes.gd"
        Marker = "MW10 cross-region Matter processes: PASS (51 assertions)"
    }
)
...
function Invoke-Mw10Suite {
    ...
    try {
        ...
        Write-Host "MW10 runner: suite $SuiteIndex/$SuiteCount [$($Suite.Name)]"
        Write-Host "MW10 runner: $($Suite.Script)"
        ...
        if (-not $Completed) {
            [Console]::Error.WriteLine(
                "MW10 runner: suite $($Suite.Name) exceeded ${TimeoutSeconds}s and was terminated."
            )
        }
        elseif (-not $ExitCodeAvailable) {
            [Console]::Error.WriteLine(
                "MW10 runner: suite $($Suite.Name) harness failure: process exit code is unavailable after WaitForExit and Refresh."
            )
        }
        elseif ([int]$ExitCode -ne 0) {
            [Console]::Error.WriteLine(
                "MW10 runner: suite $($Suite.Name) exited with code $ExitCode."
            )
        }
        elseif (-not $MarkerFound) {
            [Console]::Error.WriteLine(
                "MW10 runner: suite $($Suite.Name) did not print the required marker: $($Suite.Marker)"
            )
        }
        ...
        [Console]::Error.WriteLine(
            "MW10 runner: suite $($Suite.Name) harness failure: $Message"
        )
        return [pscustomobject]@{
            Name = $Suite.Name
            Script = $Suite.Script
...
if ($Failures.Count -gt 0) {
    [Console]::Error.WriteLine(
        "MW10 runner: FAIL ($($Failures.Count)/$($Suites.Count) suites failed)."
    )
    foreach ($Failure in $Failures) {
        ...
        [Console]::Error.WriteLine(
            "MW10 runner: failed suite [$($Failure.Name)] script=$($Failure.Script) exit=$ExitDescription marker=$($Failure.MarkerFound)"
        )
    }
...
Write-Host "MW10 runner: PASS ($($Suites.Count)/$($Suites.Count) suites)"
