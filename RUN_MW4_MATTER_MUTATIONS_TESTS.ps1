...
Write-Host "MW4 runner: starting with ${TimeoutSeconds}s timeout"
...
while (-not $Process.HasExited) {
    if ([DateTime]::UtcNow -ge $Deadline) {
        ...
        [Console]::Error.WriteLine("MW4 focused test exceeded ${TimeoutSeconds}s and was terminated. Last printed stage identifies the hot path.")
        exit 124
    }
