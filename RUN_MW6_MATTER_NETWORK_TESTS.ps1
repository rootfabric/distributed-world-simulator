...
$Arguments = @(
    "--headless",
    "--path", ('"{0}"' -f $RootDir),
    "--script", "res://tests/matter/network/test_mw6_matter_network_replication.gd"
)

Write-Host "MW6 runner: starting with ${TimeoutSeconds}s timeout"
...
while (-not $Process.HasExited) {
    if ([DateTime]::UtcNow -ge $Deadline) {
        ...
        [Console]::Error.WriteLine("MW6 focused test exceeded ${TimeoutSeconds}s and was terminated. Last printed stage identifies the hot path.")
        exit 124
    }
