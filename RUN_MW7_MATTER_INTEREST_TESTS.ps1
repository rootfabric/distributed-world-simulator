...
$Arguments = @(
    "--headless",
    "--path", ('"{0}"' -f $RootDir),
    "--script", "res://tests/matter/interest/test_mw7_matter_interest_replication.gd"
)

Write-Host "MW7 runner: starting with ${TimeoutSeconds}s timeout"
...
while (-not $Process.HasExited) {
    if ([DateTime]::UtcNow -ge $Deadline) {
        ...
        [Console]::Error.WriteLine("MW7 focused test exceeded ${TimeoutSeconds}s and was terminated. Last printed stage identifies the hot path.")
        exit 124
    }
