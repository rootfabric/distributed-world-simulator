...
$Arguments = @(
    "--headless",
    "--path", ('"{0}"' -f $RootDir),
    "--script", "res://tests/matter/handoff/test_mw8_regional_authority_handoff.gd"
)

Write-Host "MW8 runner: starting with ${TimeoutSeconds}s timeout"
...
while (-not $Process.HasExited) {
    if ([DateTime]::UtcNow -ge $Deadline) {
        ...
        [Console]::Error.WriteLine("MW8 focused test exceeded ${TimeoutSeconds}s and was terminated. Last printed stage identifies the hot path.")
        exit 124
    }
