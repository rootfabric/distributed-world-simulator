...
$ProfileRoot = Join-Path $Root "artifacts/test-results/nx0-preparation-$PID"
...
function Invoke-GodotSafe {
    ...
    if ($ExitCode -ne 0 -or $Text -cmatch "(: FAIL|SCRIPT ERROR:|Parse Error:|Compile Error:)") {
        throw "NX0 preparation step failed: $Name"
    }
}

try {
    Invoke-GodotSafe -Name "editor-import" -Arguments @("--headless","--editor","--path",$Root,"--quit")
    Invoke-GodotSafe -Name "contracts" -Arguments @("--headless","--path",$Root,"--script","res://tests/network/test_nx0_network_experience_preparation.gd")
}
finally {
...
Write-Host ""
Write-Host "NX0 network experience preparation: PASS (2/2) [$Checkpoint]" -ForegroundColor Green
