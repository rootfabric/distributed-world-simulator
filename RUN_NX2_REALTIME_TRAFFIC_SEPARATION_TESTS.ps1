...
$ProfileRoot = Join-Path $Root "artifacts/test-results/nx2-realtime-traffic-$PID"
...
function Invoke-GodotSafe {
    ...
    $Text=$Output -join "`n"
    if($ExitCode -ne 0 -or $Text -cmatch "(: FAIL|SCRIPT ERROR:|Parse Error:|Compile Error:)"){throw "NX2 realtime traffic step failed: $Name"}
}
try {
    ...
    Invoke-GodotSafe "nx2-realtime-processes" @("--headless","--path",$Root,"--script","res://tests/runtime/test_m7_playable_networked_processes.gd")
} finally {
...
Write-Host "`nNX2 realtime traffic separation: PASS (9/9) [$Checkpoint]" -ForegroundColor Green
