...
$ProfileRoot = Join-Path $Root "artifacts/test-results/nx4-prediction-$PID"
New-Item -ItemType Directory -Force -Path $ProfileRoot | Out-Null
$OriginalEnvironment = @{}
foreach ($Name in @("APPDATA","LOCALAPPDATA","XDG_DATA_HOME","XDG_CONFIG_HOME","XDG_CACHE_HOME","BREAKPOINT_RUNTIME_DISABLED","BREAKPOINT_RUNTIME_PORT","GODOT_SILENCE_ROOT_WARNING","TERM","NX4_TEST_CLIENT_NETWORK_PROFILE")) {
...
function Invoke-GodotSafe {
    ...
    $Text=$Output -join "`n"
    if($ExitCode -ne 0 -or $Text -cmatch "(: FAIL|SCRIPT ERROR:|Parse Error:|Compile Error:)"){throw "NX4 prediction step failed: $Name"}
}
try {
    ...
    Invoke-GodotSafe "nx4-contracts" @("--headless","--path",$Root,"--script","res://tests/network/test_nx4_client_prediction_reconciliation.gd")
    ...
    Invoke-GodotSafe "nx4-playable-processes" @("--headless","--path",$Root,"--script","res://tests/runtime/test_m7_playable_networked_processes.gd")
    $env:NX4_TEST_CLIENT_NETWORK_PROFILE="AVERAGE_BROADBAND"
    Invoke-GodotSafe "nx4-conditioned-playable-processes" @("--headless","--path",$Root,"--script","res://tests/runtime/test_m7_playable_networked_processes.gd")
    Remove-Item Env:NX4_TEST_CLIENT_NETWORK_PROFILE -ErrorAction SilentlyContinue
    Invoke-GodotSafe "nx4-recovery-processes" @("--headless","--path",$Root,"--script","res://tests/runtime/test_m7_playable_networked_recovery_processes.gd")
} finally {
...
Write-Host "`nNX4 client prediction and reconciliation: PASS (13/13) [$Checkpoint]" -ForegroundColor Green
