...
NX5_HAD_XDG_DATA_HOME=${XDG_DATA_HOME+x}
NX5_HAD_XDG_CONFIG_HOME=${XDG_CONFIG_HOME+x}
NX5_HAD_XDG_CACHE_HOME=${XDG_CACHE_HOME+x}
NX5_HAD_GODOT_SILENCE_ROOT_WARNING=${GODOT_SILENCE_ROOT_WARNING+x}
NX5_OLD_XDG_DATA_HOME=${XDG_DATA_HOME-}
NX5_OLD_XDG_CONFIG_HOME=${XDG_CONFIG_HOME-}
NX5_OLD_XDG_CACHE_HOME=${XDG_CACHE_HOME-}
NX5_OLD_GODOT_SILENCE_ROOT_WARNING=${GODOT_SILENCE_ROOT_WARNING-}

restore_environment() {
  if [[ -n "$NX5_HAD_XDG_DATA_HOME" ]]; then
    export XDG_DATA_HOME="$NX5_OLD_XDG_DATA_HOME"
  else
    unset XDG_DATA_HOME
  fi
  if [[ -n "$NX5_HAD_XDG_CONFIG_HOME" ]]; then
    export XDG_CONFIG_HOME="$NX5_OLD_XDG_CONFIG_HOME"
  else
    unset XDG_CONFIG_HOME
  fi
  if [[ -n "$NX5_HAD_XDG_CACHE_HOME" ]]; then
    export XDG_CACHE_HOME="$NX5_OLD_XDG_CACHE_HOME"
  else
    unset XDG_CACHE_HOME
  fi
  if [[ -n "$NX5_HAD_GODOT_SILENCE_ROOT_WARNING" ]]; then
    export GODOT_SILENCE_ROOT_WARNING="$NX5_OLD_GODOT_SILENCE_ROOT_WARNING"
  else
    unset GODOT_SILENCE_ROOT_WARNING
  fi
}
