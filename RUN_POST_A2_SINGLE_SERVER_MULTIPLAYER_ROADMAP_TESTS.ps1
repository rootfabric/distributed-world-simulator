...
function Run-Step([string]$Name,[string]$Target,[string[]]$Args){$Started=[DateTime]::UtcNow;& $Godot @Args 2>&1|Tee-Object -FilePath (Join-Path $ReportRoot "post-a2-$Name.log");$Code=$LASTEXITCODE;$Summary.steps += [ordered]@{name=$Name;target=$Target;exit_code=$Code;duration_seconds=[Math]::Round(([DateTime]::UtcNow-$Started).TotalSeconds,3);passed=($Code-eq 0)};if($Code-ne 0){Save-Summary;throw "$Name failed"}}
