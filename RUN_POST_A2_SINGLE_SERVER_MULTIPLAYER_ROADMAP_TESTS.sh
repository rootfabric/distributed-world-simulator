...
obj={'schema':'planet_simulator.post_a2_single_server_multiplayer_roadmap_summary.v1','checkpoint':'v16.9.5-roadmap-single-server-multiplayer-first','build_id':'post-a2-single-server-multiplayer-first','decision':'FULL_SINGLE_SERVER_MULTIPLAYER_FIRST','finished_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'godot':godot,'project_root':root,'declared_test_count':2,'passed':passed=='true','steps':json.loads(steps)}
