...
$Arguments = @(
    "--headless",
    "--path", ('"{0}"' -f $RootDir),
    "--script", "res://tests/representation/test_rl0_representation_contracts.gd"
)

Write-Host "RL0 runner: starting with ${TimeoutSeconds}s timeout"
...
while (-not $Process.HasExited) {
    if ([DateTime]::UtcNow -ge $Deadline) {
        ...
        [Console]::Error.WriteLine("RL0 focused test exceeded ${TimeoutSeconds}s and was terminated.")
        exit 124
    }
