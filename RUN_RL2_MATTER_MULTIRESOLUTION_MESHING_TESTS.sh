...
TIMEOUT_SECONDS="${RL2_TIMEOUT_SECONDS:-300}"
...
names=("contracts/synthetic" "real-asteroid")
scripts=(
...
for index in "${!scripts[@]}"; do
  output="$(mktemp)"
  outputs+=("$output")
  echo "RL2 runner: starting suite $((index + 1))/${#scripts[@]} [${names[$index]}]"
...
for index in "${!pids[@]}"; do
  ...
  if [[ $exit_code -eq 124 || $exit_code -eq 137 ]]; then
    echo "RL2 runner: suite [${names[$index]}] exceeded ${TIMEOUT_SECONDS}s." >&2
    failures=$((failures + 1))
    timed_out=1
  elif [[ $script_error_found -eq 1 ]]; then
    echo "RL2 runner: suite [${names[$index]}] printed a Godot script error." >&2
    failures=$((failures + 1))
  elif [[ $exit_code -ne 0 ]]; then
    echo "RL2 runner: suite [${names[$index]}] exited with code $exit_code." >&2
    failures=$((failures + 1))
  elif [[ $marker_found -ne 1 ]]; then
    echo "RL2 runner: suite [${names[$index]}] did not print required marker: ${markers[$index]}" >&2
    failures=$((failures + 1))
  else
    echo "RL2 runner: suite [${names[$index]}] PASS"
  fi
done

if [[ $failures -ne 0 ]]; then
  echo "RL2 runner: FAIL ($failures/${#scripts[@]} suites failed)." >&2
...
echo "RL2 runner: PASS (${#scripts[@]}/${#scripts[@]} suites)"
