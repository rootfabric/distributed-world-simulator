...
$Suites = @(
    [pscustomobject]@{
        ...
        Script = "res://tests/representation/test_rl3_representation_aware_network_streaming.gd"
        Marker = "RL3 representation-aware network streaming: PASS (175 assertions)"
    },
    [pscustomobject]@{
        Name = "multi-process"
        Script = "res://tests/representation/test_rl3_representation_streaming_processes.gd"
        Marker = "RL3 representation streaming processes: PASS (37 assertions)"
    }
)
...
function Start-Rl3Suite {
    ...
    try {
        ...
        Write-Host "RL3 runner: starting suite $SuiteIndex/$SuiteCount [$($Suite.Name)]"
...
function Complete-Rl3Suite {
    param([Parameter(Mandatory = $true)][pscustomobject]$Context)
    $Suite = $Context.Suite
    $Process = $Context.Process
    if ($null -eq $Process) {
        [Console]::Error.WriteLine("RL3 runner: suite $($Suite.Name) harness failure: $($Context.StartError)")
    ...
    try {
        ...
        if ($TimedOut) {
            [Console]::Error.WriteLine("RL3 runner: suite $($Suite.Name) exceeded ${TimeoutSeconds}s and was terminated.")
        }
        elseif (-not $ExitCodeAvailable) {
            [Console]::Error.WriteLine("RL3 runner: suite $($Suite.Name) harness failure: process exit code is unavailable after WaitForExit and Refresh.")
        }
        elseif ($ScriptErrorFound) {
            [Console]::Error.WriteLine("RL3 runner: suite $($Suite.Name) printed a Godot script error.")
        }
        elseif ([int]$ExitCode -ne 0) {
            [Console]::Error.WriteLine("RL3 runner: suite $($Suite.Name) exited with code $ExitCode.")
        }
        elseif (-not $MarkerFound) {
            [Console]::Error.WriteLine("RL3 runner: suite $($Suite.Name) did not print required marker: $($Suite.Marker)")
        }
        else {
            Write-Host "RL3 runner: suite [$($Suite.Name)] PASS"
        }
        ...
        [Console]::Error.WriteLine("RL3 runner: suite $($Suite.Name) harness failure: $($_.Exception.Message)")
...
for ($Index = 0; $Index -lt $Suites.Count; $Index += 1) {
    $Contexts += Start-Rl3Suite -Suite $Suites[$Index] -SuiteIndex ($Index + 1) -SuiteCount $Suites.Count
}
$Results = @()
foreach ($Context in $Contexts) {
    $Results += Complete-Rl3Suite -Context $Context
}

$Failures = @($Results | Where-Object { -not $_.Success })
if ($Failures.Count -gt 0) {
    [Console]::Error.WriteLine("RL3 runner: FAIL ($($Failures.Count)/$($Suites.Count) suites failed).")
...
Write-Host "RL3 runner: PASS ($($Suites.Count)/$($Suites.Count) suites)"
