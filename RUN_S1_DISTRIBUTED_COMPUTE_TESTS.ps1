...
$ReportPath = Join-Path $ReportDirectory "s1-distributed-compute-summary.json"
...
$Summary = [ordered]@{
    schema = "planet_simulator.s1_distributed_compute_runner_summary.v1"
...
try {
    ...
    $Summary.passed = $true
    Save-Summary
    Write-Host "S1 distributed compute tests passed." -ForegroundColor Green
