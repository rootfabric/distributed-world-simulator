...
run_checked() {
  local name="$1"; shift
  local log="$REPORT_ROOT/t1-${name}.log"
  set +e
...
run_checked contracts --headless --path "$PROJECT_ROOT" --script res://tests/network/test_t1_multi_peer_transport_contracts.gd
run_checked integration --headless --path "$PROJECT_ROOT" --script res://tests/network/test_t1_multi_peer_transport_processes.gd
echo "T1 multi-peer transport tests: PASS"
