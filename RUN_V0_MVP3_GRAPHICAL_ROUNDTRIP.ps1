...
$Output = Join-Path $Root "artifacts\mvp3-graphical-process"
if (Test-Path -LiteralPath $Output) { throw "Preserve or move previous evidence first: $Output" }
$PreviousUtf8 = $env:PYTHONUTF8
try {
    $env:PYTHONUTF8 = "1"
    & $Python (Join-Path $Root "tests\integration\test_v0_mvp3_graphical_process_roundtrip.py") --engine $GodotBin --output $Output
