...
$ArtifactRoot = Join-Path $ProjectRoot "artifacts\runtime\v0-p4-live-mvp-composition"
...
if (git -C $ProjectRoot status --porcelain) {
    throw "V0-P4 live MVP composition gate requires a clean tracked checkout."
}
...
function Assert-ProjectStable {
    ...
    if ($After -ne $ProjectHashBefore) {
        ...
        throw "V0-P4 live MVP composition gate mutated project.godot during $Stage."
    }
}
...
function Invoke-GodotContract {
    ...
    Write-Host "[V0-P4 live MVP] PASS: $Name" -ForegroundColor Green
}

Write-Host "[V0-P4 live MVP] Project: $ProjectRoot"
Write-Host "[V0-P4 live MVP] HEAD:    $ActualHead"
Write-Host "[V0-P4 live MVP] Godot:   $GodotExe"
...
Invoke-GodotContract -Name "c12-construction-multiplayer" -Script "res://tests/construction/test_c12_multiplayer_construction_integration.gd" -Summary "C12 multiplayer Construction integration: PASS (71 assertions)"
...
if (git -C $ProjectRoot status --porcelain) {
    ...
    throw "V0-P4 live MVP composition gate changed tracked checkout state."
}

Write-Host ""
Write-Host "V0-P4 live MVP composition ordering GREEN on exact HEAD $ActualHead" -ForegroundColor Green
