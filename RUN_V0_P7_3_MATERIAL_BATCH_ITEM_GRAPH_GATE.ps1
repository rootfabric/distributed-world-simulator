...
$ArtifactRoot = Join-Path $Root "artifacts\runtime\v0-p7-3-material-batch-item-graph"
...
if (-not [string]::IsNullOrWhiteSpace($ExpectedHead) -and $Head -ne $ExpectedHead.Trim().ToLowerInvariant()) {
    throw "P7_3_HEAD_MISMATCH actual=$Head expected=$ExpectedHead"
}
if (git -C $Root status --porcelain --untracked-files=no) { throw "P7.3 gate requires clean tracked checkout." }
...
function Invoke-Contract {
    ...
    Write-Host "[V0-P7.3] PASS $Name" -ForegroundColor Green
}

$ImportLog = Join-Path $ArtifactRoot "import.log"
& $GodotExe --headless --editor --path $Root --log-file $ImportLog --import
if ($LASTEXITCODE -ne 0) { Get-Content $ImportLog -Tail 500; throw "P7.3 import failed" }
...
if (git -C $Root status --porcelain --untracked-files=no) { git -C $Root status --short; throw "P7.3 gate changed tracked checkout." }
Write-Host "V0-P7.3 MATERIAL BATCH TO ITEM GRAPH GATE GREEN" -ForegroundColor Green
