...
ARTIFACT_ROOT="$ROOT/artifacts/runtime/v0-p7-3-material-batch-item-graph"
...
[[ -z "$EXPECTED_HEAD" || "$HEAD" == "$EXPECTED_HEAD" ]] || {
  echo "P7_3_HEAD_MISMATCH actual=$HEAD expected=$EXPECTED_HEAD" >&2
  exit 4
}
[[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=no)" ]] || {
  echo "P7.3 gate requires clean tracked checkout." >&2
...
[[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=no)" ]] || {
  git -C "$ROOT" status --short
  echo "P7.3 gate changed tracked checkout." >&2
  exit 6
}

echo "V0-P7.3 MATERIAL BATCH TO ITEM GRAPH GATE GREEN"
