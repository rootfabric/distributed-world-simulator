...
function Stop-Sm0Session {
    ...
    foreach ($Record in @($State.processes)) {
        $ManagedProcessId = [int]$Record.pid
        if (Test-ProcessAlive $ManagedProcessId) {
            Write-Host "[SM0] Stopping $($Record.role) (PID $ManagedProcessId)..."
...
function Show-Sm0LogTail {
    ...
    Write-Host ""
    Write-Host "[SM0] ---- $Label tail: $Path ----" -ForegroundColor Yellow
    if (Test-Path -LiteralPath $Path -PathType Leaf) {
        Get-Content -LiteralPath $Path -Tail $Lines -ErrorAction SilentlyContinue | ForEach-Object { Write-Host $_ }
    }
    else {
        Write-Host "[SM0] log file does not exist"
    }
    Write-Host "[SM0] ---- end $Label tail ----"
}
...
function Invoke-GodotWithBreakpointDisabled {
...
function Assert-LogHasNoFatalEngineError {
    param([string]$Path, [string]$Phase)
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { return }
    foreach ($Pattern in @("SCRIPT ERROR", "Parse Error", "Failed to load script", "SM0 server setup failed")) {
        if (Select-String -LiteralPath $Path -SimpleMatch $Pattern -Quiet -ErrorAction SilentlyContinue) {
            Show-Sm0LogTail -Label $Phase -Path $Path
            throw "$Phase failed; fatal pattern '$Pattern' found in $Path"
        }
    }
}
...
function Wait-LogMarker {
    ...
    while ((Get-Date) -lt $Deadline) {
        $Process.Refresh()
        Assert-LogHasNoFatalEngineError -Path $Path -Phase $Label
        if ($Process.HasExited) {
            Show-Sm0LogTail -Label $Label -Path $Path
            throw "$Label process PID=$($Process.Id) exited with code $($Process.ExitCode) before marker '$Marker'. Log: $Path"
        }
        if (Test-Path -LiteralPath $Path -PathType Leaf) {
    ...
    Show-Sm0LogTail -Label $Label -Path $Path
    throw "Timeout waiting for '$Marker' from $Label. Log: $Path"
}

Write-Harness "SM0 acceptance R1 start"
...
try {
    Write-Harness "Running cold-checkout Godot metadata import..."
    Invoke-GodotWithBreakpointDisabled {
        & $GodotExe `
            --headless `
            --editor `
            --path $ProjectRoot `
            --log-file $ImportLog `
            --import
        $script:ImportExit = $LASTEXITCODE
    }
    if ($ImportExit -ne 0) {
        Show-Sm0LogTail -Label "import" -Path $ImportLog
        throw "SM0 cold import failed with exit code $ImportExit. Log: $ImportLog"
    }
    Assert-LogHasNoFatalEngineError -Path $ImportLog -Phase "import"
    Write-Harness "Godot metadata import complete."
    ...
    if ($ContractExit -ne 0) {
        Show-Sm0LogTail -Label "contracts" -Path $ContractLog
        throw "SM0 contract tests failed with exit code $ContractExit. Log: $ContractLog"
    }
    Assert-LogHasNoFatalEngineError -Path $ContractLog -Phase "contracts"

    foreach ($Port in @($GameplayPorts + $ControlPorts + @($ClientPort))) {
        if (-not (Test-UdpPortAvailable -Port $Port)) {
            throw "SM0 required UDP port $Port is already in use. Stop the owning process and rerun with -Restart."
        }
    }
    Write-Harness "Required UDP ports are available."
    ...
    Wait-LogMarker -Path $ServerALog -Marker '"event":"SM0_SERVER_READY"' -Process $ServerA -Seconds 20 -Label "server-a"
    Wait-LogMarker -Path $ServerBLog -Marker '"event":"SM0_SERVER_READY"' -Process $ServerB -Seconds 20 -Label "server-b"
    Wait-LogMarker -Path $ServerALog -Marker '"event":"SM0_AUTHORITY_PEER_SYNCED"' -Process $ServerA -Seconds 20 -Label "server-a"
    Wait-LogMarker -Path $ServerBLog -Marker '"event":"SM0_AUTHORITY_PEER_SYNCED"' -Process $ServerB -Seconds 20 -Label "server-b"
    Wait-LogMarker -Path $ServerALog -Marker '"event":"SM0_DIRECTORY_READY"' -Process $ServerA -Seconds 20 -Label "server-a"
    Wait-LogMarker -Path $ServerBLog -Marker '"event":"SM0_DIRECTORY_READY"' -Process $ServerB -Seconds 20 -Label "server-b"
    ...
    while (-not $Client.HasExited -and (Get-Date) -lt $Deadline) {
        ...
        $ServerA.Refresh()
        $ServerB.Refresh()
        Assert-LogHasNoFatalEngineError -Path $ServerALog -Phase "server-a"
        Assert-LogHasNoFatalEngineError -Path $ServerBLog -Phase "server-b"
        Assert-LogHasNoFatalEngineError -Path $ClientLog -Phase "client"
    ...
    if (-not $Client.HasExited) {
        Show-Sm0LogTail -Label "client" -Path $ClientLog
    ...
    if ($Client.ExitCode -ne 0) {
        Show-Sm0LogTail -Label "client" -Path $ClientLog
        throw "SM0 client reported failure. See $ClientLog"
    }
    ...
    if (-not $AllowDirty -and (($GitStatusAfter -join "`n") -ne ($GitStatusBefore -join "`n"))) {
    ...
    Show-Sm0LogTail -Label "server-a" -Path $ServerALog
    Show-Sm0LogTail -Label "server-b" -Path $ServerBLog
