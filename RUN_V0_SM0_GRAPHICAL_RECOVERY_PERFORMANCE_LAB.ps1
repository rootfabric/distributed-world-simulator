...
if (-not (Test-Path -LiteralPath $SourceRunner -PathType Leaf)) {
    throw "SM0-P2.1 source graphical runner not found: $SourceRunner"
}
...
$CompileReplacement = $CompileNeedle + "`n" + '    "res://scripts/runtime/seamless/sm0/sm0_authority_server_node_recovery_performance.gd",'
$ArgsNeedle = '        "--stop-file=$StopFile", "--fault-profile=$FaultProfile", "--recovery-dir=$RecoveryRoot"'
$ArgsReplacement = '        "--stop-file=$StopFile", "--fault-profile=$FaultProfile", "--recovery-dir=$RecoveryRoot", "--recovery-performance=p21"'
...
if ($CompileMatches -ne 1) {
    throw "SM0-P2.1 expected exactly one recovery-chain compile line, found $CompileMatches."
}
if ($ArgsMatches -ne 1) {
    throw "SM0-P2.1 expected exactly one server argument line, found $ArgsMatches."
}
$Patched = $Original.Replace($CompileNeedle, $CompileReplacement).Replace($ArgsNeedle, $ArgsReplacement)
if ($Patched -eq $Original) { throw "SM0-P2.1 temporary runner patch produced no change." }
if (-not $Patched.Contains('--recovery-performance=p21')) {
    throw "SM0-P2.1 temporary runner does not contain explicit performance mode."
}

$TempRoot = Join-Path ([IO.Path]::GetTempPath()) "distributed-world-simulator-sm0-p21"
New-Item -ItemType Directory -Force -Path $TempRoot | Out-Null
$TempRunner = Join-Path $TempRoot ("RUN_V0_SM0_GRAPHICAL_RECOVERY_PERFORMANCE_{0}_{1}.ps1" -f $PID, ([Guid]::NewGuid().ToString("N")))
...
try {
    Write-Host "[SM0-P2.1] Bounded movement replay + bounded recovery history enabled." -ForegroundColor Green
    Write-Host "[SM0-P2.1] Fast visual holds: phase=${PhaseHoldMs}ms outage=${OutageHoldMs}ms."
    & $TempRunner @Invoke
    $Code = $LASTEXITCODE
}
finally {
