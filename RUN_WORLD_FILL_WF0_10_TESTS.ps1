...
$ReportPath = Join-Path $ReportDirectory "world-fill-wf0-10-summary.json"
...
$Summary = [ordered]@{
    schema = "world_fill.wf0_10_runner_summary.v1"
...
try {
    ...
    Invoke-CheckedGodot -Name "wf0_8_local_memory" -Arguments @(
        "--headless", "--path", $ProjectRoot,
        "--script", "res://tests/world_fill/test_wf0_8_local_memory.gd"
    )
    Invoke-CheckedGodot -Name "wf0_9_sign_kit" -Arguments @(
        "--headless", "--path", $ProjectRoot,
        "--script", "res://tests/world_fill/test_wf0_9_sign_kit.gd"
    )
    Invoke-CheckedGodot -Name "wf0_10_showcase_kit" -Arguments @(
        "--headless", "--path", $ProjectRoot,
        "--script", "res://tests/world_fill/test_wf0_10_showcase.gd"
    )
    ...
    $Summary.passed = $true
    Save-Summary
    Write-Host "WORLD FILL WF0.10 tests passed." -ForegroundColor Green
