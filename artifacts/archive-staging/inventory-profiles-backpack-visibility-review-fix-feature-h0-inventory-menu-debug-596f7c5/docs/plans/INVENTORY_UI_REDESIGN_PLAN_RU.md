...
Критический инвариант: открытие split, подхват виртуального стака, `Esc` и смена
профиля не меняют Item Graph. Мутация выполняется только после валидной цели через
`InventoryCommandFacade.transfer_quantity()`.
