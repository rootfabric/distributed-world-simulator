# V0 SM1 / B6 R8 — Fresh Exact-Head Machine Re-Verifier Instruction
...
Historical verifier record: V0-SM1-R1-WO-001-VERIFICATION-001.v1.json = FAILED
Required new verifier record: V0-SM1-R1-WO-001-VERIFICATION-002.v1.json

## 0. Role boundary

You are not the Implementer, previous Verifier, Reviewer, Director, Windows validation agent or Human approver.

The previous Windows R8 validation is prior evidence only. It is not a substitute for your own execution.
...
Do not start B7.
Do not activate P7.
Do not silently rerun a failed decisive or canonical gate and report only a later pass.
...
The new B6 subject is the R8 implementation HEAD itself. It already contains the original SM1 composite subject and all authorized M5 repair layers.

~~~text
SUBJECT_HEAD = 25f5ddf6280a39a44ddfc3bbec5245873021c0a1
SUBJECT_TREE = 82c6567acdc735bc02a4808159328f00722b0b6b
SUBJECT_BRANCH = repair/m5-control-read-continuity-r8
...
- SUBJECT_HEAD is exactly 46 commits ahead of OLD_SUBJECT_HEAD;
- SUBJECT_HEAD is exactly 7 commits ahead of R7_HEAD;
- R7 -> R8 delta is exactly 6 files.
...
The R7 convergence barrier must not be changed by R8:
...
Use a new checkout not used by the Implementer, previous B6 Verifier, or Windows validation agent.

~~~powershell
git clone https://github.com/rootfabric/distributed-world-simulator.git C:\dws-sm1-b6-r8-verifier
cd C:\dws-sm1-b6-r8-verifier
git fetch origin --prune
git checkout --detach 25f5ddf6280a39a44ddfc3bbec5245873021c0a1
~~~
...
if ($Head -ne "25f5ddf6280a39a44ddfc3bbec5245873021c0a1") { throw "SUBJECT_HEAD_MISMATCH" }
if ($Tree -ne "82c6567acdc735bc02a4808159328f00722b0b6b") { throw "SUBJECT_TREE_MISMATCH" }
...
~~~powershell
git merge-base --is-ancestor 6fdfc047f54e727e6b398370e576c746c7949441 25f5ddf6280a39a44ddfc3bbec5245873021c0a1
if ($LASTEXITCODE -ne 0) { throw "OLD_SUBJECT_ANCESTRY_FAILED" }

git merge-base --is-ancestor b270fb806038333c97fa1ed49655961adddd6a21 25f5ddf6280a39a44ddfc3bbec5245873021c0a1
if ($LASTEXITCODE -ne 0) { throw "SOURCE_SM1_ANCESTRY_FAILED" }

git rev-list --count 6fdfc047f54e727e6b398370e576c746c7949441..25f5ddf6280a39a44ddfc3bbec5245873021c0a1
git rev-list --count baa0e192209e72aba5ae9d04663eea85b1099e82..25f5ddf6280a39a44ddfc3bbec5245873021c0a1
git diff --name-only baa0e192209e72aba5ae9d04663eea85b1099e82..25f5ddf6280a39a44ddfc3bbec5245873021c0a1
~~~

Expected counts: 46, 7, and exactly the six paths above.
...
## 6. Fresh focused SM1 belt

Use an isolated Godot profile.

~~~powershell
$Profile = Join-Path $PWD ("artifacts\test-results\b6-r8-focused-profile-" + $PID)
$env:HOME = $Profile
...
## 7. Repeated impaired-network stability
...
## 8. Canonical full world/core regression

Only after:

- R8 M5 pre-gates PASS;
- fresh M5 10/10 PASS;
...
## 10. Historical evidence cross-check

Read but do not inherit:

~~~text
config/control/harness/executions/E2026-08-24-V0-SM1-R1/verifications/V0-SM1-R1-WO-001-VERIFICATION-001.v1.json
config/control/harness/executions/E2026-08-24-V0-SM1-R1/evidence/V0-SM1-R1-B6-M5-REPAIR-MAP-008.v1.json
config/control/harness/executions/E2026-08-24-V0-SM1-R1/evidence/V0-SM1-R1-B6-M5-R8-WINDOWS-VALIDATION-001.v1.json
~~~

The R8 Windows receipt exists on the control-only dispatch carrier and is corroborating evidence only.
...
Record standard overall, V0 health, directional overall, critical candidate-specific hits, cross-branch overlaps and human-attention items.

Known prior exact-R8 CI evidence:

~~~text
Project Control run 33244990342 = SUCCESS
standard overall = YELLOW
directional overall = YELLOW
~~~

Do not relabel YELLOW as GREEN. Do not claim formal NON_RED predicates unless current machine/policy evidence proves them.

## 12. Freshness after execution
...
if ($FinalHead -ne "25f5ddf6280a39a44ddfc3bbec5245873021c0a1") { throw "FINAL_HEAD_CHANGED" }
if ($FinalTree -ne "82c6567acdc735bc02a4808159328f00722b0b6b") { throw "FINAL_TREE_CHANGED" }
...
## 13. Exact-head review freshness warning

Historical B5 Reviewer PASS is bound to OLD_SUBJECT_HEAD 6fdfc047..., not the repaired R8 subject.

Therefore:

- B6 machine verdict may be VERIFIED if all B6 machine predicates pass;
- do not claim a fresh exact-head Reviewer PASS for R8;
- B7/checkpoint proposal remains blocked until exact-head review freshness and remaining control predicates are reconciled.

## 14. Required new verifier record

After verification complete, create exactly one new control-only record:

~~~text
config/control/harness/executions/E2026-08-24-V0-SM1-R1/verifications/V0-SM1-R1-WO-001-VERIFICATION-002.v1.json
~~~

Do not overwrite VERIFICATION-001.
...
~~~json
{
  ...
  "verified_head_sha": "25f5ddf6280a39a44ddfc3bbec5245873021c0a1",
  "verified_tree_sha": "82c6567acdc735bc02a4808159328f00722b0b6b",
  "verifier": "INDEPENDENT_VERIFIER_B6_FRESH_SESSION_R2",
...
Commit the record only on a dedicated control-only verification-record branch descended from control/v0-sm1-b6-r8-verifier-dispatch-r2. Never commit it to the runtime subject.

## 15. VERIFIED requirements

VERIFIED requires all:

- exact R8 HEAD/TREE;
- ancestry/composition proven;
- exact double Godot;
- contracts 126/0;
- R7 guard 31/0;
- R8 control-read regression 25/0;
- pipe smoke with non-empty stdout/stderr;
- fresh M5 decisive 10/10 single attempt;
- focused SM1 13/13;
- impaired network 5/5;
- first and only canonical full regression PASS;
- 304 declared / 304 discovered / 307 steps / zero bad exits;
- no persistent M5_CONTROL_READ_UNAVAILABLE;
- no M5_CONVERGENCE_RELEASE_INTEGRITY_FAILED;
- semantic audit proves no convergence weakening;
...
Pre-gate failure:
B6 BLOCKED; decisive/canonical not run.

Decisive 10x failure:
B6 BLOCKED; NO RERUN; canonical not run.

Focused SM1 or impaired-network failure:
B6 BLOCKED; preserve first exact failure.

Canonical failure:
B6 BLOCKED; preserve first canonical attempt; do not replace with rerun.
...
- P7 activated;
- fresh Reviewer PASS on R8;
- PC0 GREEN when machine says YELLOW.

## 18. Expected next state after VERIFIED

~~~text
B6 MACHINE VERIFICATION = CLOSED
R8 = VERIFIED / NOT MERGED
B7 = BLOCKED pending exact-head review freshness + remaining control reconciliation
