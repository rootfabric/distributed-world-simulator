...
## Постоянный слой мира

```text
scripts/persistence/
└── lunar_world_repository.gd
```

Репозиторий хранит только изменяемые сущности и журнал операций. Процедурные меши остаются ответственностью `scripts/world/terrain/`.
