...
1. Start once with `-ResetState`.
2. Wait until the HUD reports CH9.6 `READY` and the inventory opens.
...
7. Start again **without** `-ResetState`.
8. Verify the previously equipped items are already present in the equipment slots and on the character without any new drag operation.
9. Drag an equipped item back to the backpack and verify the same item disappears from the character.
...
- manual graphical acceptance passes on the same implementation lineage;
