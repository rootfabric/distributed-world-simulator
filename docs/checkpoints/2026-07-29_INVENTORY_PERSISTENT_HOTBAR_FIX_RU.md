...
- Выбор предмета ЛКМ обновляет инспектор без перестройки сетки, поэтому drag из рюкзака не теряет исходную ячейку.
...
- `tests/ui/test_inventory_ui_i1_interactions.gd`: 71/71 PASS.
- `git diff --check`: PASS.
