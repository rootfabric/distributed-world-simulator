...
- **Отвергнуто:** all-pairs скан как **scaling-решение** (§13 требует O(N + C + local)). В R1-микрокосме (N=25) агрегатор использует ограниченный попарный проход с отсечкой по дистанции кроны; cell-buckets обеспечивают детерминированное membership и per-cell наблюдаемость, но ещё не используются для neighbor-pruning. Bucket-pruning до O(N + C + local) — обязательная часть FFF6/FFF7 (см. ограничения).
...
2. Агрегатор в R1 — ограниченный попарный проход O(N²) при N=25 с дистанционной отсечкой; cell-buckets дают membership/наблюдаемость, но neighbor-pruning через buckets (O(N + C + local), §13) реализуется в FFF6/FFF7 при переходе к лесным масштабам.
3. Геометрия публикации (для светового поля) реализуется под базовой средой; свет входит через оценку/отбор (повторная реализация под собственным understory light в ON-режиме). Равные по высоте не затеняют друг друга (упрощение R1); послойное световое насыщение — FFF6.
4. ON/OFF: общий поток мутаций, среда назначает его линиям — назначение среды и есть тестируемая причинность (задокументировано; «common pool» в смысле EVO6 применяется к сценариям среды, здесь сравнивается ON/OFF одного сообщества).
5. Shade_tolerance генома пока не входит в light-компоненту (наследие water fitness); световая толерантность в R1 выражается через low-cost формы.
6. Water/litter каналы effect-записей зарезервированы и защищены нулями — активируются в FFF4/FFF5.
