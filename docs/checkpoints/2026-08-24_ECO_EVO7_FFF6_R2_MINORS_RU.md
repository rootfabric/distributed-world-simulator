...
- **Фактическое состояние на базовом HEAD:** свёртка `RUN_ECO_EVO6_WATER_SELECTION.ps1 -SkipBaseline` в хвост `RUN_ECO_EVO7_FFF6_TESTS.ps1` уже присутствует — коммит `c0a70efc` («test(eco): fold EVO6-WATER determinism regression into FFF6 chain»), выполненный ПОСЛЕ текста аудита (аудит смотрел HEAD `d129b0ba`). Текст MINOR-1 устарел наполовину: включение есть, но guard базового хэша отсутствовал.
