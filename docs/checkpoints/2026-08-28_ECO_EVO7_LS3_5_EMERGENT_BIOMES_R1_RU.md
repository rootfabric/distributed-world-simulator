...
## Source-integrity hardening R2

Fresh semantic review R1 rejected acceptance because the classifier deeply validated `competition_field`, but did not yet prove the entire supplied LS3.4 snapshot was canonical. R2 leaves all biome scoring/ecotone rules unchanged and hardens only the read-only source boundary.

R2 additionally proves before classification:

- exact LS3.1 EnvironmentField key set + exact revision + canonical cell/field hashes;
- exact LS3.4 snapshot key set / revision / mode / shadow authority boundary;
- every LS3.3 survivor record passes canonical record/bundle validation;
- `postcompetition_population_hash`, `occupied_map_hash`, and `hereditary_pool_hash` recompute exactly from survivor records;
- LS3.4 competition field remains valid and `record_count_after / survivor_count / culled_count` are mutually consistent;
- every current survivor maps 1:1 to a `survives=true` competition evaluation with the same cell and bundle checksum;
- exact LS3.4 state hash recomputes from the supplied snapshot.

New adversarial acceptance rejects:

1. stale EnvironmentField revision even when its old field hash still matches;
2. forged LS3.4 population hash after state rehash;
3. source ecology authority escalation even though LS3.4 state hashing uses frozen authority identity;
4. unexpected hidden LS3.4 source field;
5. survivor/evaluation relocation after complete competition-field + state rehash.

R2 exact-double focused result:

```text
ECO.EVO7 LS3.5 Emergent Biomes: PASS (71 assertions)
rc=0
```

R2 inherited full chain:

```text
45 / 19 / 48 / 51 / LS2 smoke / 89 / 11369 / 24 / Spatial 1024 / LS3.2 2382 / LS3.3 44 / LS3.4 42 / LS3.5 71
runner_rc=0
runtime=123s
```
