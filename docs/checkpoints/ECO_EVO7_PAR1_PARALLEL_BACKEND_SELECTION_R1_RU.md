...
Статус: R2 ROLL-FORWARD LOCAL CANDIDATE (не принят; external review pending).
...
- причина: на больших фикстурах 512/1024/2048 геометрическое среднее p50 полного времени рекрутмента на зафиксированных R1 измерениях ≈963.2 ms против ≈1343.7 ms у WTP (PROCESS преимущество ≈39.5% по принятой формуле сравнения); p95 у PROCESS также был лучше. R2 исправляет selector так, чтобы fresh benchmark вычислял geometric mean и реально применял p95 guard.
...
External review обнаружил, что R1 selector декларировал geometric mean + p95 guard, но фактически использовал arithmetic mean и не применял p95 guard в решении. R2 исправляет только decision code:

- geometric mean p50 считается буквально по fixtures 512/1024/2048;
- worker count выбирается по минимальному geometric-mean p50;
- >=15% победитель допускается только если geometric-mean p95 не хуже другого backend более чем на 20%;
- при отсутствии валидного победителя применяется документированный WTP tie-break.

Production activation всё ещё НЕ происходит в PAR1; fresh benchmark/verifier обязателен перед acceptance.
