...
Goal: make the current validated build the normal product continuation base.
...
No new gameplay capability should be mixed into this integration.
...
The donor commit is attractive because it is mostly additive: canonical world-item runtime/presentation, target interaction, Earth spatial projection and focused tests. Port its capability onto the current V0 frontier; do NOT merge its historical base.
...
This is the highest-value next capability because it closes the gap between the existing inventory UI and an interactive shared world.

### V0-P2 — Reconnectable shared state as a hard gate

Before resource/mining work, make reconnect a routine regression rather than a one-off demonstration.
...
Do not proceed to terrain mutation while this is unstable.
...
This proves the gameplay economy loop without coupling the MVP to volumetric terrain foundations.

Historical MW/Matter branches are design/implementation donors, not merge bases.

### V0-P4 — Construction consumes real resources

Current Construction is already interactive and authoritative. The next meaningful upgrade is economy coupling, not more Construction geometry.
...
Failure must not consume resources without construction and must not construct without canonical consumption.
...
Main control records CH9.6 as accepted/frozen evidence. Transfer only presentation/equipment capability needed by the V0 loop:
...
Do this after resource commands exist, so equipment has gameplay meaning instead of being a detached visual system.

### V0-P6 — Persistent shared outpost loop

Combine the already-proven pieces into one repeatable scenario:
...
This is the right point to call the product slice a stable playable baseline for ordinary feature development, subject to the project's independent review/control rules.

### V0-P7 — Terrain/matter mutation

Only after V0-P6 is stable, start a bounded terrain interaction track.

Potential donor concepts exist in MW4/MW5/MW6 and later Matter/representation branches, but they must be projected onto current canonical World Query/Transaction/Material/replication foundations rather than merged as an old stacked lineage.

First terrain slice should be intentionally small:
...
### Later — scale and richness

After the core loop is stable:

- richer procedural generation and biome/world content;
- advanced materials and volumetric Matter;
- large Construction / structural simulation;
- ecology;
- distributed zones/server handoff;
- high-count agents;
- advanced ships/stations/economy.

These are not prerequisites for the next playable iteration.
