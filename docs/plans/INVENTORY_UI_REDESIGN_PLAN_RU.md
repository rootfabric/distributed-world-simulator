...
Тип этапа: presentation-only UX gate перед `v16.4 Foundation Gate` и `N0 Network Contracts`

Статус реализации:

- `UI-I0` — реализован в `feature/ui-i0-inventory-shell`, checkpoint `v16.3.1-ui-i0`;
- `UI-I1` — следующий этап этой feature-ветки;
- checkpoint: `docs/checkpoints/2026-07-27_UI_I0_COMPONENT_INVENTORY_SHELL_RU.md`.
...
```text
v16.3 current checkpoint
→ UI-I0/UI-I1 Inventory UX stabilization
→ v16.4 Foundation Gate + N0
→ UI-I2 search/large storage параллельно
→ UI-I3 batch operations после command/network contracts
```

UI-I0/UI-I1 — ограниченный pre-roadmap checkpoint. Он не должен разрастаться в
новую gameplay-систему и задерживать разделение simulation/presentation.
