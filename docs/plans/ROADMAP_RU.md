...
- отдельная визуальная лаборатория, доступная по `F5`;
...
## Текущий приоритет: v15.4 Item Relations Foundation

До продолжения placement/building реализуется и стабилизируется домен предметов:

1. единый `ItemInstance` и отношения `WORLD`, `CONTAINER`, `ATTACHMENT`;
2. транзакционный перенос с идемпотентным `operation_id`;
3. вложенные контейнеры, рекурсивная масса и запрет циклов;
4. физическое представление свободного предмета;
5. жёсткие сокеты сборок и демонтаж модулей;
6. обязательный headless regression suite.

Placement preview, переработка и модульный ровер строятся только поверх прошедшего эти тесты домена.
