...
## Текущее состояние: v15.5.1-fixed

Перед продолжением контейнеров и первой базы введены общий слой симулятора и distributed-ready координатный фундамент.
`main.tscn` больше не является конкретной Earth–Moon картой: он запускает
`SimulatorApp`, который выбирает runtime из каталога, владеет консолью и
регрессионным реестром. Существующие Luna/Earth/Item системы сохранены как
подключаемые миры.

Автоматическая матрица обязана загружать все пять начальных миров в одном
процессе и выполнять smoke-тесты каждого из них. Это становится обязательным
барьером перед следующими изменениями предметов, контейнеров и строительства.

## Текущий приоритет: двигаться от v15.5.1-fixed к контейнерам пользователя

1. принять `v15.5.1-fixed` как базовую координатную и partition-версию;
2. перед каждой следующей итерацией запускать coordinate foundation и полный
   multi-world regression на целевой double-precision сборке Godot;
3. закреплять новые действия командами вместо прямых биндов;
4. использовать `item_lab` для доменных сценариев предметов;
5. использовать `playground` для закрытых пользовательских контейнеров,
   интеракций и строительных прототипов;
6. продолжить persistence предметного домена и контейнеры пользователя;
7. затем перейти к placement preview и первой локальной базе.
