...
Реализован первый постоянный изменяемый слой поверх процедурной Луны.
Survey Beacon сохраняется в JSON своего чанка, выгружается вместе с чанком и
восстанавливается при возвращении области в Warm/Active окно.

Ближайший обязательный этап — первая локальная база: несколько типов модулей,
placement preview, проверка уклона и пересечений, connection sockets и простой
power graph.
