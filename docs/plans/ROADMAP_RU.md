...
## Текущее состояние: v15.5

Перед продолжением контейнеров и первой базы введён общий слой симулятора.
...
## Текущий приоритет: стабилизация v15.5, затем контейнеры пользователя

1. прогнать multi-world regression на целевой double-precision сборке Godot;
2. закрепить команды вместо новых прямых биндов;
3. использовать `item_lab` для доменных сценариев предметов;
4. использовать `playground` для закрытых пользовательских контейнеров,
   интеракций и строительных прототипов;
5. после зелёной регрессии продолжить persistence предметного домена и
   контейнеры пользователя;
6. затем перейти к placement preview и первой локальной базе.
