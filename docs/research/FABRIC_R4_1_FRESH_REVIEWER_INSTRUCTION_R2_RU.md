# FABRIC R4.1 — FRESH REVIEWER DISPATCH R2
...
## R2 freshness amendment

R1 dispatch is stale because the product test-only fixture helper was missing in the remote `5ed033...` tree. R2 binds the corrected exact subject `a5001ee... / 934ecfed...`. The final subject-only change adds the missing `_f64_le()` helper; it does not change runtime solver or acceptance thresholds. The Reviewer must still inspect the complete diff from historical `e6228a39...`.
