...
8. ~~**Manifest/A4 stock bounds mismatch**~~ — **ИСПРАВЛЕНО R2**: polygon manifest теперь явно владеет `CELL_CAPACITY_MG = 1_000_000`, совпадающим с фактической capacity локального ecology field. Это сохраняет A4/A5 water-normalization (`water = stock/capacity`) и fail-closed отклоняет oversized zone/patch inputs вместо скрытого изменения ecological semantics. P1/P4 фиксируют boundary regression.
