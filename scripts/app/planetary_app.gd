...
func is_spectator_active_for_teleport() -> bool:
	return shared_space_mode or super.is_spectator_active_for_teleport()


func teleport_player_to_spectator() -> bool:
	if not shared_space_mode:
		return super.teleport_player_to_spectator()
	...
	if observer_body_id != "moon":
		last_action_result = "Игрок пока может телепортироваться только к спектатору у Луны"
		return false
	var moon_local_position: Vector3 = celestial_system.to_body_local(
		observer_space_position,
		"moon"
...
func create_runtime_snapshot() -> Dictionary:
	...
	snapshot["schema"] = "planet_simulator.planetary_world_runtime.v1"
