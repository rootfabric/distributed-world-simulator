...
static func build_damage_batch(
	plan: Dictionary,
	before_item_graph_state: Dictionary,
	after_item_graph_state: Dictionary,
	before_ledger_state: Dictionary,
	...
	if not bool(plan_validation.get("success", false)):
		return plan_validation
	if authority_owner_id.is_empty() or authority_epoch < 1 or item_graph_revision < 0 or ledger_revision < 0 or server_tick < 0:
		return _failure("INVALID_C9_M0_TRANSLATION_AUTHORITY")
	var rows: Array = []
	rows.append(_update_row(ITEM_GRAPH_AGGREGATE_ID, ITEM_GRAPH_KIND, ITEM_GRAPH_STATE_SCHEMA, item_graph_revision, before_item_graph_state, after_item_graph_state, authority_owner_id, authority_epoch, server_tick))
	rows.append(_update_row(LEDGER_AGGREGATE_ID, LEDGER_KIND, LEDGER_STATE_SCHEMA, ledger_revision, before_ledger_state, after_ledger_state, authority_owner_id, authority_epoch, server_tick))
	for mutation in plan["construct_mutations"]:
		var construct_id: String = String(mutation["construct_id"])
		var row_result: Dictionary = _construct_row(
			aggregate_id_for_construct(construct_id), mutation, authority_owner_id, authority_epoch, server_tick,
			int(construct_authority_revisions.get(construct_id, -1))
		)
		if not bool(row_result.get("success", false)):
			return row_result
		rows.append(row_result["row"])
	rows.sort_custom(func(left, right): return String(left["aggregate_id"]) < String(right["aggregate_id"]))
	var preconditions: Array = []
	var operations: Array = []
	for row in rows:
		preconditions.append(row["precondition"])
		operations.append(row["operation"])
	var batch_id: String = batch_id_for_plan(plan)
	var batch: Dictionary = BatchScript.create(batch_id, String(plan["operation_id"]), authority_owner_id, authority_epoch, server_tick, preconditions, operations, [])
	var validation: Dictionary = BatchScript.validate(batch)
	if not bool(validation.get("success", false)):
		return _failure("C9_M0_BATCH_REJECTED", {"cause": validation})
	return _success({"batch": batch, "batch_id": batch_id})
