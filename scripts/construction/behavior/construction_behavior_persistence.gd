...
var _profile_store
var _state_store
var _state_key: String = "construction-behavior-profiles"
var _configured: bool = false


func setup(profile_store, state_store, state_key: String = "construction-behavior-profiles") -> Dictionary:
	if profile_store == null or not profile_store.has_method("to_dict") or not profile_store.has_method("load_dict"):
		return _failure("CONSTRUCTION_BEHAVIOR_PROFILE_STORE_REQUIRED")
	if state_store == null or not state_store.has_method("save_state") or not state_store.has_method("load_state"):
		return _failure("CONSTRUCTION_BEHAVIOR_STATE_STORE_REQUIRED")
	if state_key.strip_edges().is_empty():
		return _failure("CONSTRUCTION_BEHAVIOR_STATE_KEY_REQUIRED")
...
func save() -> Dictionary:
	if not _configured:
		return _failure("CONSTRUCTION_BEHAVIOR_PERSISTENCE_NOT_CONFIGURED")
...
func load() -> Dictionary:
	...
	if not state_value is Dictionary:
		return _failure("INVALID_CONSTRUCTION_BEHAVIOR_PERSISTED_STATE")
