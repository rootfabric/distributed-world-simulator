...
const SCHEMA: String = "planet_simulator.construction_mobile_profile_store.v1"
...
func publish_profile(profile: Dictionary) -> Dictionary:
	...
	if _profiles_by_construct_id.has(construct_id):
		...
		if incoming_revision < current_revision:
			return _failure("STALE_CONSTRUCTION_MOBILE_PROFILE", {
				"construct_id": construct_id,
				"incoming_revision": incoming_revision,
				"current_revision": current_revision,
			})
		if incoming_revision == current_revision:
			return _failure("CONSTRUCTION_MOBILE_PROFILE_SAME_REVISION_CONFLICT", {
				"construct_id": construct_id,
...
func remove_profile(construct_id: String, expected_construct_checksum: String) -> Dictionary:
	...
	if String(existing["construct_checksum"]) != expected_construct_checksum:
		return _failure("CONSTRUCTION_MOBILE_PROFILE_REMOVE_PRECONDITION_MISMATCH")
...
static func validate_state(value: Dictionary) -> Dictionary:
	...
	if value.get("schema") != SCHEMA:
		return _failure("UNSUPPORTED_CONSTRUCTION_MOBILE_PROFILE_STORE_SCHEMA")
	if not UtilsScript.is_json_integer(value.get("generation")) or int(value["generation"]) < 0:
		return _failure("INVALID_CONSTRUCTION_MOBILE_PROFILE_STORE_GENERATION")
	if typeof(value.get("profiles")) != TYPE_ARRAY:
		return _failure("INVALID_CONSTRUCTION_MOBILE_PROFILE_STORE_PROFILES")
	var seen: Dictionary = {}
	var previous_construct_id: String = ""
	for raw in value["profiles"]:
		if typeof(raw) != TYPE_DICTIONARY:
			return _failure("INVALID_PERSISTED_CONSTRUCTION_MOBILE_PROFILE")
		...
		if seen.has(construct_id):
			return _failure("DUPLICATE_PERSISTED_CONSTRUCTION_MOBILE_PROFILE")
		if not previous_construct_id.is_empty() and construct_id < previous_construct_id:
			return _failure("PERSISTED_CONSTRUCTION_MOBILE_PROFILES_NOT_SORTED")
		seen[construct_id] = true
		previous_construct_id = construct_id
	if String(value.get("checksum", "")) != compute_checksum(value):
		return _failure("CONSTRUCTION_MOBILE_PROFILE_STORE_CHECKSUM_MISMATCH")
	if not bool(UtilsScript.canonicalize(value).get("success", false)):
		return _failure("CONSTRUCTION_MOBILE_PROFILE_STORE_NOT_JSON_SAFE")
	return _success()
