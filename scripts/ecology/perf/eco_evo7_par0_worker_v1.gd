...
func _evaluate_candidate_build(job_id: String, payload: Dictionary, started: int) -> Dictionary:
	...
	if not parents.is_empty():
		candidates = Par3Kernel.build_all(
	...
	## Empty canonical slices are valid when parent_count < worker_count.
	## They return a normal zero-result response and must never fail the job.
	var input_keys := PackedStringArray()
