...
static func evaluate_bundle_against_observation(bundle: Dictionary, observation: Dictionary) -> Dictionary:
	...
	var result := {
		...
		"photosynthetic_gain_proxy": snappedf(float(phenotype["photosynthetic_gain_proxy"]), 1e-9),
		"realized_photosynthetic_gain": snappedf(realized_photosynthetic_gain, 1e-9),
		"maintenance_cost_proxy": snappedf(float(phenotype["maintenance_cost_proxy"]), 1e-9),
		"establishment_capacity": snappedf(float(phenotype["establishment_capacity"]), 1e-9),
		"fitness_establishment_term": snappedf(0.35 * float(phenotype["establishment_capacity"]), 1e-9),
		"water_match": snappedf(water_match, 1e-9),
		"fitness_water_match_term": snappedf(0.30 * water_match, 1e-9),
		"fitness_shade_term": snappedf(shade_adaptation, 1e-9),
		"fitness_drought_cost": snappedf(drought_cost, 1e-9),
