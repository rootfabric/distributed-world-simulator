...
const VERSION := "1.0.0"
const REVISION := "ECO.EVO7-LS3.0.2"
...
func validate_patch(patch: Dictionary) -> bool:
    ...
    if not is_finite(radius) or radius < 1000.0:
        return false
    if not is_finite(float(patch.get("patch_width_m", NAN))) or absf(float(patch["patch_width_m"]) - float(grid_size) * cell_size_m) > 1e-9:
        return false
    for vector_field in ["center_direction", "east", "north"]:
        var vector_value = patch.get(vector_field)
        if not vector_value is Vector3 or not _finite_direction(Vector3(vector_value)):
            return false
    ...
    for value in cells:
        ...
        if not direction_value is Vector3 or not _finite_direction(Vector3(direction_value)):
            return false
...
func _patch_hash(patch: Dictionary) -> String:
    ...
    var center: Vector3 = patch["center_direction"]
    tokens.append(_f(center.x)); tokens.append(_f(center.y)); tokens.append(_f(center.z))
    var by_index := {}
