...
const VERSION := "1.0.0"
const REVISION := "ECO.EVO7-LS3.5.1"
const GRID_SIZE := 32
...
func _valid_sources(environment_field: Dictionary, ecology_snapshot: Dictionary) -> bool:
    if String(environment_field.get("schema", "")) != ENV_SCHEMA or String(environment_field.get("version", "")) != ENV_VERSION:
        return false
    ...
    if String(environment_field.get("field_hash", "")) != String(env_validator.call("_field_hash", environment_field)):
        return false
    if String(ecology_snapshot.get("schema", "")) != ECOLOGY_SCHEMA or String(ecology_snapshot.get("version", "")) != ECOLOGY_VERSION:
        return false
    if int(ecology_snapshot.get("generation", 0)) < 1 or not bool(ecology_snapshot.get("competition_enabled", false)):
        return false
    if String(ecology_snapshot.get("environment_field_hash", "")) != String(environment_field["field_hash"]):
        return false
    if String(ecology_snapshot.get("state_hash", "")).length() != 64 or String(ecology_snapshot.get("postcompetition_population_hash", "")).length() != 64:
        return false
    if not ecology_snapshot.get("records") is Array or int(ecology_snapshot.get("record_count", -1)) != Array(ecology_snapshot["records"]).size():
        return false
    if int(ecology_snapshot.get("survivor_count", -1)) != int(ecology_snapshot.get("record_count", -2)):
        return false
    if not ecology_snapshot.get("competition_field") is Dictionary:
        return false
    var field: Dictionary = ecology_snapshot["competition_field"]
