...
def _close_gate(continuation: dict[str, object]) -> tuple[bool, int, str]:
    """Authorize session termination only at a machine-declared safe boundary."""
    if continuation.get("session_exit_allowed") is True:
        return True, 0, "SESSION_EXIT_AUTHORIZED"
    action = str(continuation.get("next_action") or "UNKNOWN_NEXT_ACTION")
    obligation = str(continuation.get("stop_obligation") or "CONTINUE_ACTIVE_ROLE")
    return False, EXIT_CODES["SESSION_EXIT_FORBIDDEN"], f"SESSION_EXIT_FORBIDDEN:{action}:{obligation}"


def _next_payload(state: dict[str, object], continuation: dict[str, object]) -> dict[str, object]:
    active = state["active_work_order"]
    assert isinstance(active, dict)
    return {
        "checkpoint": active["goal_checkpoint"],
        "work_order": active["work_order_id"],
        ...
        "evidence_sink": continuation["evidence_sink"],
        "resume_condition": continuation["resume_condition"],
        "on_success": continuation["on_success"],
        "on_failure": continuation["on_failure"],
        "human_decision_required": continuation["human_decision_required"],
        "session_exit_allowed": continuation["session_exit_allowed"],
        "closure_loop_required": continuation["closure_loop_required"],
        "stop_obligation": continuation["stop_obligation"],
        "verification_commands": state["verification_commands"],
        "human_gate": active.get("human_approval_required_for", []),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("mode", choices=("status", "plan", "resume", "close"))
    ...
    try:
        ...
        state["next"] = _next_payload(state, continuation)
        ...
        if args.mode == "plan":
            ...
            state["resume"] = {
                ...
                "next_work_order": state["active_work_order"]["work_order_id"] if continuation["next_actor"] in {"IMPLEMENTER", "VERIFIER", "INTEGRATOR"} else None,
                ...
                "mission": {
                    ...
                    "parent_mission_id": continuation["parent_mission_id"],
                    "mission_complete": continuation["mission_complete"],
                },
                "next_actor": continuation["next_actor"],
                "next_action": continuation["next_action"],
            ...
            if not authorized:
                state["ok"] = False
                state["error"] = {"code": "SESSION_EXIT_FORBIDDEN", "detail": detail}
                _emit(state)
                return exit_code
