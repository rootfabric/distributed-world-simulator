...
func item_recursive_mass_kg(item_id: String, seen: Dictionary = {}) -> float:
	if seen.has(item_id):
		return 0.0
	seen[item_id] = true
	...
	if not owned_container_id.is_empty():
		total += container_mass_kg(owned_container_id, seen)
	return total


func container_mass_kg(container_id: String, seen: Dictionary = {}) -> float:
	var container = container_registry.get_container(container_id)
	if container == null:
		return 0.0
	var total := 0.0
	for item_id in container.item_ids:
		total += item_recursive_mass_kg(item_id, seen)
	return total
