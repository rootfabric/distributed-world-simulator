	...
	if Relations.kind_of(new_relation) == Relations.CONTAINER:
		...
		if bool(merge_result.get("merged", false)):
			var result = {
				...
				"item_id": item_id,
				"merged": true,
				"merged_into_item_id": merge_result.get(
					"merged_into_item_id",
					""
				),
			}
...
func _validate_capacity(
	...
	var item = item_registry.get_item(item_id)
	var item_mass = mass_service.item_recursive_mass_kg(item_id)
	var item_volume = mass_service.item_external_volume_l(item_id)
	var current_mass = mass_service.container_mass_kg(container_id)
	var current_volume = mass_service.container_direct_volume_l(container_id)

	var already_in_target = (
		item != null
