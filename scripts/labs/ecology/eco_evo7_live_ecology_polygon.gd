...
## ECO.EVO7 LS2 — live ecology polygon.
## Presentation-only shell over the LS1 RAM session. The real ProceduralEarthWorld
## supplies environment observations; only the copied in-memory populations evolve.
## Nothing in this scene can write world/ecology/persistence/network/XFER state.
...
func _autocap() -> void:
	...
	if not ready_success:
		print("ECO.EVO7 LS2 Live Ecology Polygon: FAIL init")
		get_tree().quit(1)
		return
	var before_generation := int(snapshot.get("generation", -1))
	if not step_now(1):
		print("ECO.EVO7 LS2 Live Ecology Polygon: FAIL step")
		get_tree().quit(1)
		return
	print("ECO.EVO7 LS2 Live Ecology Polygon: PASS generation=%d->%d plants=%d state=%s" % [
		before_generation, int(snapshot.get("generation", -1)), visual_plant_count,
		String(snapshot.get("state_hash", "")).substr(0, 16)])
	get_tree().quit(0)
...
func _advance_generations(count: int) -> bool:
	var next_snapshot: Dictionary = session.step_generations(count)
...
func _refresh_hud() -> void:
	...
	var lines := PackedStringArray([
		"ECO.EVO7 LS2 — LIVE EARTH ECOLOGY POLYGON / SHADOW ONLY / NO WORLD WRITES",
	...
	for zone_value in Array(snapshot.get("zones", [])):
		...
		lines.append("%-10s %7.3f  %7.3f  %8.3f %8.3f %7.3f %7.3f  %s (%d/12)" % [
			...
			float(zone.get("moisture", 0.0)), float(zone.get("sunlight", 0.0)),
...
func _refresh_status() -> void:
	if not ready_success:
		return
	status.text = "runtime=%s | pending=%d | presentation source=real ProceduralEarthWorld | mutation=canonical LineageExtension only | persistence/network/XFER=OFF" % [
