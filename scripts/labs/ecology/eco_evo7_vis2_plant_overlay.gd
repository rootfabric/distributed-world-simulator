...
func crown_radius_px(descriptor: Dictionary, cell_px: float) -> float:
    ...
    return clampf(cell_px * (0.055 + 0.17 * sqrt(lai_norm)) * shoot_fraction, 1.3, cell_px * 0.25)
...
func root_depth_px(descriptor: Dictionary, cell_px: float) -> float:
    ...
    return clampf(cell_px * 0.30 * sqrt(clampf(depth_m / 5.0, 0.0, 1.0)), 0.0, cell_px * 0.30)
...
func root_spread_px(descriptor: Dictionary, cell_px: float) -> float:
    ...
    return clampf(cell_px * 0.24 * sqrt(clampf(spread_m / 6.0, 0.0, 1.0)), 0.0, cell_px * 0.24)
...
func _draw_plant(descriptor: Dictionary, center: Vector2, cell_px: float) -> void:
    ...
    var color := Color(0.72, 0.78, 0.70, 0.88) if founder else lineage_color(String(descriptor.get("lineage_id", "")), health)
    ...
    var stem_width := clampf(1.0 + camera_zoom * (0.45 + (1.0 - root_shoot) * 0.85), 1.0, 4.2)
    ...
    if show_roots and not founder:
        ...
        var root_color := Color(0.55, 0.36, 0.19, 0.50)
        draw_line(ground, ground + Vector2(0.0, root_depth), root_color, maxf(1.0, stem_width * 0.72), true)
        draw_line(ground + Vector2(0.0, root_depth * 0.46), ground + Vector2(-root_spread, root_depth * 0.80), root_color, 1.0, true)
        draw_line(ground + Vector2(0.0, root_depth * 0.52), ground + Vector2(root_spread, root_depth * 0.84), root_color, 1.0, true)
