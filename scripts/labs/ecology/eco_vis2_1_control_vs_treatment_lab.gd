...
func sample_environment_context_at(x: float, z: float) -> Dictionary:
	...
	context["environment"] = sample_environment_at(x, z)
	context["experiment_profile"] = _vis21_experiment_for_generation(_vis18r_generation)
	context["experiment_intensity"] = _vis21_intensity_for_generation(_vis18r_generation)
	return context
...
func begin_paired_experiment() -> Dictionary:
	...
	var control_result: Dictionary = _vis21_control.configure_from_fork(_vis21_fork_generation, _vis21_fork_map, _vis21_fork_history)
	...
	var treatment_result: Dictionary = _vis21_treatment.configure_from_fork(_vis21_fork_generation, _vis21_fork_map, _vis21_fork_history, _vis21_treatment_profile, _vis21_treatment_intensity, _vis21_common_root)
	...
	_vis21_treatment_schedule = [{"effective_generation": _vis21_fork_generation + 1, "experiment_id": _vis21_treatment_profile, "intensity": _vis21_treatment_intensity}]
	...
	_update_vis18r_title()
	_update_status()
	return {"success": true, "fork_generation": _vis21_fork_generation, "common_random_seed_hash": _vis21_common_root, "field_hash": String(fork_point.get("control_field_hash", ""))}
...
func advance_paired_to(target_generation: int) -> Dictionary:
	...
	if target_generation > _vis21_simulated_generation:
		var control_result: Dictionary = _vis21_control.advance_to(target_generation)
		if not bool(control_result.get("success", false)):
			return {"success": false, "reason": "CONTROL_ADVANCE_FAILED", "detail": control_result}
		var treatment_result: Dictionary = _vis21_treatment.advance_to(target_generation)
		if not bool(treatment_result.get("success", false)):
			return {"success": false, "reason": "TREATMENT_ADVANCE_FAILED", "detail": treatment_result}
		_vis21_simulated_generation = target_generation
	_vis18r_generation = target_generation
	_vis18r_play_accumulator = 0.0
	_clear_vis19_progressive_detail()
	_render_vis21_generation(target_generation)
	_rebuild_vis21_comparison()
	_update_vis18r_title()
	_update_status()
	return {"success": bool(_vis21_comparison_summary.get("success", false)), "generation": target_generation}
...
func set_realtime_turnover_generation(generation: int) -> void:
	if not _vis21_paired:
		...
		return
	var target := clampi(generation, _vis21_fork_generation, _vis21_simulated_generation)
...
func set_treatment(profile: String, intensity: float) -> Dictionary:
	...
	if _vis18r_generation < _vis21_simulated_generation:
		var rewind_result: Dictionary = _vis21_treatment.restart_from_fork()
		if not bool(rewind_result.get("success", false)):
			return rewind_result
		if _vis18r_generation > _vis21_fork_generation:
			var replay_result: Dictionary = _vis21_treatment.advance_to(_vis18r_generation)
	...
	retained.append({"effective_generation": effective_generation, "experiment_id": normalized_profile, "intensity": normalized_intensity})
...
func get_vis21_state() -> Dictionary:
	return {"stage": VIS21_STAGE, "mode": VIS21_MODE, "paired": _vis21_paired, "fork_generation": _vis21_fork_generation, "paired_generation": _vis18r_generation, "simulated_generation": _vis21_simulated_generation, "common_random_seed_hash": _vis21_common_root, "control_map": _vis21_control.generation_map(_vis18r_generation) if _vis21_paired else {}, "treatment_map": _vis21_treatment.generation_map(_vis18r_generation) if _vis21_paired else {}, "control_trace_count": _vis21_control_trace.size(), "treatment_trace_count": _vis21_treatment_trace.size(), "comparison_point_count": int(_vis21_comparison_summary.get("point_count", 0)), "treatment_profile": _vis21_treatment_profile, "treatment_intensity": _vis21_treatment_intensity, "control_data_only": true, "visible_population_fields": 1, "whole_field_ph5_rebuilds": _vis18r_ph5_rebuilds_during_turnover, "progressive_ph5_count": _vis19_detail_nodes.size(), "scene_node_count": get_tree().get_node_count() if get_tree() != null else 0, "peak_scene_node_count": _vis21_peak_node_count}
...
func get_vis21_canonical_traces() -> Dictionary:
	return {"control": _vis21_control_trace.duplicate(true), "treatment": _vis21_treatment_trace.duplicate(true)}
...
func _rebuild_vis21_comparison() -> void:
	...
	if not _vis21_paired:
		...
		return
	for generation in range(_vis21_fork_generation, _vis21_simulated_generation + 1):
		...
		var control_point := TraceAdapter.from_generation_map(generation, control_map, ControlRunner.BRANCH_ID, ControlRunner.EXPERIMENT_ID, String(control_environment.get("environment_revision", "")))
		var treatment_point := TraceAdapter.from_generation_map(generation, treatment_map, TreatmentRunner.BRANCH_ID, _vis21_experiment_for_generation(generation), String(treatment_environment.get("environment_revision", "")))
	...
	while _vis21_control_trace.size() > VIS21_SERIES_WINDOW:
		_vis21_control_trace.pop_front()
		_vis21_treatment_trace.pop_front()
	_vis21_comparison_summary = ComparisonModel.summarize(_vis21_control_trace, _vis21_treatment_trace, _vis21_fork_generation, true)
...
func _render_vis21_generation(generation: int) -> void:
	...
	var point := TraceAdapter.from_generation_map(generation, generation_map, TreatmentRunner.BRANCH_ID, _vis21_experiment_for_generation(generation), String(_vis21_treatment.sample_environment_for_generation(generation, 0.0, 0.0).get("environment_revision", "")))
...
func _update_vis18r_title() -> void:
	...
	if title == null:
		return
	title.text = "ECO.VIS2.1 — %s fork=G%d paired=G%d — %s %d%% — reps=%d" % ["PLAY" if _vis18r_playing else "PAUSE", _vis21_fork_generation, _vis18r_generation, _vis21_treatment_profile, int(round(_vis21_treatment_intensity * 100.0)), _vis18r_current_visual_count]
...
func _update_status() -> void:
	...
	if status == null:
		return
	if not _vis21_paired:
		status.text += "\nVIS2.1=READY treatment=%s %d%% | F forks the current BASELINE generation; 2/3/4/5 select treatment without altering pre-fork world." % [_vis21_treatment_profile, int(round(_vis21_treatment_intensity * 100.0))]
		return
	var current := _vis21_point_for_generation(_vis21_comparison_summary.get("points", []), _vis18r_generation)
	status.text += "\nVIS2.1=PAIRED fork=G%d paired=G%d treatment=%s %d%% root=%s" % [_vis21_fork_generation, _vis18r_generation, _vis21_treatment_profile, int(round(_vis21_treatment_intensity * 100.0)), _vis21_common_root.left(12)]
	if not current.is_empty():
		status.text += "\nCONTROL reps=%d fitness=%.3f genomes=%d | TREATMENT reps=%d fitness=%.3f genomes=%d" % [int(current.get("control_population", 0)), float(current.get("control_mean_fitness", 0.0)), int(current.get("control_unique_genomes", 0)), int(current.get("treatment_population", 0)), float(current.get("treatment_mean_fitness", 0.0)), int(current.get("treatment_unique_genomes", 0))]
		status.text += "\nDELTA population=%+d fitness=%+.4f genomes=%+d deaths=%+d alpha_share=%+.4f | CONTROL=data-only visible_fields=1 progressive_PH5=OFF" % [int(current.get("delta_population", 0)), float(current.get("delta_mean_fitness", 0.0)), int(current.get("delta_unique_genomes", 0)), int(current.get("delta_deaths", 0)), float(current.get("delta_alpha_share", 0.0))]
