...
Не добавлять без технической необходимости обратную зависимость `scripts/network -> simulator gameplay domain`.

Если алгоритму не нужно знать о конкретном gameplay domain, он должен оставаться generic.

## Core ownership rule

Network runtime может владеть identity, authority, epochs, operations, routing, handoff, replication, projection sequencing, recovery, transport abstraction, observability и fault handling.

Он не должен становиться владельцем Item Graph contents, inventory, Matter, Construction, Ecology или других gameplay semantics.

Не создавать второй gameplay truth, второй Item Graph, параллельную authority model или transport-specific fork canonical gameplay semantics.

## Generic contracts

Для новых canonical network contracts по возможности сохранять explicit schema/version, exact validation, deterministic serialization/hash, explicit authority epoch/state revision, replay-safe operation identity и fail-closed deterministic error codes.

Production topology/authority/routing data предпочтительно получать через configuration/provider/registry/directory, а не hardcode конкретного мира.
