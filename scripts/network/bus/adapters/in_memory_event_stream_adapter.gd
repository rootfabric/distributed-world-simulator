...
var _max_events_per_stream: int
var _streams: Dictionary = {}
var _event_fingerprints: Dictionary = {}


func _init(adapter_id: String = "adapter/in-memory-event-direct", max_events_per_stream: int = 1024) -> void:
	_adapter_id = adapter_id
	_max_events_per_stream = maxi(1, max_events_per_stream)


func get_descriptor() -> Dictionary:
	return DescriptorScript.create("EVENT_STREAM", _adapter_id, ["append_only", "canonical_round_trip", "idempotent_event_id", "monotonic_sequence"])
...
func publish(event_value: Dictionary) -> Dictionary:
	...
	if stream.size() >= _max_events_per_stream:
		return ResultScript.failure("BACKPRESSURE", "EVENT_STREAM_CAPACITY", true, {"stream_id": stream_id})
	...
	stream.append(copied.get("value"))
	_streams[stream_id] = stream
	_event_fingerprints[event_id] = fingerprint
	return ResultScript.success("ACCEPTED", {"duplicate": false, "sequence": expected_sequence})
