...
func send_to_peer(peer_id: String, frame: Dictionary) -> Dictionary:
	...
	_peer.transfer_mode = _transfer_mode(String(frame["delivery_mode"]))
	...
	return TransportUtilsScript.success({"enqueued": true, "packet_bytes": int(encoded.get("details", {}).get("packet_bytes", 0))})
...
func _channel_index(channel: String) -> int:
	match channel:
		"CONTROL", "COMMAND": return 0
		"STATE", "EVENT": return 1
		_: return 2


func _transfer_mode(delivery_mode: String) -> int:
	return MultiplayerPeer.TRANSFER_MODE_UNRELIABLE_ORDERED if delivery_mode == "UNRELIABLE_SEQUENCED" else MultiplayerPeer.TRANSFER_MODE_RELIABLE
