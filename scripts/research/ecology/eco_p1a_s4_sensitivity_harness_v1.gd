...
static func run(config: Dictionary = {}, grid_size: int = DEFAULT_GRID_SIZE, seasons: int = DEFAULT_SEASONS, seed: int = Fixture.DEFAULT_SEED) -> Dictionary:
	...
	var result := {
		"schema": SCHEMA, "version": VERSION, "experiment_revision": EXPERIMENT_REVISION,
		"seed": seed, "grid_size": grid_size, "seasons": seasons, "config": effective_config,
		"config_hash": config_hash(effective_config), "accepted_environment_hash": Fixture.environment_hash(Fixture.LOGICAL_GRID_SIZE, seed),
		"genome_checksum": String(genome["checksum"]), "patch_count": patch_count,
		"average_initial_net": total_initial_net / float(patch_count), "average_final_biomass": total_final_biomass / float(patch_count),
		"average_peak_biomass": total_peak_biomass / float(patch_count), "total_biomass_series": total_biomass_series,
		"total_biomass_series_hash": _float_series_hash(total_biomass_series), "viability_counts": viability_counts,
		"limiting_counts": limiting_counts, "zone_summary": zone_summary,
	}
	result["result_hash"] = _result_hash(result, record_tokens)
	return result

static func sensitivity_summary(config: Dictionary = {}, grid_size: int = DEFAULT_GRID_SIZE, seed: int = Fixture.DEFAULT_SEED) -> Dictionary:
	var effective_config := baseline_config() if config.is_empty() else config.duplicate(true)
	...
	var result := {"experiment_revision": EXPERIMENT_REVISION, "seed": seed, "grid_size": grid_size, "config": effective_config, "config_hash": config_hash(effective_config), "patch_count": patch_count, "average_initial_net": total_net / float(patch_count), "viability_counts": viability_counts, "limiting_counts": limiting_counts}
	result["summary_hash"] = "|".join(PackedStringArray([EXPERIMENT_REVISION, str(seed), str(grid_size), String(result["config_hash"]), _format_float(float(result["average_initial_net"])), "\n".join(tokens)])).sha256_text()
	return result

static func control_point(point_name: String, config: Dictionary = {}) -> Dictionary:
	if not Fixture.CONTROL_POINTS.has(point_name): return {}
	var effective_config := baseline_config() if config.is_empty() else config.duplicate(true)
	if not bool(validate_config(effective_config).get("success", false)): return {}
	var genome := _perturbed_genome(effective_config)
	...
	return {"point": point_name, "environment": environment, "genome": genome, "balance": balance, "simulation": simulation}

static func config_hash(config: Dictionary) -> String:
	var tokens := PackedStringArray([EXPERIMENT_REVISION])
	for key in CONFIG_KEYS: tokens.append("%s=%s" % [key, _format_float(float(config.get(key, 0.0)))])
...
static func _perturbed_environment(base: Dictionary, config: Dictionary) -> Dictionary:
	...
	return EnvironmentSample.create(float(base["world_x_m"]), float(base["world_z_m"]), float(base["temperature_c"]), moisture, sunlight, float(base["nutrients"]), float(base["flood_frequency"]), int(base["seed"]), revision)
...
static func _perturbed_genome(config: Dictionary) -> Dictionary:
	...
	return PlantGenome.create("%s/%s/%s" % [String(base["genome_id"]), EXPERIMENT_REVISION, config_hash(config).substr(0, 12)], float(base["height_m"]), float(base["growth_rate"]), root_depth, float(base["water_preference"]), float(base["water_tolerance_width"]), shade_tolerance, int(base["seed_count"]), float(base["seed_dispersal_distance_m"]), float(base["lifespan_years"]))

static func _simulate_experiment(environment: Dictionary, genome: Dictionary, seasons: int, config: Dictionary) -> Dictionary:
	# Full biomass dynamics stay delegated to the accepted S2 simulator.
	# Cost-coefficient sweeps are summary-only, so S4 never forks the S2 update kernel.
	if not _cost_scales_are_baseline(config): return {}
...
static func _cost_scales_are_baseline(config: Dictionary) -> bool:
	return absf(float(config["root_cost_scale"]) - 1.0) <= 0.000000001 and absf(float(config["maintenance_cost_scale"]) - 1.0) <= 0.000000001 and absf(float(config["flood_penalty_scale"]) - 1.0) <= 0.000000001
...
static func _apply_cost_overrides(balance: Dictionary, config: Dictionary) -> Dictionary:
	...
	var total_cost := float(adjusted["maintenance_cost"]) + float(adjusted["root_cost"]) + float(adjusted["structural_cost"]) + float(adjusted["growth_allocation_cost"]) + float(adjusted["reproduction_allocation_cost"]) + float(adjusted["water_stress_penalty"]) + float(adjusted["flood_penalty"]) + float(adjusted["density_cost"])
...
static func _dominant_limiting_factor(balance: Dictionary) -> String:
	var entries: Array = [["LIGHT", float(balance["light_limitation"])], ["WATER", float(balance["water_limitation"])], ["NUTRIENT", float(balance["nutrient_limitation"])], ["TEMPERATURE", float(balance["temperature_limitation"])], ["FLOOD", float(balance["flood_limitation"])]]
	var best: Array = Array(entries[0])
	for entry_value in entries:
		var entry: Array = Array(entry_value)
		if float(entry[1]) > float(best[1]): best = entry
	return String(best[0])

static func _viability_class(net: float) -> String:
	if net >= ResourceModel.FAVOURABLE_THRESHOLD: return "FAVOURABLE"
	if net >= ResourceModel.MARGINAL_THRESHOLD: return "MARGINAL"
	return "UNSUSTAINABLE"

static func _zone_id(x: float, z: float) -> String:
	if x < 0.0 and z < 0.0: return "SW"
	if x >= 0.0 and z < 0.0: return "SE"
	if x < 0.0 and z >= 0.0: return "NW"
	return "NE"

static func _float_series_hash(series: Array[float]) -> String:
	var tokens := PackedStringArray()
	for value in series: tokens.append(_format_float(value))
	return "\n".join(tokens).sha256_text()

static func _result_hash(result: Dictionary, record_tokens: PackedStringArray) -> String:
	var tokens := PackedStringArray([SCHEMA, VERSION, String(result["experiment_revision"]), str(int(result["seed"])), str(int(result["grid_size"])), str(int(result["seasons"])), String(result["config_hash"]), String(result["accepted_environment_hash"]), String(result["genome_checksum"]), _format_float(float(result["average_initial_net"])), _format_float(float(result["average_final_biomass"])), String(result["total_biomass_series_hash"]), "\n".join(record_tokens)])
	return "|".join(tokens).sha256_text()
