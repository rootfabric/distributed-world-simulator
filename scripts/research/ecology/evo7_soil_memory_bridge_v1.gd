...
static func run_all(lineage_seed := 20260823, source_cycles := 8, generations := 18, population_size := 18, offspring_per_parent := 4) -> Dictionary:
	...
	var result := {"schema":SCHEMA,"version":VERSION,"revision":REVISION,"lineage_seed":lineage_seed,"source_cycles":source_cycles,"source_removed":true,
		"modified_soil":modified,"pristine_soil":pristine,"common_first_candidate_pool_hash":String(modified_result["first_candidate_pool_hash"]),
		"modified":modified_result,"pristine":pristine_result,
...
static func _run_mode(ancestor: Dictionary, soil: Dictionary, lineage_seed: int, generations: int, population_size: int, offspring_per_parent: int, policy: Dictionary, mode: String) -> Dictionary:
	...
	return {"mode":mode,"soil_state_hash":String(soil["state_hash"]),"first_candidate_pool_hash":first_pool_hash,"final_population_hash":"|".join(checksums).sha256_text(),
		"mean_fitness":snappedf(fitness_sum / float(population.size()),1e-9),"mean_leaf_economics_proxy":snappedf(economics_sum / float(population.size()),1e-9),"mean_features":means}
...
static func _evaluate(bundle: Dictionary, soil: Dictionary, seed: int, generation: int, mode: String, parent_index: int, offspring_index: int) -> Dictionary:
	...
	var env := EnvSample.create(0.0,0.0,24.0,clampf(0.24 + retention * 0.90,0.0,1.0),0.88,clampf(0.20 + nutrient_bonus * 1.20,0.0,1.0),0.0,seed,
		"ECO.EVO7-FFF5|%s|g%d|p%d|o%d" % [mode,generation,parent_index,offspring_index])
	var fp := _functional(bundle,env,"fff5-eval|%s|%d|%d|%d" % [mode,generation,parent_index,offspring_index])
	if fp.is_empty(): return {}
...
static func _result_hash(result: Dictionary) -> String:
	return "|".join(PackedStringArray([SCHEMA,VERSION,REVISION,str(int(result["lineage_seed"])),str(int(result["source_cycles"])),String(result["modified_soil"]["state_hash"]),String(result["pristine_soil"]["state_hash"]),String(result["common_first_candidate_pool_hash"]),String(result["modified"]["final_population_hash"]),String(result["pristine"]["final_population_hash"])] )).sha256_text()