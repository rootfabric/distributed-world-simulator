class_name Fabric0ConservationFabricV1
extends RefCounted
...
static func solve(network: Dictionary) -> Dictionary:
	...
	for island in islands:
		var result := _solve_island(network, cell_map, island)
...
static func read_port_state(
	...
	return element["state"]["ports"].get(port_name, {}).duplicate(true)
...
static func cell_for_port(network: Dictionary, element_id: String, port_name: String) -> Dictionary:
	var target := _port_ref(element_id, port_name)
...
static func _solve_island(
	network: Dictionary,
	cell_map: Dictionary,
	island: Array
) -> Dictionary:
	var domain := String(network["cells"][island[0]]["domain"])
	for cell_index in island:
		if String(network["cells"][cell_index]["domain"]) != domain:
			var mixed := {"ok": false, "code": "MIXED_DOMAIN_ISLAND", "cells": _cell_ids(network, island)}
			_mark_island_failed(network, island, "MIXED_DOMAIN_ISLAND")
			network["diagnostics"].append(mixed)
			return mixed
	...
	for element_id in element_ids:
		...
		if global_cells.size() != port_names.size():
			var split := {
				"ok": false,
				"code": "ELEMENT_SPLIT_ACROSS_SOLVE_ISLAND",
				"element_id": element_id,
			}
			_mark_island_failed(network, island, "ELEMENT_SPLIT_ACROSS_SOLVE_ISLAND")
		...
		match op:
			...
			"ideal_common_constraint":
				...
				constraints.append({
					"cell": cell,
					"global_cell": int(global_cells[0]),
			...
			_:
				...
				_mark_island_failed(network, island, "UNKNOWN_PHYSICAL_LAW")
				network["diagnostics"].append(unknown)
				return unknown

	var constraint_check := _validate_constraints(network, island, constraints)
	...
	for constraint_index in range(constraints.size()):
		var row := size + constraint_index
		var cell := int(constraints[constraint_index]["cell"])
		augmented[cell][row] = 1.0
		augmented[row][cell] = 1.0
		augmented_rhs[row] = float(constraints[constraint_index]["value"])

	var solved := _solve_dense(augmented, augmented_rhs)
	if not bool(solved.get("ok", false)):
		var singular := {
			"ok": false,
			"code": "SINGULAR_FLOATING_ISLAND",
			"domain": domain,
			"cells": _cell_ids(network, island),
		}
		_mark_island_failed(network, island, "SINGULAR_FLOATING_ISLAND")
	...
	for constraint_index in range(constraints.size()):
		...
		ideal_flows[String(constraint["element_id"])] = -float(solution[size + constraint_index])

	_apply_port_solution(network, cell_map, island, ideal_flows)
	_compute_cell_residuals(network, cell_map, island)

	return {"ok": true, "domain": domain, "cells": _cell_ids(network, island)}

static func _validate_constraints(
	network: Dictionary,
	island: Array,
	constraints: Array
) -> Dictionary:
	...
	for constraint in constraints:
		var global_cell := int(constraint["global_cell"])
		...
		for item in items:
			...
			if absf(float(item["value"]) - first_value) > EPSILON:
				return {
					...
					"cell_id": String(network["cells"][global_cell]["id"]),
					"elements": element_ids,
				}
		return {
			"ok": false,
			"code": "REDUNDANT_IDEAL_CONSTRAINT",
			"cell_id": String(network["cells"][global_cell]["id"]),
			"elements": element_ids,
		}
	return {"ok": true}
	...
	for element_id in element_ids:
		...
		match op:
			...
			"fixed_balance_terminal":
				var cell := int(cell_map[_port_ref(element_id, "p")])
				var common := float(network["cells"][cell]["common"])
				_set_port_state(element, "p", common, float(element["law"]["balance"]))
			"ideal_common_constraint":
				var cell := int(cell_map[_port_ref(element_id, "p")])
				var common := float(network["cells"][cell]["common"])
				_set_port_state(element, "p", common, float(ideal_flows.get(element_id, 0.0)))
			"linear_difference_coupler":
...
static func _compute_cell_residuals(
	network: Dictionary,
	cell_map: Dictionary,
	island: Array
) -> void:
...
static func _physical_element(
	...
	for port_name in port_names:
		ports[port_name] = {
			"direction": "physical",
			"domain": String(port_domains[port_name]),
		}
		port_states[port_name] = {
			"common": 0.0,
			"balance": 0.0,
			"power_into_cell": 0.0,
		}
