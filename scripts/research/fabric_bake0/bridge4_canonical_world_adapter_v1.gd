...
const BINDING_SCHEMA := "planet_simulator.fabric_bridge4_canonical_binding.v1"
...
static func compile(snapshot: Dictionary, matter_batch: Dictionary, execution_owner: String, authority_epoch: int, applied_event_ids: Array = []) -> Dictionary:
	...
	if not Utils.is_canonical_id(execution_owner, 2) or not Utils.is_json_integer(authority_epoch) or authority_epoch < 1:
		return Utils.failure("BRIDGE4_AUTHORITY_INVALID")
	...
	if boundaries.size() < 2 or boundaries.size() > 8 or internals.size() < 100:
	...
	var matter := SourceRevision.create(
		"MATTER", String(matter_batch["batch_id"]), authority_epoch, 0,
	...
	var construction_key := Utils.source_key("CONSTRUCTION", String(snapshot["construct_id"]))
	var matter_key := Utils.source_key("MATTER", String(matter_batch["batch_id"]))
	var authority := AuthorityEnvelope.create(
		execution_owner,
		[
			{"source_domain": "CONSTRUCTION", "source_id": snapshot["construct_id"], "authority_epoch": authority_epoch, "owner_id": execution_owner},
			{"source_domain": "MATTER", "source_id": matter_batch["batch_id"], "authority_epoch": authority_epoch, "owner_id": execution_owner},
		],
		[construction_key],
		[matter_key]
	)
	if authority.is_empty() or not bool(AuthorityEnvelope.validate_b0_safety(authority).get("success", false)):
		return Utils.failure("BRIDGE4_AUTHORITY_ENVELOPE_INVALID")
	var binding := {
...
static func binding_matches(binding: Dictionary, snapshot: Dictionary, matter_batch: Dictionary, execution_owner: String, authority_epoch: int, applied_event_ids: Array = []) -> Dictionary:
	...
	if String(binding.get("checksum", "")) != String(actual.get("checksum", "")):
...
static func validate_successor(previous: Dictionary, successor: Dictionary, matter_before: Dictionary, matter_after: Dictionary, failed_bond_id: String) -> Dictionary:
	...
	if previous["root_item_instance_id"] != successor["root_item_instance_id"] or previous["parts"] != successor["parts"] or previous["compiled_facets"] != successor["compiled_facets"]:
