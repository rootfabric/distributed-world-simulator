...
static func build(include_power: bool = true) -> Dictionary:
	...
	var observation := {
		...
		"canonical_frontier_hash_before": String(subject["frontier"]["frontier_hash"]),
		"canonical_frontier_hash_after": String(break_bundle["current_frontier"]["frontier_hash"]),
		"construction_revision_before": int(subject["construction"]["source_revision"]),
		"construction_revision_after": int(break_bundle["current_construction"]["source_revision"]),
		"canonical_execution_owner": String(subject["authority"]["execution_owner"]),
