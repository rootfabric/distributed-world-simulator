...
static func compile(sources: Dictionary, authority: Dictionary) -> Dictionary:
	if not U.validate_exact_fields(sources, SOURCE_FIELDS).success:
		return U.failure("R3_SOURCE_FIELDS")
	for key in SOURCE_FIELDS:
		if not sources[key] is Dictionary:
			return U.failure("R3_SOURCE_TYPE")
	for key in ["mechanical", "electrical"]:
		if sources[key].get("parts") is Array and sources[key].parts.size() > MAX_ELEMENTS + 1:
			return U.failure("R3_TOPOLOGY_BUDGET")
		if sources[key].get("bonds") is Array and sources[key].bonds.size() > MAX_ELEMENTS:
			return U.failure("R3_TOPOLOGY_BUDGET")
	...
	if not A.validate_b0_safety(authority).success:
		return U.failure("R3_AUTHORITY_INVALID")
	var mutable: Array = []
	var readonly: Array = []
	for key in ["mechanical", "electrical"]:
		if not sources[key].build_state in ["OPERATIONAL", "DAMAGED"]:
			return U.failure("R3_SOURCE_NOT_EXECUTABLE")
	...
	mutable.sort()
	readonly.sort()
	if mutable[0] == mutable[1] or readonly[0] == readonly[1] or authority.mutable_source_ids != mutable or authority.readonly_source_ids != readonly:
		return U.failure("R3_AUTHORITY_SOURCE_COVERAGE")
	var controls = sources.mechanical.compiled_facets.get("composition_r3")
	if not controls is Dictionary or not U.validate_exact_fields(controls, CONTROL_FIELDS).success:
		return U.failure("R3_CONTROLS_REQUIRED")
	for key in CONTROL_FIELDS:
		if not U.is_finite_number(controls[key]) or absf(float(controls[key])) > 1.0e6:
			return U.failure("R3_CONTROL_ENVELOPE", {"field": key})
	if controls.coupling_n_per_a <= 0.0 or controls.guard_fraction <= 0.0 or controls.guard_fraction >= 1.0:
		return U.failure("R3_COUPLER_OR_GUARD_INVALID")
	var m: Dictionary = mech.details.model
	var e: Dictionary = elec.details.model
	if m.elements.size() > MAX_ELEMENTS or e.elements.size() > MAX_ELEMENTS:
		return U.failure("R3_TOPOLOGY_BUDGET")
	var slider := {}
	var nodes := {}
	for node in m.nodes:
	...
	for element in m.elements:
		if element.node_a != slider.node_id and element.node_b != slider.node_id:
			return U.failure("R3_PARALLEL_SUPPORTS_REQUIRED")
		...
		# Same-side collinear anchors give an ID-independent oriented scalar port.
		if axis == Vector3.ZERO: axis = direction
		if not nodes[anchor_id].anchored or direction.dot(axis) < 1.0 - 1.0e-10:
			return U.failure("R3_COLLINEAR_SAME_SIDE_ANCHORS_REQUIRED")
		covered[anchor_id] = true
		if "|" in element.element_id: return U.failure("R3_ELEMENT_ID_DELIMITER")
		if not U.is_positive_number(element.capacity_n) or element.capacity_n < MIN_CAPACITY_N:
			return U.failure("R3_CAPACITY_BELOW_EVENT_RESOLUTION")
		if element.active:
	...
	if not is_finite(rate) or mass < 1.0e-6 or resistance < 1.0e-9:
		return U.failure("R3_NUMERIC_ENVELOPE")
	var max_step := minf(0.02, 0.1 / maxf(rate, 1.0))
	if max_step < 1.0e-7: return U.failure("R3_TIMESCALE_OUT_OF_SCOPE")
	var model := {
		"schema": SCHEMA, "mechanical_model": m, "electrical_model": e,
		"source_hash": U.canonical_hash(sources), "authority_hash": authority.checksum,
		"controls": controls.duplicate(true), "slider_id": slider.node_id,
		"axis": [axis.x, axis.y, axis.z], "mass_kg": mass,
		"stiffness_n_per_m": stiffness, "damping_ns_per_m": damping,
		"resistance_ohm": resistance, "load_id": series.details.load_id,
		"max_step_s": max_step,
...
static func _series(model: Dictionary, snapshot: Dictionary) -> Dictionary:
	...
	if ends != 2 or seen.size() != adjacency.size() or not U.is_positive_number(total):
		return U.failure("R3_CONNECTED_SERIES_PATH_REQUIRED")
	return U.success({"resistance_ohm": total, "load_id": load_id})

# Both representations compile into the unchanged, dimension-checking FABRIC0 DSL.
static func system(model: Dictionary, fidelity: String, values: Dictionary = {}, time_s: float = 0.0, mode: String = "LIVE") -> Dictionary:
	if not U.validate_checksum(model).success or model.get("schema") != SCHEMA:
		return U.failure("R3_MODEL_CHECKSUM")
	if not fidelity in ["FULL", "BAKE"] or not mode in ["LIVE", "REFINED", "WAIT_CANONICAL"]:
		return U.failure("R3_MODE_INVALID")
	var s := D.new_system()
	var controls: Dictionary = model.controls
	var parameters := {
		"m": [model.mass_kg, D.dim_mass()],
		"k": [model.stiffness_n_per_m, D.dim_div(D.dim_force(), D.dim_length())],
		"c": [model.damping_ns_per_m, D.dim_div(D.dim_force(), D.dim_velocity())],
		"r": [model.resistance_ohm, P.dim_div(P.dim_voltage(), P.dim_current())],
		"g": [controls.coupling_n_per_a, P.dim_div(P.dim_force(), P.dim_current())],
		"u": [controls.source_voltage_v, P.dim_voltage()], "f": [controls.external_force_n, D.dim_force()],
	}
	...
	var flows := {
		"x": v, "v": D.expr_div(force, D.expr_parameter("m")),
		"source_work": D.expr_mul(D.expr_parameter("u"), current),
		"external_work": D.expr_mul(D.expr_parameter("f"), v),
		"joule_heat": D.expr_mul(D.expr_parameter("r"), D.expr_pow_int(current, 2)),
		"damper_heat": D.expr_mul(D.expr_parameter("c"), D.expr_pow_int(v, 2)),
	}
	...
	for element in model.mechanical_model.elements:
		...
		for sign_value in [-1.0, 1.0]:
			for kind in ["guard", "failure"]:
				...
				var transition := {
					"id": "%s|%s|%s" % [kind, element.element_id, str(sign_value)],
					"from_modes": ["LIVE"] if kind == "guard" else ["LIVE", "REFINED"],
					"to_mode": "REFINED" if kind == "guard" else "WAIT_CANONICAL", "priority": 0,
					"guard": {"expr": D.expr_sub(D.expr_mul(D.expr_constant(sign_value), effort), D.expr_constant(limit, D.dim_force())), "nominal": limit, "direction": 1, "kind": "crossing"},
				}
