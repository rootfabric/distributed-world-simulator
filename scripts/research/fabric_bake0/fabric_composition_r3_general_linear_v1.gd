...
static func compile_model(sources: Dictionary, mechanical_model: Dictionary, electrical_model: Dictionary, controls: Dictionary, source_hash: String, authority_hash: String) -> Dictionary:
	var facets = sources.electrical.compiled_facets.get(BOUNDARY_FACET)
	if not facets is Dictionary or facets.is_empty(): return U.failure("R3_GENERAL_BOUNDARY_VOLTAGES_REQUIRED")
	var boundaries: Dictionary = facets.duplicate(true)
	if boundaries.size() < 2 or boundaries.size() > MAX_NODES: return U.failure("R3_GENERAL_BOUNDARY_COUNT")
	var electrical_ids := {}
	for node in electrical_model.nodes: electrical_ids[str(node.node_id)] = true
	for raw_id in boundaries:
		if typeof(raw_id) != TYPE_STRING or not electrical_ids.has(str(raw_id)) or not U.is_finite_number(boundaries[raw_id]): return U.failure("R3_GENERAL_BOUNDARY_INVALID")
		boundaries[raw_id] = float(boundaries[raw_id])
	var mechanical := Mechanics.compile_mechanics(mechanical_model, controls)
	if not mechanical.success: return mechanical
	var roles := _electrical_roles(sources.electrical)
	if not roles.success: return roles
	...
	if source_edge.is_empty(): return U.failure("R3_GENERAL_SOURCE_EDGE_MISSING")
	var source_candidates: Array[String] = []
	for endpoint in [str(source_edge.node_a), str(source_edge.node_b)]:
		if boundaries.has(endpoint): source_candidates.append(endpoint)
	if source_candidates.size() != 1: return U.failure("R3_GENERAL_SOURCE_BOUNDARY_REQUIRED")
	var source_port_id: String = source_candidates[0]
	# G2 boundary potentials are canonical electrical source data. Do not infer,
	# collapse, shift or overwrite any independent Dirichlet port from the legacy
	# scalar source_voltage_v control. Generalized voltage mutation requires a
	# canonical electrical-boundary command rather than a hidden compile rewrite.
	var nominal := Graph.solve_resistive(electrical_model, boundaries)
	if not nominal.success: return nominal
	...
	if not is_finite(di_dv) or di_dv > RESIDUAL_REL_TOL * (1.0 + absf(i0)): return U.failure("R3_GENERAL_NONPASSIVE_SOURCE_RESPONSE")
	...
	if absf(power_curvature) > RESIDUAL_REL_TOL * (1.0 + absf(p0) + absf(p_plus) + absf(p_minus)): return U.failure("R3_GENERAL_EXTERNAL_POWER_NONLINEAR")
	...
	for velocity in [-1.0, 0.0, 1.0]:
		...
		if absf(residual) > 2.0e-9 * scale: return U.failure("R3_GENERAL_COUPLER_POWER_IDENTITY")
		if float(graph.details.power_residual_w) > 2.0e-9 * (1.0 + absf(heat)): return U.failure("R3_GENERAL_GRAPH_POWER_RESIDUAL")
	...
	var model := {
		"schema": "planet_simulator.fabric_composition_r3.v1", "solver_kind": SOLVER_KIND,
		"mechanical_model": mechanical_model.duplicate(true), "electrical_model": electrical_model.duplicate(true),
		"source_hash": source_hash, "authority_hash": authority_hash, "controls": controls.duplicate(true),
		"axis": mechanics.axis.duplicate(), "mobile_nodes": mechanics.mobile_nodes.duplicate(),
		"mobile_masses_kg": mechanics.mobile_masses_kg.duplicate(), "mobile_index": mechanics.mobile_index.duplicate(true),
		"coupler_node_id": mechanics.coupler_node_id, "boundary_voltages_v": boundaries.duplicate(true),
		"source_port_id": source_port_id, "source_edge_id": source_edge_id, "load_id": roles.details.load_id,
		"source_current_offset_a": i0, "source_current_velocity_gain": di_dv,
		"external_power_offset_w": p0, "external_power_velocity_gain": power_linear,
		"joule_power_offset_w": h0, "joule_power_velocity_gain": heat_linear, "joule_power_velocity2_gain": heat_quadratic,
		"max_step_s": max_step, "bake_kind": "EXACT_ALGEBRAIC_ELIMINATION", "dynamic_state_reduction": false, "checksum": "",
	}
...
static func _compile_mechanics(model: Dictionary, controls: Dictionary) -> Dictionary: return Mechanics.compile_mechanics(model, controls)
static func solve_resistive(model: Dictionary, boundary_voltages_v: Dictionary) -> Dictionary: return Graph.solve_resistive(model, boundary_voltages_v)
static func solve_mechanical_static(model: Dictionary, coupler_node_id: String) -> Dictionary: return Mechanics.solve_mechanical_static(model, coupler_node_id)
static func system(model: Dictionary, fidelity: String, values: Dictionary = {}, time_s: float = 0.0, mode: String = "LIVE") -> Dictionary: return System.system(model, fidelity, values, time_s, mode)
static func observe(model: Dictionary, system_value: Dictionary, fidelity: String, fracture_j: float = 0.0) -> Dictionary: return System.observe(model, system_value, fidelity, fracture_j)
static func element_elastic_energy(model: Dictionary, system_value: Dictionary, element_id: String) -> float: return System.element_elastic_energy(model, system_value, element_id)
