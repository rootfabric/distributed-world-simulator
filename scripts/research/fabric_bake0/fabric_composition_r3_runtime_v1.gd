...
func fence(sources: Dictionary, authority: Dictionary) -> Dictionary:
	if _model.is_empty() or U.canonical_hash(_model) != _model_hash:
		return U.failure("R3_MODEL_INVALID")
	if U.canonical_hash(sources) != _model.source_hash or not C.A.validate_b0_safety(authority).success or authority.checksum != _model.authority_hash:
		return U.failure("R3_SOURCE_OR_AUTHORITY_STALE")
	return U.success()
...
func set_fidelity(target: String, sources: Dictionary, authority: Dictionary, artifact: Dictionary = {}) -> Dictionary:
	...
	if target == "BAKE" and _utilization() >= _model.controls.guard_fraction:
		return U.failure("R3_GUARD_REQUIRES_FULL")
	var expected := _make_certificate()
	if not artifact.is_empty() and (not U.validate_checksum(artifact).success or U.canonical_hash(artifact) != U.canonical_hash(expected)):
		return U.failure("R3_BAKE_ARTIFACT_STALE_OR_TAMPERED")
...
func advance(dt: float, sources: Dictionary, authority: Dictionary) -> Dictionary:
	var checked := fence(sources, authority)
	if not checked.success: return checked
	if not U.is_positive_number(dt) or dt < 1.0e-7 or dt > _model.max_step_s:
		return U.failure("R3_TIMESTEP_OUTSIDE_ENVELOPE", {"max_step_s": _model.max_step_s})
	if not _pending.is_empty(): return U.failure("R3_WAITING_CANONICAL_COMMIT")
	var candidate: Dictionary = _system.duplicate(true)
	candidate.events = []
	# FABRIC0's public advance handles crossings, not conditions at the initial
	# instant. Reuse its generic condition/event engine for a post-commit overload.
	...
	for instant in events:
		for transition in instant.transitions:
			...
			if token.begins_with("failure|"):
				...
				pending = {
					"bond_id": token.get_slice("|", 1), "time_s": instant.time,
					"source_hash": _model.source_hash, "authority_hash": _model.authority_hash,
					"event_id": "event/r3-" + U.canonical_hash([_model.source_hash, instant.time, token]),
				}
	...
	for instant in events:
		for transition in instant.transitions:
			_events.append({"time_s": instant.time, "transition": transition.transition_id, "source_hash": _model.source_hash})
	return U.success({"snapshot": inspect()})

# Prepare an observer for a validated canonical successor. This method cannot
# write ConstructionStore; the caller publishes it only after an atomic commit.
func prepare_successor(previous: Dictionary, successor: Dictionary, authority: Dictionary, kind: String) -> Dictionary:
	...
	if kind == "failure":
		...
		for element in _model.mechanical_model.elements:
			if element.element_id == _pending.bond_id:
				released = 0.5 * float(element.stiffness_n_per_m) * pow(_value("x"), 2)
	elif kind == "input":
		...
		for field in ["source_voltage_v", "external_force_n"]:
			after.compiled_facets.composition_r3[field] = before.compiled_facets.composition_r3[field]
		...
		for i in range(before.bonds.size()):
			if before.bonds[i].bond_id == _model.load_id:
				after.bonds[i].metadata.area_m2 = before.bonds[i].metadata.area_m2
		checked = _only_revision_changed(before, after)
		if not checked.success: return checked
...
func _adopt_prepared(prepared: Dictionary) -> void:
	# Called only by the canonical bridge after it has validated and staged both sides.
...
func inspect() -> Dictionary:
	...
	var supports: Array = []
	for element in _model.mechanical_model.elements:
		supports.append({"bond_id": element.element_id, "active": element.active, "effort_n": (float(element.stiffness_n_per_m) * x + float(element.damping_ns_per_m) * v) if element.active else 0.0, "capacity_n": element.capacity_n})
	return {
		"time_s": _system.time, "displacement_m": x, "velocity_m_per_s": v,
		"current_a": current, "back_emf_v": g * v, "actuator_force_n": g * current,
		"constraint_residual_v": float(_model.resistance_ohm) * current + g * v - float(_model.controls.source_voltage_v),
		"source_power_w": float(_model.controls.source_voltage_v) * current,
		"coupler_electrical_power_w": g * v * current, "coupler_mechanical_power_w": g * current * v,
		"kinetic_j": kinetic, "elastic_j": elastic, "fracture_heat_j": _fracture_j,
		"source_work_j": _value("source_work"), "external_work_j": _value("external_work"),
		"joule_heat_j": _value("joule_heat"), "damper_heat_j": _value("damper_heat"),
		"energy_residual_j": kinetic + elastic + _fracture_j + _value("joule_heat") + _value("damper_heat") - _value("source_work") - _value("external_work"),
		"coupler_power_residual_w": g * v * current - g * current * v,
		"supports": supports, "fidelity": _fidelity, "mode": _system.mode,
		"pending_proposal": _pending.duplicate(true), "events": _events.duplicate(true),
		"source_hash": _model.source_hash, "authority_hash": _model.authority_hash,
		"mechanical_revision": _model.mechanical_model.construct_revision,
		"electrical_revision": _model.electrical_model.construct_revision,
		"max_step_s": _model.max_step_s, "guard_fraction": _model.controls.guard_fraction,
		"utilization": _utilization(), "model_hash": _model_hash,
		"axis": _model.axis.duplicate(), "certificate": _certificate.duplicate(true),
	}

func _make_certificate() -> Dictionary:
	var value := {"schema": "planet_simulator.fabric_composition_r3_elimination_certificate.v1", "source_hash": _model.source_hash, "authority_hash": _model.authority_hash, "model_hash": _model_hash, "eliminated_algebraics": 1, "dynamic_states_before": 2, "dynamic_states_after": 2, "energy_accounting_states": 4, "identity": "i=(u-g*v)/r; g*i*v=g*v*i", "checksum": ""}
...
func _utilization() -> float:
	...
	for element in _model.mechanical_model.elements:
		if element.active:
			result = maxf(result, absf(float(element.stiffness_n_per_m) * _value("x") + float(element.damping_ns_per_m) * _value("v")) / float(element.capacity_n))
	return result
