...
func setup(config: Dictionary) -> Dictionary:
	...
	_movement_service = MovementService.new()
...
func _handle_player_move(command: Dictionary, payload: Dictionary) -> Dictionary:
	...
	if typeof(delta_seconds_value) not in [TYPE_INT, TYPE_FLOAT]:
	...
	var movement_result: Dictionary = _movement_service.apply_authoritative_state(
		_player_state,
		Dictionary(candidate_value),
		float(delta_seconds_value)
	)
	if not bool(movement_result.get("success", false)):
		return _reject(command, String(movement_result.get("error_code", "PLAYER_MOVE_REJECTED")))
	var candidate: Dictionary = movement_result.get("details", {}).get("player_state", {}).duplicate(true)
	var input_sequence: int = int(candidate.get("last_input_sequence", 0))
