...
static func validate_player_record(record: Dictionary) -> Dictionary:
	...
	var transport_session_id := String(record.get("transport_session_id", ""))
	if connected and not transport_session_id.begins_with("transport-session/"): return Wire.failure("INVALID_MULTIPLAYER_TRANSPORT_SESSION")
	if not connected and not transport_session_id.is_empty() and not transport_session_id.begins_with("transport-session/"): return Wire.failure("INVALID_MULTIPLAYER_TRANSPORT_SESSION")
