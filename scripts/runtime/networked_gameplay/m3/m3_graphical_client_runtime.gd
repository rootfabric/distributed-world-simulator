...
func execute_item_command_blocking(
	...
	if not _send("ITEM_COMMAND", {"logical_player_id":_logical_player_id,"ownership_epoch":command_epoch,"operation_id":op,"command_type":command_type,"payload":payload.duplicate(true)}):
