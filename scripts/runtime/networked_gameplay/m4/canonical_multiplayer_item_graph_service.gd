...
# Composition correction for multiplayer player materialization and slot-aware
# transfers. The accepted base keeps inventory/container membership in compact
# arrays, while the graphical inventory is slot based. Keep those arrays for
# durable/backward compatibility and make location.slot_index the authoritative
# slot identity used by M7/NX6.
...
func _transfer(
	...
	if normalized_target.begins_with("container/"):
		return _transfer_to_container_slot(
			player_id,
			item_id,
			amount,
			normalized_target,
...
func _transfer_to_inventory_slot(
	player_id: String,
	item_id: String,
	amount: int,
	target_slot_index: int
	...
	if resolved_slot < 0:
		resolved_slot = _first_free_inventory_slot(
			player_id,
			item_id if whole_move and source_is_same_inventory and not source_hotbar_assigned else ""
		)
	...
	var exclude_source := (
		item_id
		if whole_move and source_is_same_inventory and not source_hotbar_assigned
		else ""
	)
	...
	if (
		whole_move
		and source_is_same_inventory
		and not source_hotbar_assigned
		and int(location.get("slot_index", -1)) == resolved_slot
	):
		return _success({
			"item_id": item_id,
...
func _transfer_to_container_slot(
	player_id: String,
	item_id: String,
	amount: int,
	container_id: String,
	...
	var source_quantity := int(item.get("quantity", 1))
	var whole_move := amount == source_quantity
	var source_is_same_container := (
		String(location.get("kind", "")) == "CONTAINER"
		and String(location.get("container_id", "")) == container_id
	)
	var resolved_slot := target_slot_index
	if resolved_slot < 0:
		resolved_slot = _first_free_container_slot(
			container_id,
			item_id if whole_move and source_is_same_container else ""
		)
...
func _normalize_player_inventory_slots(player_id: String) -> void:
	...
	for item_id_value in membership:
		...
		if (
			...
			or String(location.get("player_id", "")) != player_id
		):
			continue
