...
const SCHEMA := "planet_simulator.container_interaction_service.v1"
const SUPPORTED_COMMANDS: Array[String] = [
	"container.open",
	"container.close",
...
func setup(item_graph_service_reference) -> Dictionary:
	if _configured:
		return _failure("CONTAINER_INTERACTION_SERVICE_ALREADY_CONFIGURED")
...
func handle_command(command: Dictionary) -> Dictionary:
	if not _configured:
		return _failure("CONTAINER_INTERACTION_SERVICE_NOT_CONFIGURED")
	if not supports(String(command.get("command_type", ""))):
		return _failure("UNSUPPORTED_CONTAINER_COMMAND")
	_commands_routed += 1
