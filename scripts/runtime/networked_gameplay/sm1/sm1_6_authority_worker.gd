...
func _standby_sync(request_id: String, payload: Dictionary) -> void:
	...
	if String(candidate.get("product_session_id", "")) != Support.PRODUCT_SESSION_ID 			or String(candidate.get("logical_player_id", "")) != Support.LOGICAL_PLAYER_ID 			or String(candidate.get("player_entity_id", "")) != Support.PLAYER_ENTITY_ID 			or int(candidate.get("spawn_generation", 0)) != 1:
...
func _warm_load(request_id: String, payload: Dictionary) -> void:
	...
	if not incoming is Dictionary:
		...
		return
	var candidate: Dictionary = Dictionary(incoming).duplicate(true)
	if String(candidate.get("logical_player_id", "")) != Support.LOGICAL_PLAYER_ID 			or String(candidate.get("player_entity_id", "")) != Support.PLAYER_ENTITY_ID 			or int(candidate.get("spawn_generation", 0)) != 1:
