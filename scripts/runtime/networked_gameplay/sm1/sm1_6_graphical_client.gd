...
const COMMANDS := [
	{"operation_id": "operation/sm1/graphical/1", "input_sequence": 1, "command_kind": "ACTION", "delta_x": 0.0},
	{"operation_id": "operation/sm1/graphical/2", "input_sequence": 2, "command_kind": "MOVE", "delta_x": 11.0},
	{"operation_id": "operation/sm1/graphical/3", "input_sequence": 3, "command_kind": "ACTION", "delta_x": 0.0},
	{"operation_id": "operation/sm1/graphical/4", "input_sequence": 4, "command_kind": "MOVE", "delta_x": -11.0},
	{"operation_id": "operation/sm1/graphical/5", "input_sequence": 5, "command_kind": "ACTION", "delta_x": 0.0},
]
...
func _initialize() -> void:
	...
	var connected: Dictionary = _boundary.connect_client(
		...
		_peer_id,
		"transport-session/sm1/graphical-client-%s" % _client_id,
		"route/sm1/graphical-client-%s" % _client_id,
		1)
...
func _handle_state(payload: Dictionary) -> void:
	...
	var authority_id := String(payload.get("active_authority_id", ""))
	if authority_id != _last_authority:
...
func _send_next_command() -> void:
	...
	var sent := Support.send(_boundary, _peer_id, {
		"type": "EXECUTE",
		"request_id": "client-a/request/%d" % (_command_index + 1),
...
func _finish_success(payload: Dictionary) -> void:
	...
	var expected_revisions := [1, 2, 3, 4, 5]
	...
	var passed := _connect_count == 1 \
		...
		and int(payload.get("world_revision", 0)) == 5
	if _client_id == "a":
	...
	var report := {
		"schema": "planet_simulator.sm1_graphical_client_report.v1",
	...
	_finished = true
	_boundary.stop()
	print("SM1_6_GRAPHICAL_CLIENT_COMPLETE id=%s passed=%s" % [_client_id, str(passed)])
...
func _finish_failure(error_code: String, details: Dictionary) -> void:
	...
	if _boundary != null:
		_boundary.stop()
	push_error("SM1.6 graphical client failed: %s" % error_code)
	quit(1)
