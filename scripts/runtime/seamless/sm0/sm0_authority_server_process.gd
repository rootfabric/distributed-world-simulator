...
func _init() -> void:
	...
	print("[SM0_BOOT] authority=%s zone=%s gameplay_port=%s control_port=%s peer_control_port=%s fault_profile=%s" % [
		authority_id,
		zone_id,
	...
	if not bool(result.get("success", false)):
		...
		quit(2)
		return
	print("[SM0_BOOT] setup_success authority=%s fault_profile=%s" % [
		authority_id,
		fault_profile if not fault_profile.is_empty() else "none",
	])
