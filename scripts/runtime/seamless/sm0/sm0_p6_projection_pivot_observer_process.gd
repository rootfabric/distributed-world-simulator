...
func _init() -> void:
	...
	observer.name = "Sm0P6ProjectionPivotObserver"
	...
	var result: Dictionary = observer.setup({
		...
		"listen_port": int(options.get("listen-port", "26100")),
		"stop_file": String(options.get("stop-file", "")),
	})
	if not bool(result.get("success", false)):
		print("[SM0_P6_OBSERVER_BOOT] setup_failed=%s" % JSON.stringify(result, "", false, true))
		quit(2)
		return
	print("[SM0_P6_OBSERVER_BOOT] setup_success viewer=%s" % viewer_authority_id)
