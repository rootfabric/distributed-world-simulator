...
func _init() -> void:
	...
	var result: Dictionary = observer.setup({
		...
		"visual_response_hz": float(options.get("visual-response-hz", "14.0")),
	})
	if not bool(result.get("success", false)):
		print("[SM0_P8_VISUAL_BOOT] setup_failed=%s" % JSON.stringify(result, "", false, true))
		push_error("SM0 P8 observer setup failed: %s" % result)
		quit(2)
		return
	print("[SM0_P8_VISUAL_BOOT] setup_success listen_port=%s reference_frame=ship-root response_hz=%s" % [options.get("listen-port", "0"), options.get("visual-response-hz", "14.0")])
