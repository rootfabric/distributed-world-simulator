...
const FIELDS: Array[String] = ["schema", "inputs_hash", "rule_package_hash", "algorithm_version", "deterministic_seed", "from_tick", "to_tick", "fingerprint"]


static func create(inputs: Array, rule_package_hash: String, algorithm_version: String, deterministic_seed: int, from_tick: int, to_tick: int) -> Dictionary:
	var inputs_hash := NetworkUtilsScript.payload_hash(inputs)
	var value := {
		"schema": SCHEMA,
...
static func validate(value: Dictionary) -> Dictionary:
	...
	for field in ["inputs_hash", "rule_package_hash", "fingerprint"]:
