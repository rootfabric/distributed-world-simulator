...
const ACTIVE_FILE_NAME := "matter-cross-region-transactions.json"
const PREVIOUS_FILE_NAME := "matter-cross-region-transactions.previous.json"
const LOCK_DIRECTORY_NAME := ".matter-cross-region-transactions.lock"
...
func configure(root_path: String) -> Dictionary:
	...
	if normalized.is_empty():
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_REPOSITORY_PATH_REQUIRED")
	...
	if error != OK and error != ERR_ALREADY_EXISTS:
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_REPOSITORY_CREATE_FAILED", {"godot_error": error})
...
func save_atomic(checkpoint: Dictionary) -> Dictionary:
	if _active_path.is_empty():
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_REPOSITORY_NOT_CONFIGURED")
	...
	if FileAccess.file_exists(_active_path):
		...
		if not bool(current.get("success", false)):
			return MatterUtils.failure("CURRENT_MATTER_CROSS_REGION_TRANSACTION_CHECKPOINT_CORRUPTED", {"cause": current})
...
func prepare(checkpoint: Dictionary) -> Dictionary:
	...
	var pending_path: String = _root_path.path_join(
		".matter-cross-region-transactions.%d.%d.pending.json" % [OS.get_process_id(), Time.get_ticks_usec()]
	)
	var encoded: String = PersistenceCodec.encode_persistence_json(checkpoint)
	if encoded.is_empty():
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_CHECKPOINT_ENCODING_FAILED")
	var file := FileAccess.open(pending_path, FileAccess.WRITE)
	if file == null:
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_PENDING_OPEN_FAILED", {"godot_error": FileAccess.get_open_error()})
	...
	if write_error != OK:
		_remove_file(pending_path)
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_PENDING_WRITE_FAILED", {"godot_error": write_error})
	var verification: Dictionary = _read_checkpoint(pending_path)
	if not bool(verification.get("success", false)):
		_remove_file(pending_path)
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_PENDING_VERIFY_FAILED", {"cause": verification})
...
func commit_prepared(pending_path: String) -> Dictionary:
	...
	if normalized_pending.get_base_dir() != _root_path \
		or not normalized_pending.get_file().begins_with(".matter-cross-region-transactions.") \
		or not normalized_pending.get_file().ends_with(".pending.json"):
		return MatterUtils.failure("INVALID_MATTER_CROSS_REGION_TRANSACTION_PENDING_PATH")
...
func load_committed() -> Dictionary:
	...
	if FileAccess.file_exists(_previous_path):
		...
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_PREVIOUS_CHECKPOINT_CORRUPTED", {"cause": previous})
	if not active_error.is_empty():
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_CHECKPOINT_CORRUPTED", {"cause": active_error})
	return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_CHECKPOINT_NOT_FOUND", {"pending_files": pending_files})
...
func list_pending_files() -> Array[String]:
	...
	for file_name in directory.get_files():
		if file_name.begins_with(".matter-cross-region-transactions.") and file_name.ends_with(".pending.json"):
...
func _commit_prepared_locked(normalized_pending: String) -> Dictionary:
	...
	if not bool(pending.get("success", false)):
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_PENDING_INVALID", {"cause": pending})
	...
	if had_active:
		...
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_RECOVERY_REQUIRED")
	elif int(pending_checkpoint["generation"]) != 1:
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_INITIAL_GENERATION_REQUIRED")
	for _attempt in range(MAX_REPLACE_ATTEMPTS):
		...
		if replace_error == OK:
			...
			return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_COMMIT_VERIFY_FAILED", {"cause": active})
		if had_active and FileAccess.file_exists(_previous_path) and not FileAccess.file_exists(_active_path):
			DirAccess.rename_absolute(_previous_path, _active_path)
		OS.delay_msec(RETRY_DELAY_MS)
	return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_ATOMIC_REPLACE_FAILED")


func _repair_active_from_previous_locked() -> Dictionary:
	var previous: Dictionary = _read_checkpoint(_previous_path)
	if not bool(previous.get("success", false)):
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_PREVIOUS_UNAVAILABLE", {"cause": previous})
	var checkpoint: Dictionary = previous["details"]["checkpoint"]
	var encoded: String = PersistenceCodec.encode_persistence_json(checkpoint)
	if encoded.is_empty():
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_PREVIOUS_ENCODING_FAILED")
	var temporary: String = _root_path.path_join(".matter-cross-region-repair.%d.json" % Time.get_ticks_usec())
	var file := FileAccess.open(temporary, FileAccess.WRITE)
	if file == null:
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_REPAIR_OPEN_FAILED")
	...
	if write_error != OK:
		...
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_REPAIR_WRITE_FAILED", {"godot_error": write_error})
	_remove_file(_active_path)
	if DirAccess.rename_absolute(temporary, _active_path) != OK:
		_remove_file(temporary)
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_REPAIR_REPLACE_FAILED")
	var active: Dictionary = _read_checkpoint(_active_path)
	if not bool(active.get("success", false)) \
		or String(active["details"]["checkpoint"]["checksum"]) != String(checkpoint["checksum"]):
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_REPAIR_VERIFY_FAILED")
...
func _acquire_lock() -> Dictionary:
	...
	for attempt in range(MAX_LOCK_ATTEMPTS):
		...
		OS.delay_msec(RETRY_DELAY_MS)
	_remove_directory(candidate)
	return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_LOCK_TIMEOUT", {
...
func _release_lock(token: String) -> Dictionary:
	...
	# Keep release as one namespace operation. Removing owner.json while the
	# canonical lock directory still exists creates an ownerless race window.
...
func _read_checkpoint(path: String) -> Dictionary:
	if not FileAccess.file_exists(path):
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_CHECKPOINT_FILE_MISSING", {"path": path})
	var file := FileAccess.open(path, FileAccess.READ)
	if file == null:
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_CHECKPOINT_OPEN_FAILED", {"path": path})
	var encoded: String = file.get_as_text()
	file.close()
	var checkpoint: Dictionary = PersistenceCodec.decode_persistence_json(encoded)
	if checkpoint.is_empty():
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_CHECKPOINT_DECODE_FAILED", {"path": path})
	var checked: Dictionary = Checkpoint.validate(checkpoint)
	if not bool(checked.get("success", false)):
		return MatterUtils.failure("MATTER_CROSS_REGION_TRANSACTION_CHECKPOINT_INVALID", {"path": path, "cause": checked})
	return MatterUtils.success({"checkpoint": checkpoint})


func _pending_owner_pid(file_name: String) -> int:
	var prefix := ".matter-cross-region-transactions."
