...
const STABLE_KEY_PREFIX: String = "fluid-key/"
...
static func derive(
	...
	var type_slug: String = fluid_type_id.get_slice("/", fluid_type_id.get_slice_count("/") - 1)
	return GeoUtilsScript.success({
		"fluid_region_id": "%s%s/%s" % [PREFIX, type_slug, identity_hash],
