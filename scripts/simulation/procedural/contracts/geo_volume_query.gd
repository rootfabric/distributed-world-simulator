...
const SCHEMA: String = "planet_simulator.geo_volume_query.v1"
...
static func validate(value: Dictionary) -> Dictionary:
	...
	if typeof(value.get("schema")) != TYPE_STRING or String(value["schema"]) != SCHEMA:
		return GeoUtilsScript.failure("UNSUPPORTED_GEO_VOLUME_QUERY_SCHEMA")
	if not GeoUtilsScript.is_canonical_id(value.get("body_id"), 2):
		return GeoUtilsScript.failure("INVALID_GEO_VOLUME_QUERY_BODY_ID")
	...
	var safe: Dictionary = GeoUtilsScript.validate_json_safe(value, "$.geo_volume_query")
