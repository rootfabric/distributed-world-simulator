...
func show_moon_mode(distance_m: float) -> void:
	label.text = (
		...
		+ "P — включить единое абсолютное пространство Земля–Луна"
	) % (distance_m / 1000.0)
...
func show_shared_space_mode(
	...
	var moon_local: Vector3 = celestial_system.to_body_local(space_position, "moon")
	var earth_surface_distance: float = celestial_system.get_surface_distance(
		"earth",
		space_position
	)
	var moon_surface_distance: float = celestial_system.get_surface_distance(
		"moon",
		space_position
	)
	...
	label.text = (
		"ЕДИНОЕ ПРОСТРАНСТВО: ЗЕМЛЯ + ЛУНА\n"
		+ "%s\n"
		+ "Скорость %.1f м/с | координаты: [%.1f, %.1f, %.1f] км\n"
		+ "До поверхности: Луна %.1f км | Земля %.1f км | центры %.1f км\n"
		+ "%s\n"
		+ "%s\n"
		+ "Патч Земли: min %.1f м | max %.1f м | перепад %.1f м | geom-склон max %.1f°\n"
		+ "1 лес  2 равнины  3 пустыня  4 тундра  5 горы | 6 Луна | 7 Земля\n"
		+ "G смотреть на другое тело | F4 debug | F9 диагностика | Y правила | P выход\n"
		+ "WASD/QE движение | Shift ускорение | колесо скорость | H горизонт"
	) % [
		body_description,
