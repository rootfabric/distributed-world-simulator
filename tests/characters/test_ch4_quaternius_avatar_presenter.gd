...
func _run() -> void:
	...
	var source_skeleton: Skeleton3D = synthetic_presenter.get_node("QuaterniusAnimationSource/Skeleton")
	var target_skeleton: Skeleton3D = synthetic_presenter.get_node("AvatarYawRoot/QuaterniusModel/Skeleton")
	var source_hips_index := source_skeleton.find_bone("hips")
	var source_root_index := source_skeleton.find_bone("root")
	var target_hips_index := target_skeleton.find_bone("hips")
	var target_root_index := target_skeleton.find_bone("root")
	_assert(source_hips_index >= 0 and source_root_index >= 0, "Synthetic source anchor bones missing")
	_assert(target_hips_index >= 0 and target_root_index >= 0, "Synthetic target anchor bones missing")
	var target_hips_anchor := target_skeleton.get_bone_pose_position(target_hips_index)
	var target_root_anchor := target_skeleton.get_bone_pose_position(target_root_index)
	source_skeleton.set_bone_pose_position(source_hips_index, Vector3(0.6, -0.35, 0.4))
	source_skeleton.set_bone_pose_position(source_root_index, Vector3(3.0, 2.0, 1.0))
	synthetic_presenter._copy_animation_pose()
	var crouched_hips_position := target_skeleton.get_bone_pose_position(target_hips_index)
	_assert(is_equal_approx(crouched_hips_position.y, target_hips_anchor.y - 0.35), "Crouch retarget dropped the vertical pelvis grounding offset")
	_assert(is_equal_approx(crouched_hips_position.x, target_hips_anchor.x), "Crouch retarget leaked horizontal pelvis X translation")
	_assert(is_equal_approx(crouched_hips_position.z, target_hips_anchor.z), "Crouch retarget leaked horizontal pelvis Z translation")
	_assert(target_skeleton.get_bone_pose_position(target_root_index).is_equal_approx(target_root_anchor), "Crouch retarget leaked root translation")

	synthetic_presenter.apply_motion(Vector3.ZERO, Vector3.UP, Vector3.FORWARD, {"grounded": true, "crouching": false})
	synthetic_presenter._copy_animation_pose()
	_assert(target_skeleton.get_bone_pose_position(target_hips_index).is_equal_approx(target_hips_anchor), "Pelvis grounding offset did not restore after standing")
