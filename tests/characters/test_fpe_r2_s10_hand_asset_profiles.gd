...
func _run() -> void:
	...
	_assert(String(wrad_profile.get("status", "")) == "IMPORTED_INSPECTED_RUNTIME_PROBE_READY", "WRAD profile status is not probe ready")
	...
	_assert(String(wrad_retarget.get("rest_space_policy", "")) == "AUTO_CANONICAL_REBIND", "WRAD automatic rebind policy missing")
