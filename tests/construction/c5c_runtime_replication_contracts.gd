...
func _test_snapshot_and_replica_semantics() -> void:
	...
	_assert(int(snap1.revision) == 1, "snapshot revision tracks runtime generation")
	_assert(String(snap1.state_checksum) == String(state1.checksum), "snapshot state checksum mismatch")
	...
	_assert(bool(first.details.accepted), "initial snapshot not accepted")
	_assert(String(first.details.reset_reason) == "INITIAL", "initial reset reason mismatch")

	var replay: Dictionary = replica.accept_snapshot(snap1)
	_assert_ok(replay, "exact replay rejected")
	_assert(bool(replay.details.replay), "exact replay not classified")
	...
	_assert(bool(clock_result.details.clock_only), "clock-only update not classified")
	_assert(int(replica.get_snapshot().server_tick) == 11, "clock-only server tick not adopted")
	...
	_assert(bool(stale_result.details.stale), "stale clock not classified")
	...
	_assert(bool(second.details.accepted), "new revision not accepted")
	_assert(int(replica.get_snapshot().revision) == 2, "replica revision mismatch")
	_assert(String(Dictionary(replica.get_subject("runtime/contracts/replica/door").state).position) == "OPEN", "replica subject state mismatch")
	...
	_assert(String(reset_result.details.reset_reason) == "AUTHORITY_CHANGED", "authority reset reason mismatch")
	_assert(int(replica.get_snapshot().authority_epoch) == 2, "authority epoch not adopted")

	var report: Dictionary = replica.get_report()
	_assert(int(report.replays) == 1, "replica replay counter mismatch")
	_assert(int(report.stale) >= 1, "replica stale counter mismatch")
	_assert(int(report.clock_only) == 1, "replica clock-only counter mismatch")
	_assert(int(report.conflicts) == 1, "replica conflict counter mismatch")
	_assert(int(report.authority_resets) == 1, "replica authority reset counter mismatch")
