...
func _init() -> void:
	_test_selective_process_routing()
	_finish()


func _test_selective_process_routing() -> void:
	var port: int = _find_port()
	_assert(port > 0, "T1A.7.3 port allocation")
	if port <= 0:
		return
	var out: String = ProjectSettings.globalize_path("res://artifacts/test-results/t1a7-3-%d" % OS.get_process_id())
	DirAccess.make_dir_recursive_absolute(out)
	var exe: String = OS.get_executable_path()
	var root_path: String = ProjectSettings.globalize_path("res://")
	var server_file: String = out.path_join("server.json")
	var server_control: String = out.path_join("server-control.json")
	var a_file: String = out.path_join("a.json")
	var a_action: String = out.path_join("a-action.json")
	var b_file: String = out.path_join("b.json")
	var b_action: String = out.path_join("b-action.json")
	var b2_file: String = out.path_join("b2.json")
	var b2_action: String = out.path_join("b2-action.json")

	var server: int = _spawn(
		exe,
		[
			"--headless", "--quiet", "--path", root_path,
			"--log-file", out.path_join("server.log"),
			"--script", "res://tools/runtime/t1a7_3_runtime_server.gd",
			"--",
			"--host=127.0.0.1",
			"--port=%d" % port,
			"--result-file=%s" % server_file,
			"--control-file=%s" % server_control,
			"--m0-root=user://t1a7-3-m0",
		],
		out.path_join("user-server")
	)
	pids.append(server)
	_assert(server > 0, "T1A.7.3 server launch")
	_assert(String(_wait_state(server_file, "READY", SERVER_TIMEOUT_MS).get("state", "")) == "READY", "T1A.7.3 server ready")
	if server <= 0:
		return

	var a: int = _spawn_client(exe, root_path, port, "a", a_file, a_action, out.path_join("a.log"), out.path_join("user-a"))
	pids.append(a)
	_assert(a > 0, "client A launch")
	var a_ready: Dictionary = _wait_state(a_file, "READY", CLIENT_TIMEOUT_MS)
	_assert(String(a_ready.get("state", "")) == "READY", "client A gameplay join")
	_assert(Dictionary(a_ready.get("runtime_snapshot", {})).is_empty(), "A received runtime before interest")

	_write_json(server_control, {"serial": 1, "client_id": "a", "interest_revision": 1, "selected_construct_ids": [D0]})
	var control_a: Dictionary = _wait_control(server_file, 1, CLIENT_TIMEOUT_MS)
	_assert_ok(Dictionary(control_a.get("control_result", {})), "A interest activation")
	var a_baseline: Dictionary = _wait_subject_revision(a_file, DOOR_RUNTIME_ID, 0, CLIENT_TIMEOUT_MS)
	_assert(String(_subject(Dictionary(a_baseline.get("runtime_snapshot", {})), DOOR_RUNTIME_ID).get("state", {}).get("position", "")) == "CLOSED", "A baseline mismatch")
	...
	_write_json(a_action, {
		...
		"expected_revision": 0,
		"operation_id": "operation/t1a7-3/a/open-door"
	})
	...
	pids.append(b)
	_assert(b > 0, "client B launch")
	var b_ready: Dictionary = _wait_state(b_file, "READY", CLIENT_TIMEOUT_MS)
	_assert(String(b_ready.get("state", "")) == "READY", "client B gameplay join")
	OS.delay_msec(300)
	b_ready = _read(b_file)
	_assert(_runtime_snapshot_count(b_ready) == 0, "B received runtime before interest")

	_write_json(server_control, {"serial": 2, "client_id": "b", "interest_revision": 1, "selected_construct_ids": [D0]})
	var control_b: Dictionary = _wait_control(server_file, 2, CLIENT_TIMEOUT_MS)
	_assert_ok(Dictionary(control_b.get("control_result", {})), "B interest activation")
	var b_open: Dictionary = _wait_subject_revision(b_file, DOOR_RUNTIME_ID, 1, CLIENT_TIMEOUT_MS)
	_assert(String(_subject(Dictionary(b_open.get("runtime_snapshot", {})), DOOR_RUNTIME_ID).get("state", {}).get("position", "")) == "OPEN", "B late baseline mismatch")
	var b_count_before_leave: int = _runtime_snapshot_count(b_open)

	_write_json(server_control, {"serial": 3, "client_id": "b", "interest_revision": 2, "selected_construct_ids": []})
	_assert_ok(Dictionary(_wait_control(server_file, 3, CLIENT_TIMEOUT_MS).get("control_result", {})), "B leaves interest")
	...
	_write_json(a_action, {
		...
		"expected_revision": 1,
		"operation_id": "operation/t1a7-3/a/close-door"
	})
	_assert_ok(Dictionary(_wait_action(a_file, 2, CLIENT_TIMEOUT_MS).get("action_result", {})), "A closes door")
	var a_closed: Dictionary = _wait_subject_revision(a_file, DOOR_RUNTIME_ID, 2, CLIENT_TIMEOUT_MS)
	_assert(String(_subject(Dictionary(a_closed.get("runtime_snapshot", {})), DOOR_RUNTIME_ID).get("state", {}).get("position", "")) == "CLOSED", "A did not receive CLOSED selective mutation")
	OS.delay_msec(500)
	var b_after_close: Dictionary = _read(b_file)
	...
	_assert_ok(Dictionary(_wait_control(server_file, 4, CLIENT_TIMEOUT_MS).get("control_result", {})), "B re-enters interest")
	var b_closed: Dictionary = _wait_subject_revision(b_file, DOOR_RUNTIME_ID, 2, CLIENT_TIMEOUT_MS)
	_assert(String(_subject(Dictionary(b_closed.get("runtime_snapshot", {})), DOOR_RUNTIME_ID).get("state", {}).get("position", "")) == "CLOSED", "B re-entry baseline mismatch")

	_write_json(b_action, {"serial": 1, "action_kind": "LEAVE"})
	_assert(_wait_exit(b, 10000), "B graceful leave")
	_assert(int(Dictionary(_wait_connected_peers(server_file, 1, CLIENT_TIMEOUT_MS).get("m3", {})).get("connected_peer_count", -1)) == 1, "server did not observe B disconnect")

	var b2: int = _spawn_client(exe, root_path, port, "b", b2_file, b2_action, out.path_join("b2.log"), out.path_join("user-b2"))
	pids.append(b2)
	_assert(b2 > 0, "client B reconnect launch")
	_assert(String(_wait_state(b2_file, "READY", CLIENT_TIMEOUT_MS).get("state", "")) == "READY", "client B reconnect gameplay join")
	var b2_closed: Dictionary = _wait_subject_revision(b2_file, DOOR_RUNTIME_ID, 2, CLIENT_TIMEOUT_MS)
	_assert(String(_subject(Dictionary(b2_closed.get("runtime_snapshot", {})), DOOR_RUNTIME_ID).get("state", {}).get("position", "")) == "CLOSED", "reconnected B retained-interest baseline mismatch")
...
func _spawn_client(exe: String, root_path: String, port: int, client_id: String, result_file: String, action_file: String, log_file: String, user_dir: String) -> int:
	return _spawn(exe, [
		...
		"--",
		"--host=127.0.0.1", "--port=%d" % port, "--client-id=%s" % client_id,
		"--result-file=%s" % result_file, "--action-file=%s" % action_file,
	], user_dir)


func _spawn(exe: String, args: Array[String], user_dir: String) -> int:
	var names: Array = ["HOME", "XDG_DATA_HOME", "XDG_CONFIG_HOME", "XDG_CACHE_HOME", "APPDATA", "LOCALAPPDATA"]
	var old: Dictionary = {}
	for name in names:
		old[name] = {"set": OS.has_environment(String(name)), "value": OS.get_environment(String(name))}
	...
	var pid: int = OS.create_process(exe, args, false)
	for name in names:
		if bool(old[name]["set"]):
			OS.set_environment(String(name), String(old[name]["value"]))
		else:
...
func _wait_state(path: String, expected: String, timeout_ms: int) -> Dictionary:
	...
	while Time.get_ticks_msec() - started < timeout_ms:
		value = _read(path)
		if String(value.get("state", "")) == expected or String(value.get("state", "")) in ["FAILED", "TIMEOUT"]:
			return value
...
func _wait_subject_revision(path: String, runtime_id: String, revision: int, timeout_ms: int) -> Dictionary:
	...
	while Time.get_ticks_msec() - started < timeout_ms:
		value = _read(path)
		var subject: Dictionary = _subject(Dictionary(value.get("runtime_snapshot", {})), runtime_id)
...
func _find_port() -> int:
	for port in range(48000 + OS.get_process_id() % 500, 50000):
...
func _finish() -> void:
	...
	if failures.is_empty():
		print("T1A.7.3 selective replication processes: PASS (%d assertions)" % assertions)
		quit(0)
		return
	print("T1A.7.3 selective replication processes: FAIL (%d failures, %d assertions)" % [failures.size(), assertions])
	quit(1)
