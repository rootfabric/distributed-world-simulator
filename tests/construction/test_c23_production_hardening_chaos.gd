...
func _test_lost_terminal_write_boundary() -> void:
	...
	var retry: Dictionary = bundle["service"].submit(operation)
	...
	var exact: Dictionary = bundle["service"].submit(operation)
...
func _test_corruption_fallback() -> void:
	...
	var first: Dictionary = service.checkpoint(120)
	_ok(first, "checkpoint one")
	_ok(service.submit(F.operation(121)), "checkpoint two operation")
	var second: Dictionary = service.checkpoint(121)
	...
	var old_replay: Dictionary = service.submit(F.operation(120))
	_ok(old_replay, "previous operation replay after recovery")
	_assert(bool(old_replay["replay"]), "previous replay preserved")
	var rolled_back: Dictionary = service.submit(F.operation(121))
