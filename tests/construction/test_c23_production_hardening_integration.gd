...
func _test_permission_and_rate_limit_terminal_replay() -> void:
	...
	var limited: Dictionary = limited_bundle["service"].submit(F.operation(32, 3))
...
func _test_untrusted_dependency_results_are_fenced() -> void:
	...
	var invalid_authorizer_replay: Dictionary = invalid_authorizer_bundle["service"].submit(invalid_authorizer_operation)
	...
	var invalid_executor_replay: Dictionary = invalid_executor_bundle["service"].submit(invalid_executor_operation)
	...
	var unsafe_replay: Dictionary = unsafe_bundle["service"].submit(unsafe_operation)
...
func _test_restart_and_legacy_load() -> void:
	...
	var replay: Dictionary = restored["service"].submit(F.operation(40))
	...
	var loaded: Dictionary = legacy_restored["service"].load_envelope(legacy)
