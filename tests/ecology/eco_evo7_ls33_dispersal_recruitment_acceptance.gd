...
func _init() -> void:
    ...
    var placement_counterfactual = LS33.new()
    _check(placement_counterfactual.setup(patch, water, FOUNDER_SEED, PLACEMENT_SEED + 1, EVOLUTION_SEED, INITIAL_RECORDS), "alternate placement LS3.3 initializes")
    var placement_g1 := placement_counterfactual.step_generation()
    _check(not placement_g1.is_empty() and String(placement_g1["candidate_pool_hash"]) == String(g1a["candidate_pool_hash"]), "founder placement cannot change generation-one mutation candidate identities")
    ...
    var mutation_parent: Dictionary = Dictionary(before_off["records"][0]).duplicate(true)
...
func _source_guard() -> void:
    ...
    var founder_identity_start := source.find("func _founder_reproductive_identity")
