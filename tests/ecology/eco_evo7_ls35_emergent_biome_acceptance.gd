...
func _init() -> void:
    ...
    var stale_revision: Dictionary = environment.duplicate(true)
    stale_revision["revision"] = "ECO.EVO7-LS3.1.TAMPERED"
    _check(classifier.classify(stale_revision, observed_snapshot, true).is_empty(), "stale physical revision fails closed even when field hash is unchanged")

    var bad_population_hash: Dictionary = observed_snapshot.duplicate(true)
    bad_population_hash["postcompetition_population_hash"] = "0".repeat(64)
    bad_population_hash["state_hash"] = LS34.new().call("_state_hash", bad_population_hash)
    _check(classifier.classify(environment, bad_population_hash, true).is_empty(), "forged LS3.4 population hash fails closed after state rehash")

    var ecology_authority_tamper: Dictionary = observed_snapshot.duplicate(true)
    ecology_authority_tamper["authorities"]["competition_authority"] = true
    ecology_authority_tamper["state_hash"] = LS34.new().call("_state_hash", ecology_authority_tamper)
    _check(classifier.classify(environment, ecology_authority_tamper, true).is_empty(), "source ecology authority escalation fails closed even though LS3.4 state hash uses frozen authority hash")

    var hidden_source_field: Dictionary = observed_snapshot.duplicate(true)
    hidden_source_field["hidden_biome_authority"] = true
    _check(classifier.classify(environment, hidden_source_field, true).is_empty(), "unexpected hidden LS3.4 source field fails closed")

    var evaluation_relocation: Dictionary = observed_snapshot.duplicate(true)
    if not Array(evaluation_relocation["records"]).is_empty():
        var source_record: Dictionary = Array(evaluation_relocation["records"])[0]
        var source_id := String(source_record["record_id"])
        var relocated := false
        for evaluation_value in Array(Dictionary(evaluation_relocation["competition_field"])["evaluations"]):
            var evaluation: Dictionary = evaluation_value
            if String(evaluation.get("record_id", "")) != source_id:
                continue
            evaluation["cell_index"] = (int(evaluation["cell_index"]) + 1) % 1024
            evaluation["evaluation_hash"] = LS34.new().call("_evaluation_hash", evaluation)
            relocated = true
            break
        if relocated:
            evaluation_relocation["competition_field"]["field_hash"] = LS34.new().call("_competition_field_hash", evaluation_relocation["competition_field"])
            evaluation_relocation["competition_hash"] = evaluation_relocation["competition_field"]["field_hash"]
            evaluation_relocation["state_hash"] = LS34.new().call("_state_hash", evaluation_relocation)
            _check(classifier.classify(environment, evaluation_relocation, true).is_empty(), "survivor/evaluation spatial mismatch fails closed after complete competition/state rehash")
...
func _source_guard() -> void:
    ...
    _check(source.contains("_validate_current_records") and source.contains("_population_hash") and source.contains("_occupied_map_hash") and source.contains("_hereditary_pool_hash"), "LS3.5 deeply validates accepted LS3.4 survivor records and canonical hashes")
    _check(source.contains("surviving_evaluations") and source.contains("_state_hash"), "LS3.5 binds survivors to competition evidence and exact LS3.4 state hash")
