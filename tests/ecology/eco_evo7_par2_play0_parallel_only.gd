...
## injected through the public RuleWorkbench recruitment-executor facade. The
...
func _init() -> void:
	...
	if workbench == null:
		...
		_finish()
		return
	_check(not workbench.has_recruitment_executor(), "no executor pre-injected through Workbench facade")
	...
	if not _executor.setup({
		...
		_finish()
		return
	_check(workbench.set_recruitment_executor(_executor), "PAR2 executor injected through Workbench public facade")
	_check(workbench.has_recruitment_executor(), "Workbench public facade sees the executor")
