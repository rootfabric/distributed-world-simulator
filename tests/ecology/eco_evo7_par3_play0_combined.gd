...
func _init() -> void:
	...
	if not _par3.setup({
		...
		"evolution_seed": int(ls33.evolution_seed),
		...
		_finish()
		return
	_check(ls33.set_candidate_executor(_par3), "PAR3 candidate executor injected")
	_check(ls33.set_recruitment_executor(_par2), "PAR2 recruitment executor injected")
...
func _finish() -> void:
	...
	if _playground != null:
		if _playground.get_workbench() != null:
			...
			if ls33 != null:
				ls33.call("clear_candidate_executor")
