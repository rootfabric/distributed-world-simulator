...
func _source_guards() -> void:
	...
	_check(candidate_source.contains("static func candidate_pool_hash_from_canonical_order("), "R7 candidate kernel exposes canonical-order pool hash seam")
	_check(candidate_source.contains("candidate_hash_value < previous_hash"), "R7 canonical pool hash seam fails closed on non-monotonic order")
	_check(executor_source.contains("CandidateKernel.candidate_pool_hash_from_canonical_order("), "R7 optimized executor uses canonical-order pool hash seam")
