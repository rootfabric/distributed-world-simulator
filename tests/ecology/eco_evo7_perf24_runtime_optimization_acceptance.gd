...
func _source_guards() -> void:
	...
	_check(candidate_source.contains("adopt_reproduction_bundle: bool = false"), "R6 candidate adoption is optional and legacy-default false")
	_check(candidate_source.contains("else Dictionary(reproduction[\"bundle\"]).duplicate(true)"), "R6 legacy path retains historical defensive deep copy")
	_check(executor_source.contains("int(immutable_context[\"offspring_per_parent\"]),\n\t\t\t\ttrue)"), "R6 optimized STREAM1 explicitly adopts fresh reproduction bundles")
