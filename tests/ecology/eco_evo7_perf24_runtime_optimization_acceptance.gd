...
func _source_guards() -> void:
	...
	_check(ls34_source.contains("var field := {"), "R11 competition field is constructed as a fresh local value")
	_check(ls34_source.contains("_stream1_owned_field_adoption"), "R11 LS3.4 owns an explicit optimized field adoption flag")
	_check(ls34_source.contains("if _stream1_owned_field_adoption"), "R11 optimized path adopts the fresh competition field")
	_check(ls34_source.contains('else Dictionary(competition_result["field"]).duplicate(true)'), "R11 legacy path retains historical competition-field deep copy")
	_check(ls34_source.contains("_stream1_owned_field_adoption = false"), "R11 field ownership mode resets fail-closed")
