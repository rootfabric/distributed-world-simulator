...
func _init() -> void:
	...
	_check(not prepared_reproduction.is_empty(), "R9 prepares frozen default reproduction context")
	if not prepared_reproduction.is_empty():
		...
		_check(bool(LineageExtension.validate_policy(prepared_policy).get("success", false)), "R9 prepared EVO7 policy remains canonically valid")
		_check(LineageExtension.policy_hash(prepared_policy) == String(prepared_reproduction.get("evo7_policy_hash", "")), "R9 prepared EVO7 policy hash equals canonical recomputation")
		_check(bool(LineageKernel.validate_policy(genome_policy).get("success", false)), "R9 prepared genome policy remains canonically valid")
		_check(LineageKernel.policy_hash(genome_policy) == String(kernel_context.get("policy_hash", "")), "R9 prepared genome policy hash equals canonical recomputation")
		_check(CandidateKernel.validate_prepared_reproduction_context(prepared_reproduction), "R9 prepared reproduction context certifies at boundary")
		var malformed_policy := prepared_policy.duplicate(true)
		malformed_policy["morphology_probability"] = 2.0
		_check(not bool(LineageExtension.validate_policy(malformed_policy).get("success", false)), "R9 malformed prepared EVO7 policy fails canonical validation")
		var tampered_context := prepared_reproduction.duplicate(true)
		var tampered_policy: Dictionary = Dictionary(tampered_context["policy"]).duplicate(true)
		tampered_policy["morphology_probability"] = 0.41
		tampered_context["policy"] = tampered_policy
		_check(not CandidateKernel.validate_prepared_reproduction_context(tampered_context), "R9 valid-but-nondefault prepared policy fails certification")
		var tampered_hash_context := prepared_reproduction.duplicate(true)
		tampered_hash_context["evo7_policy_hash"] = "0".repeat(64)
		_check(not CandidateKernel.validate_prepared_reproduction_context(tampered_hash_context), "R9 prepared hash tamper fails certification")
...
func _source_guards() -> void:
	...
	var ls34_source := FileAccess.get_file_as_string("res://scripts/ecology/shadow/eco_evo7_ls34_local_competition_v1.gd")
	...
	_check(candidate_source.contains("static func prepare_default_reproduction_context()"), "R9 candidate kernel exposes prepared default reproduction context")
	_check(candidate_source.contains("static func validate_prepared_reproduction_context("), "R9 candidate kernel exposes prepared-context certification")
	_check(candidate_source.contains("reproduction_context_validated := false"), "R9 chunk seam owns one certification state")
	_check(candidate_source.contains("reproduction_context, reproduction_context_validated"), "R9 chunk seam threads certification into candidates")
	_check(executor_source.contains("_optimized_reproduction_context"), "R9 optimized executor owns prepared reproduction context")
	_check(lineage_extension_source.count("static func reproduce_bundle(") == 1, "R9 retains one canonical EVO7 reproduce_bundle implementation")
	_check(lineage_kernel_source.count("static func reproduce(") == 1, "R9 retains one canonical genome reproduce implementation")
	_check(not lineage_extension_source.contains("reproduce_bundle_fast"), "R9 does not add a parallel fast EVO7 mutator")
	_check(not lineage_kernel_source.contains("reproduce_fast"), "R9 does not add a parallel fast genome mutator")
	_check(lineage_extension_source.contains("static func validate_prepared_reproduction_context("), "R9 EVO7 authority certifies prepared context")
	_check(lineage_kernel_source.contains("static func validate_prepared_policy_context("), "R9 genome authority certifies prepared context")
	_check(lineage_extension_source.contains("policy != default_policy()"), "R9 EVO7 certification binds exact frozen default policy")
	_check(lineage_kernel_source.contains("policy != default_policy()"), "R9 genome certification binds exact frozen default policy")
	_check(lineage_extension_source.contains("and declared_hash == policy_hash(policy)"), "R9 EVO7 certification binds canonical policy hash")
	_check(lineage_kernel_source.contains("and declared_hash == expected_hash"), "R9 genome certification binds canonical policy hash")
	_check(ls34_source.contains("survivors.append(record.duplicate(true))"), "R10 competition survivors are freshly owned before adoption")
	_check(ls34_source.contains("_stream1_owned_survivor_adoption"), "R10 LS3.4 owns an explicit optimized survivor adoption flag")
	_check(ls34_source.contains('== "OPTIMIZED_GENERATION_BOUNDARY_CANONICALIZATION"'), "R10 survivor adoption is limited to optimized STREAM1 mode")
	_check(ls34_source.contains("if _stream1_owned_survivor_adoption"), "R10 optimized path adopts already-fresh survivor records")
	_check(ls34_source.contains("else survivors.duplicate(true)"), "R10 legacy path retains historical survivor deep copy")
	_check(ls34_source.contains("_stream1_owned_survivor_adoption = false"), "R10 survivor ownership mode resets fail-closed")
	_check(ls34_source.contains("_stream1_post_snapshot_elision"), "R12 LS3.4 owns explicit optimized post-snapshot elision state")
	_check(ls34_source.contains("last_postcompetition_population_hash = String(core.population_hash)"), "R12 optimized path uses already-refreshed canonical population hash without intermediate snapshot")
	_check(ls34_source.contains("post_record_count = core.records.size()"), "R12 optimized path uses canonical live post-competition record count")
	_check(ls34_source.contains("var post: Dictionary = core.get_snapshot()"), "R12 legacy path retains historical intermediate core snapshot")
	_check(ls34_source.contains("if _stream1_post_snapshot_elision"), "R12 snapshot copy elision is explicitly gated")
	_check(ls34_source.contains("else Array(base.get(\"records\", [])).duplicate(true)"), "R12 legacy LS3.4 snapshot retains historical records deep copy")
	_check(ls34_source.contains("_stream1_post_snapshot_elision = false"), "R12 snapshot elision mode resets fail-closed")
	...
	_check(executor_source.contains("_optimized_environment_cache_identity"), "R5 cache is fenced by immutable environment identity")
	_check(executor_source.contains("optimized environment cache bound exceeded"), "R5 cache fails closed if it exceeds environment cell count")
	_check(recruitment_source.contains('context.get("environment_sample_cache")'), "shared recruitment kernel consumes cache only when explicitly supplied")
	_check(recruitment_source.contains("prepared_environment_sample: Dictionary = {}"), "observation builder retains uncached legacy default path")
	_check(recruitment_source.contains("EnvironmentSample.validate(env)"), "cold environment sample path retains canonical validation")
