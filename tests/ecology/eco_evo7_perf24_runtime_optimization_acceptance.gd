...
func _source_guards() -> void:
	...
	_check(candidate_source.contains("static func build_presorted_unsorted("), "candidate kernel retains presorted unsorted chunk seam")
	_check(route_source.contains("static func build_in_input_order("), "route kernel retains input-order chunk seam")
	_check(executor_source.contains("PERF2.4 R4: avoid transient chunk/candidate/route/recruitment"), "R4 optimized path elides transient chunk-stage arrays")
	_check(executor_source.contains("CandidateKernel.build_candidate("), "R4 optimized path still uses canonical candidate kernel")
	_check(executor_source.contains("RouteKernel.build_route("), "R4 optimized path still uses canonical route kernel")
	_check(executor_source.contains("RecruitmentKernel.evaluate_recruitment_event("), "R4 optimized path still uses canonical recruitment kernel")
