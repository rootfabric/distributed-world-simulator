...
ACTIVE_EXECUTION = ROOT / "config/control/harness/executions/E2026-08-11-H0-0-R3"
ACTIVE_WORK_ORDER_ID = "H0-0-R3-WO-001"
ACTIVE_BRANCH = "control/h0-closed-loop-development-r3"
EPOCH_BASE_SHA = "9ad18eab0a1361bc3efcd38a19027880aac4e339"


class H0ControlHarnessTests(unittest.TestCase):
    """Phase-independent H0.0 R3 acceptance and authorization-isolation tests."""
    ...
    def historical_evidence_map(self):
        return {
            ...
            "root_cause_or_reason": "Historical evidence cannot authorize active R3.",
            "changed_surfaces": [],
        ...
        value.update(
            {
                ...
                "intent": "Active R3 test Evidence Map",
                **updates,
            }
        )
        return value
    ...
    def active_review(self, head_sha, **updates):
        ...
        value.update(
            {
                "review_id": "H0-0-R3-WO-001-TEST-POSTBUILD-REVIEW",
                "work_order_id": ACTIVE_WORK_ORDER_ID,
                "risk_class": "HIGH",
                "reviewed_head_sha": head_sha,
                "reviewer": "INDEPENDENT_R3_TEST_REVIEWER",
                **updates,
            }
        )
        return value

    def test_active_ledger_matches_declared_phase(self):
        reduced = self.reduce()
    ...
    def test_canonical_contracts_and_work_order_semantics_load(self):
        expected = {
            "harness_policy",
            "project_registry",
            "project_goals",
        ...
        self.assertTrue(expected.issubset(set(self.bundle.contracts)))
        epoch = read_json(ACTIVE_EXECUTION / "project-epoch.v1.json")
        _validate_semantics(self.bundle, epoch, self.work_order)
        self.assertEqual(67, epoch["registry_generation"])
        self.assertEqual(EPOCH_BASE_SHA, epoch["base_sha"])

    def test_historical_pass_claims_validate_but_cannot_authorize_r3(self):
    ...
    def test_reducer_rejects_sequence_identity_and_branch_corruption(self):
        if len(self.events) < 1:
            self.fail("R3 event ledger must not be empty")
    ...
    def test_no_runtime_or_historical_execution_paths_in_r3_diff(self):
        base = subprocess.run(
        ...
        forbidden_prefixes = (
            ...
            "config/control/harness/executions/E2026-08-11-H0-0-R1/",
    ...
    def test_epoch_exact_moved_audited_and_invalidated(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                "EXACT_BASE", validate_epoch(repo, epoch, "main", None)["status"]
            )
            subprocess.run(
    ...
    def test_implementation_head_uses_selected_execution_and_ignores_ledger(self):
        with tempfile.TemporaryDirectory() as directory:
            repo = Path(directory)
            subprocess.run(
            ...
            subprocess.run(
                ["git", "config", "user.email", "h0@example.test"], cwd=repo, check=True
            )
            subprocess.run(
                ["git", "config", "user.name", "H0"], cwd=repo, check=True
            )
            execution = repo / "config/control/harness/executions/E-FRESH"
            ...
            self.assertEqual(
                implementation, _git_implementation_head(repo, execution)
            )
    ...
    def test_status_plan_resume_use_r3_epoch(self):
        ...
        for switch, command in (
            ...
            with self.subTest(command=command):
                ...
                self.assertEqual(
                    "E2026-08-11-H0-0-R3", envelope["epoch"]["epoch_id"]
                )
                self.assertEqual(
