...
ACTIVE_EXECUTION = ROOT / "config/control/harness/executions/E2026-08-11-H0-1-R6"
ACTIVE_WORK_ORDER_ID = "H0-1-R6-C22-WO-001"
EPOCH_BASE_SHA = "1112d1f7cfad1df18fb3621a537e191e674848c6"
H0_0_R3_EXECUTION = ROOT / "config/control/harness/executions/E2026-08-11-H0-0-R3"


class H01ClosedLoopControlTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_canonical_contracts_and_r6_instances_validate(self):
        self.bundle.validate("project_epoch_schema", self.epoch, "h0_1_r6_epoch")
        self.bundle.validate("work_order_schema", self.work_order, "h0_1_r6_work_order")
        for event in self.events:
            self.bundle.validate("event_schema", event, "h0_1_r6_event")
        _validate_semantics(self.bundle, self.epoch, self.work_order)
        self.assertEqual(75, self.epoch["registry_generation"])
    ...
    def test_plan_is_zero_workers_before_dispatch_and_one_after(self):
        ...
        if self.reduced["state"] == "PLANNED":
            ...
            if self.reduced["state"] in {"DISPATCHED", "IN_PROGRESS", "IMPLEMENTED", "VERIFYING"}:
                self.assertEqual("SINGLE_RUNTIME_PILOT", plan["mode"])

    def test_planned_state_simulation_allows_exactly_one_worker_after_dispatch(self):
    ...
    def test_launcher_points_only_to_fresh_r6_execution(self):
        launcher = (ROOT / "CONTROL_DEVELOPMENT.ps1").read_text(encoding="utf-8")
        self.assertIn("E2026-08-11-H0-1-R6", launcher)
        self.assertNotIn("$executionPath = 'config/control/harness/executions/E2026-08-11-H0-1-R5'", launcher)

    def test_h0_0_checkpoint_evidence_is_historical_and_terminal(self):
        old_work_order = read_json(H0_0_R3_EXECUTION / "work-orders/H0-0-R3-WO-001.v1.json")
        terminal = read_json(H0_0_R3_EXECUTION / "events/H0-0-R3-WO-001/0025-h0-0-scaffold-ready-checkpoint-proposed.v1.json")
    ...
    def test_registry_generation_identity_remains_fail_closed(self):
        ...
        changed["registry_generation"] = 74
    ...
    def test_work_order_is_bounded_to_c22_and_forbids_other_runtime_domains(self):
        ...
        self.assertNotIn("scripts/construction/**", forbidden)
    ...
    def test_old_source_branch_is_evidence_not_authority(self):
        self.assertEqual("feature/h0-1-c22-current-main-r1", self.work_order["branch"])
        self.assertNotEqual("feature/c22-incremental-local-rebuild", self.work_order["branch"])
        self.assertIn("C22 SOURCE_ACCEPTED passport and PR #59 evidence", self.work_order["design_brief"]["dependencies"])
