...
class ProposedR3OwnershipProjectionTests(unittest.TestCase):
    ...
    def _projection(self) -> tuple[dict, dict, dict, dict[str, dict[str, str]]]:
        ...
        registry = copy.deepcopy(
            self._load_local_json("config/control/project-program-registry.v1.json")
        )
        policy = copy.deepcopy(
            self._load_local_json("config/control/project-control-policy.v1.json")
        )
        self.assertEqual(78, registry.get("registry_generation"))
        self.assertEqual(R2, registry.get("architecture_revision"))

        ownership_blob = pc._core.git(
            "rev-parse",
            f"{FROZEN_R3_TARGET}:{FROZEN_R3_OWNERSHIP_PATH}",
            allow_fail=True,
        )
        self.assertEqual(FROZEN_R3_OWNERSHIP_BLOB, ownership_blob)
        ownership = self._load_git_json(FROZEN_R3_TARGET, FROZEN_R3_OWNERSHIP_PATH)
        ...
        for key in LEGACY_PROGRAMS:
            ...
            self.assertRegex(head, r"^[0-9a-f]{40}$", key)
            self.assertRegex(blob, r"^[0-9a-f]{40}$", key)
            identity = {
                "program": key,
                "branch": branch,
        ...
        registry["programs"]["T"]["historical_passport_ownership_transitions"] = copy.deepcopy(
            T_TRANSITIONS
        )
