...
func _run() -> void:
	...
	_assert(String(before.schema) == GraphPersistence.SCHEMA, "Graph snapshot schema must be v1")
	_assert(int(before.schema_version) == 1, "Graph snapshot version must be 1")
