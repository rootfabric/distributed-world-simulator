...
func _init() -> void:
	var root_path: String = ProjectSettings.globalize_path(
		"user://mw10-lock-release-retry-%d-%d" % [OS.get_process_id(), Time.get_ticks_usec()]
	)
	...
	_assert(
		...
		"Release retry left released-path residue"
	)
	_assert(
	...
	if failures.is_empty():
		print("MW10 lock release retry: PASS (%d assertions)" % assertions)
		quit(0)
		return
	for failure in failures:
		push_error(failure)
	print(
		"MW10 lock release retry: FAIL (%d assertions, %d failures)"
