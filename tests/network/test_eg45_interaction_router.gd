...
func test_neg07_replay_after_success_exactly_once() -> void:
	...
	_assert(bool(replay_fresh_router.get("duplicate_replay", false)), "post-restart replay is marked DUPLICATE_REPLAY at the envelope level")
	_assert(String(replay_fresh_router["result"]["result"]) == "COMMITTED", "post-restart replay returns the stored prior canonical result verbatim")
	_assert(JSON.stringify(replay_fresh_router["result"]) == JSON.stringify(first["result"]), "replayed result identical to the original commit result")
