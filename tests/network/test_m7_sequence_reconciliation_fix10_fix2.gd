...
func _test_mtu_budget_contract() -> void:
	...
	_assert(
		...
		"FIX10 fix2 uses a conservative ENet unreliable packet budget"
	)
	_assert(
		server._fix10_unreliable_budget_decision(1200, true)
		== ServerRuntime.FIX10_UNRELIABLE_DECISION_SEND,
		"FIX10 fix2 sends an in-budget snapshot with compact ACK"
	)
	_assert(
...
func _test_source_composition() -> void:
	...
	_assert(
		server_source.contains("COMPACT_ARRAY_V1"),
		"FIX10 fix2 must preserve fix1 compact ACK wire policy"
	)
	_assert(
	...
	_assert(
		...
		"FIX10 fix2 does not measure exact canonical ProtocolFrame bytes"
	)
	_assert(
	...
	var budget_position: int = server_source.find("_fix10_unreliable_budget_decision(packet_bytes")
	var send_position: int = server_source.find("_boundary.send_to_peer(peer_id, frame)")
	_assert(budget_position >= 0, "FIX10 fix2 budget decision call missing")
	_assert(send_position >= 0, "FIX10 fix2 direct preflighted transport send missing")
	_assert(
		budget_position < send_position,
		"FIX10 fix2 MTU decision must happen before transport queue commit"
	)
