...
func _init() -> void:
	...
	if port <= 0:
		_finish()
		return
	var result_dir: String = ProjectSettings.globalize_path("res://artifacts/test-results/n1-enet-snapshot-%d" % OS.get_process_id())
	...
	var summary_path: String = ProjectSettings.globalize_path("res://artifacts/test-results/n1-enet-snapshot-summary.json")
	...
	server_pid = OS.create_process(executable, [
		...
		"--script", "res://tools/network/n1_snapshot_server.gd", "--",
		"--host=127.0.0.1", "--port=%d" % port,
		"--result-file=%s" % server_result_path,
		"--timeout-ms=%d" % TIMEOUT_MS,
		"--node-id=sim-n1",
	], false)
	_assert(server_pid > 0, "Failed to create N1 ENet server process")
	var listening: Dictionary = _wait_for_state(server_result_path, "LISTENING", 5000)
	_assert(bool(listening.get("success", false)), "N1 ENet server did not become LISTENING: %s" % listening)
	...
	client_pid = OS.create_process(executable, [
		"--headless", "--path", project_root,
		"--script", "res://tools/network/n1_snapshot_client.gd", "--",
		"--host=127.0.0.1", "--port=%d" % port,
		"--result-file=%s" % client_result_path,
		"--timeout-ms=%d" % TIMEOUT_MS,
		"--node-id=bot-n1",
	], false)
	_assert(client_pid > 0, "Failed to create N1 ENet client process")
	...
	_assert(bool(client_report.get("handshake_accepted", false)), "Client did not receive accepted handshake")
	_assert(bool(server_report.get("ack_accepted", false)), "Server did not accept snapshot ack")
	_assert(bool(client_report.get("ack_accepted", false)), "Client did not create accepted snapshot ack")
	_assert(String(server_report.get("session_id", "")) == String(client_report.get("session_id", "")), "Session IDs differ")
	_assert(String(server_report.get("snapshot_id", "")) == String(client_report.get("snapshot_id", "")), "Snapshot IDs differ")
	_assert(String(server_report.get("entity_id", "")) == String(client_report.get("entity_id", "")), "Entity IDs differ")
	_assert(String(server_report.get("snapshot_checksum", "")) == String(client_report.get("snapshot_checksum", "")), "Server/client snapshot checksum mismatch")
	_assert(int(client_report.get("snapshot_revision", -1)) == 12, "Client snapshot revision incorrect")
	_assert(String(client_report.get("authority_owner_id", "")) == "sim-n1", "Client authority owner incorrect")
	_assert(int(client_report.get("authority_epoch", -1)) == 5, "Client authority epoch incorrect")
	_assert(int(client_report.get("server_tick", -1)) == 500, "Client server tick incorrect")
	_assert(int(server_report.get("messages_sent", 0)) == 2, "Server sent unexpected message count")
	_assert(int(server_report.get("messages_received", 0)) == 2, "Server received unexpected message count")
	_assert(int(client_report.get("messages_sent", 0)) == 2, "Client sent unexpected message count")
	_assert(int(client_report.get("messages_received", 0)) == 2, "Client received unexpected message count")
	_wait_for_process_exit(server_pid, 3000)
	_wait_for_process_exit(client_pid, 3000)
...
func _write_summary(path: String, port: int, server_report: Dictionary, client_report: Dictionary) -> void:
	var summary: Dictionary = {
		"schema": "planet_simulator.n1_enet_snapshot_summary.v1",
		...
		"checksum_equal": String(server_report.get("snapshot_checksum", "")) == String(client_report.get("snapshot_checksum", "")),
	}
...
func _finish() -> void:
	...
	if failures.is_empty():
		print("N1 ENet snapshot process test: PASS (%d assertions)" % assertions)
		quit(0)
		return
	for failure in failures:
		push_error(failure)
	print("N1 ENet snapshot process test: FAIL (%d failures, %d assertions)" % [failures.size(), assertions])
	quit(1)
