...
func _test_g3_surface_adapter() -> void:
	...
	var direct_values_value = direct.get("details", {}).get("values", {})
	_assert(typeof(direct_values_value) == TYPE_DICTIONARY and Dictionary(direct_values_value).has(Registry.SURFACE_HEIGHT_M), "G3 direct provider uses accepted details.values envelope")
	var direct_values: Dictionary = Dictionary(direct_values_value) if typeof(direct_values_value) == TYPE_DICTIONARY else {}
	...
	_assert(absf(float(sample.get("value", 0.0)) - float(direct_values.get(Registry.SURFACE_HEIGHT_M, 0.0))) < 0.000000001, "G3 semantic value equals accepted provider value")
	var refs: Array = sample.get("provenance", {}).get("source_refs", [])
	_assert(refs.size() == 1, "G3 provenance has one provider source")
	_assert(refs.size() == 1 and String(refs[0].get("id", "")) == MacroProvider.PROVIDER_ID, "G3 provenance preserves provider id")
...
func _test_g6_fluid_adapter() -> void:
	...
	_assert(water_sample.has("channel_width_m") and not water_sample.has("width_m"), "G6 WaterSurfaceSample uses canonical channel_width_m key")
	...
	_assert(absf(float(samples.get(Registry.RIVER_WIDTH_M, {}).get("value", -1.0)) - float(water_sample.get("channel_width_m", -2.0))) < 0.000000001, "river-width projects G6 channel width")
