...
func _test_config_contract() -> void:
	...
	for key in Harness.CONFIG_KEYS: _check(base.has(key), "config contains %s" % key)
...
func _test_sensitivity_matrix() -> void:
	var variants := {
		"moisture_low": Harness.with_override("moisture_amplitude_scale", 0.95), "moisture_high": Harness.with_override("moisture_amplitude_scale", 1.05),
		"sunlight_low": Harness.with_override("sunlight_amplitude_scale", 0.95), "sunlight_high": Harness.with_override("sunlight_amplitude_scale", 1.05),
		"root_cost_low": Harness.with_override("root_cost_scale", 0.95), "root_cost_high": Harness.with_override("root_cost_scale", 1.05),
		"maintenance_low": Harness.with_override("maintenance_cost_scale", 0.95), "maintenance_high": Harness.with_override("maintenance_cost_scale", 1.05),
		"flood_cost_low": Harness.with_override("flood_penalty_scale", 0.95), "flood_cost_high": Harness.with_override("flood_penalty_scale", 1.05),
		"shade_low": Harness.with_override("shade_tolerance_delta", -0.05), "shade_high": Harness.with_override("shade_tolerance_delta", 0.05),
	}
...
func _test_failure_classification() -> void:
	...
	var initial_total := float(series[0]); var final_total := float(series[-1]); var max_total := 0.0
	for value in series:
		var v := float(value); _check(is_finite(v) and v >= 0.0, "biomass series finite/nonnegative"); max_total = maxf(max_total, v)
	var patch_count := int(baseline_full["patch_count"]); var hard_capacity := float(patch_count) * PatchSimulator.MAX_BIOMASS_KG_M2
	...
	failure_matrix["global_extinction"] = "PASS"; failure_matrix["unbounded_biomass"] = "PASS_BOUNDED_FAR_BELOW_CAP"
	var limits: Dictionary = baseline_summary["limiting_counts"]; var nonzero := 0; var maximum := 0
	for key in limits.keys():
		var count := int(limits[key]); if count > 0: nonzero += 1
	...
	var first := Harness.run(Harness.baseline_config(), 5, 12); var second := Harness.run(Harness.baseline_config(), 5, 12)
...
func _test_truth_boundaries() -> void:
	for boundary_index in [17, 64, 111]:
		var x := Fixture.logical_cell_boundary_x(boundary_index); _assert_samples_continuous(Fixture.sample_at(x - SEAM_EPSILON_M, 321.5), Fixture.sample_at(x + SEAM_EPSILON_M, 321.5), "x boundary %d" % boundary_index)
	for boundary_index in [13, 64, 116]:
		var z := Fixture.logical_cell_boundary_z(boundary_index); _assert_samples_continuous(Fixture.sample_at(-517.25, z - SEAM_EPSILON_M), Fixture.sample_at(-517.25, z + SEAM_EPSILON_M), "z boundary %d" % boundary_index)
	failure_matrix["boundary_seams"] = "PASS_PARENT_CONTINUITY_RECHECKED"
	var source := FileAccess.get_file_as_string("res://scripts/research/ecology/synthetic_environment_fixture_v1.gd") + FileAccess.get_file_as_string("res://scripts/research/ecology/plant_resource_model_v1.gd") + FileAccess.get_file_as_string("res://scripts/research/ecology/single_plant_patch_simulator_v1.gd") + FileAccess.get_file_as_string("res://scripts/research/ecology/eco_p1a_s4_sensitivity_harness_v1.gd")
	for forbidden in ["biome ==", "biome_id", "DESERT_PLANT", "RIVER_PLANT", "FOREST_TREE"]: _check(source.find(forbidden) < 0, "no hidden biome/region conditional: %s" % forbidden)
	failure_matrix["hidden_biome_conditionals"] = "PASS_SOURCE_SCAN"
	for forbidden in ["Camera3D", "CanvasLayer", "presentation_lod", "surface_cell_key", "AuthorityRegion", "ENetMultiplayerPeer"]: _check(source.find(forbidden) < 0, "S4 truth/sensitivity source excludes presentation/authority: %s" % forbidden)
	failure_matrix["presentation_resolution_dependency"] = "PASS_NO_PRESENTATION_INPUTS"
	var point := Fixture.CONTROL_POINTS["lower_slope"] as Vector2; var direct_a := Fixture.sample_at(point.x, point.y); var direct_b := Fixture.sample_at(point.x, point.y)
...
func _assert_samples_continuous(a: Dictionary, b: Dictionary, label: String) -> void:
	...
	for field_name in EnvironmentSample.RATIO_FIELDS: _check(absf(float(a[field_name]) - float(b[field_name])) <= RATIO_SEAM_TOLERANCE, "%s %s continuity" % [label, field_name])

func _approx(a: float, b: float, tolerance: float = 0.000000001) -> bool: return absf(a - b) <= tolerance
...
func _finish() -> void:
	if failures.is_empty():
		...
		print("ECO.P1A-S4 Determinism/Sensitivity: PASS (%d assertions)" % assertions); quit(0); return
	for failure in failures: push_error("ECO.P1A-S4 FAIL: %s" % failure)
	print("ECO.P1A-S4 Determinism/Sensitivity: FAIL (%d assertions, %d failures)" % [assertions, failures.size()]); quit(1)
