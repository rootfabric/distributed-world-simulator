...
class E34Tests(unittest.TestCase):
    ...
    def test_11_no_colonization_is_valid(self):
        c, cat, dec = program_fixture(20_000)
        out = impl.build_colonization_program(c, dec, cat)
    ...
    def test_18_program_serialization_deterministic(self):
        c, cat, dec = program_fixture()
        a = impl.serialize_program(impl.build_colonization_program(c, dec, cat))
        b = impl.serialize_program(impl.build_colonization_program(c, dec, cat))
        self.assertEqual(a, b)

    def test_19_input_order_of_patches_does_not_change_output(self):
        c, cat, dec = program_fixture()
        a = impl.build_colonization_program(c, dec, cat)
        dec = copy.deepcopy(dec)
        dec["patches"].reverse()
        b = impl.build_colonization_program(c, dec, cat)
        self.assertEqual(a, b)
    ...
    def test_21_contract_biome_table_promotion_rejected_after_rehash(self):
        c = load(CONTRACT_PATH)
        c["input_policy"]["biome_species_table_allowed"] = True
        rehash_contract(c)
        with self.assertRaises(ValueError):
            impl.validate_contract(c)

    def test_22_contract_target_injection_rejected_after_rehash(self):
        c = load(CONTRACT_PATH)
        c["input_policy"]["target_aware_species_injection_allowed"] = True
        rehash_contract(c)
        with self.assertRaises(ValueError):
            impl.validate_contract(c)
    ...
    def test_23_contract_prefilter_rejected_after_rehash(self):
        c = load(CONTRACT_PATH)
        c["input_policy"]["catalog_prefilter_allowed"] = True
        rehash_contract(c)
        with self.assertRaises(ValueError):
            impl.validate_contract(c)
    ...
    def test_24_contract_rebake_rejected_after_rehash(self):
        c = load(CONTRACT_PATH)
        c["input_policy"]["catalog_rebake_allowed"] = True
        rehash_contract(c)
        with self.assertRaises(ValueError):
            impl.validate_contract(c)
    ...
    def test_25_contract_target_tuning_rejected_after_rehash(self):
        c = load(CONTRACT_PATH)
        c["input_policy"]["catalog_target_tuning_allowed"] = True
        rehash_contract(c)
        with self.assertRaises(ValueError):
            impl.validate_contract(c)
    ...
    def test_26_contract_production_promotion_rejected_after_rehash(self):
        c = load(CONTRACT_PATH)
        c["output_policy"]["production_binding_authorized"] = True
        rehash_contract(c)
        with self.assertRaises(ValueError):
            impl.validate_contract(c)
    ...
    def test_27_contract_canonical_taxonomy_rejected_after_rehash(self):
        c = load(CONTRACT_PATH)
        c["output_policy"]["canonical_species_taxonomy"] = True
        rehash_contract(c)
        with self.assertRaises(ValueError):
            impl.validate_contract(c)
    ...
    def test_28_contract_e3_5_promotion_rejected_after_rehash(self):
        c = load(CONTRACT_PATH)
        c["successor"]["e3_5_authorized"] = True
        rehash_contract(c)
        with self.assertRaises(ValueError):
            impl.validate_contract(c)

    def test_29_catalog_missing_entry_rejected(self):
        c, cat, _ = program_fixture()
        cat = copy.deepcopy(cat)
        cat["entries"].pop()
        with self.assertRaises(ValueError):
            impl.validate_catalog(cat, c)

    def test_30_catalog_extra_entry_rejected(self):
        c, cat, _ = program_fixture()
        cat = copy.deepcopy(cat)
        cat["entries"].append(copy.deepcopy(cat["entries"][0]))
        cat["entries"][-1]["research_species_id"] += "x"
        with self.assertRaises(ValueError):
            impl.validate_catalog(cat, c)
    ...
    def test_31_catalog_canonical_promotion_rejected(self):
        c, cat, _ = program_fixture()
        cat = copy.deepcopy(cat)
        cat["canonical_species_declared"] = True
        with self.assertRaises(ValueError):
            impl.validate_catalog(cat, c)
    ...
    def test_32_entry_canonical_promotion_rejected(self):
        c, cat, _ = program_fixture()
        cat = copy.deepcopy(cat)
        cat["entries"][0]["canonical_species_declared"] = True
        with self.assertRaises(ValueError):
            impl.validate_catalog(cat, c)
    ...
    def test_40_program_hash_tamper_rejected(self):
        c, cat, dec = program_fixture()
        out = impl.build_colonization_program(c, dec, cat)
        out["summary"]["colonized_patch_count"] += 1
        with self.assertRaises(ValueError):
            impl.validate_program_integrity(out)
    ...
    def test_41_provenance_tamper_rejected(self):
        c, cat, dec = program_fixture()
        out = impl.build_colonization_program(c, dec, cat)
        out["provenance"]["historical_eco_anchor"] = "0" * 40
        with self.assertRaises(ValueError):
            impl.validate_program_integrity(out)
    ...
    def test_42_manifest_species_drop_rejected_after_program_rehash(self):
        c, cat, dec = program_fixture()
        out = impl.build_colonization_program(c, dec, cat)
        out["input_species_manifest"].pop()
        body = copy.deepcopy(out)
        body.pop("colonization_program_hash")
        out["colonization_program_hash"] = impl.sha256_canonical(body)
        with self.assertRaises(ValueError):
            impl.validate_program_integrity(out)
    ...
    def test_43_source_port_species_drop_rejected_after_program_rehash(self):
        c, cat, dec = program_fixture()
        out = impl.build_colonization_program(c, dec, cat)
        out["source_port"]["species_ids"].pop()
        body = copy.deepcopy(out)
        body.pop("colonization_program_hash")
        out["colonization_program_hash"] = impl.sha256_canonical(body)
        with self.assertRaises(ValueError):
    ...
    def test_48_serialized_artifact_ends_with_single_newline(self):
        c, cat, dec = program_fixture()
        data = impl.serialize_program(impl.build_colonization_program(c, dec, cat))
