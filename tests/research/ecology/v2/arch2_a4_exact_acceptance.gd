...
func _run(genome: Dictionary, env: Dictionary) -> Dictionary:
	...
	for _tick in 3:
		...
		for _slice in 4096:
			...
			if not result.success or result.status == "BUDGET_BLOCKED": return {}
			state = result.state
			if result.status == "TICK_COMPLETE": break
	return state
...
func _ownership_and_scale() -> void:
	...
	var here := L.sample(signal_change.state, _req("here", [500,0,500])); var there := L.sample(signal_change.state, _req("there", [1500,0,500]))
	...
	var reversed := demands.duplicate(true); reversed.reverse()
...
func _development_bridge() -> void:
	var dry := _field(100, 1000, 1, 1); var wet := _field(900, 1000, 1, 1)
	...
	var dry_state := _run(genome, dry_env); var wet_state := _run(genome, wet_env)
	_check(not dry_state.is_empty() and not wet_state.is_empty(), "field_driven_development")
	var dry_p := H.compile(dry_state, genome); var wet_p := H.compile(wet_state, genome)
