...
func _initialize() -> void:
	...
	if exact.success:
		_check(absf(float(exact.details.potentials_v.x) - 279.0/469.0) < 1e-12, "V-R41-R4 independent x potential", exact.details)
		_check(absf(float(exact.details.potentials_v.y) - 265.0/469.0) < 1e-12, "V-R41-R4 independent y potential", exact.details)
		_check(absf(float(exact.details.edge_currents_a.xy) - 2.0/469.0) < 1e-12, "V-R41-R4 independent bridge current", exact.details)
		_check(_finite(exact.details), "V-R41-R4 successful graph details contain no nonfinite", exact.details)
		var perm := _model([2.0,3.0,4.0,5.0,7.0])
		perm.nodes.reverse()
		perm.elements.reverse()
		var replay := Graph.solve_resistive(perm, {"b":0.0,"a":1.0})
		_check(replay.success and U.canonical_hash(replay.details)==U.canonical_hash(exact.details), "V-R41-R4 permutation deterministic", replay)

	var high := Graph.solve_resistive(_two_node(1e-200), {"a":1.0,"b":0.0})
	_check(high.success and _finite(high.details), "V-R41-R4 high finite result remains finite", high)
	if high.success:
		_check(absf(float(high.details.edge_currents_a.r)/1e200 - 1.0) < 1e-12, "V-R41-R4 high finite current", high.details)
		_check(absf(float(high.details.joule_power_w)/1e200 - 1.0) < 1e-12, "V-R41-R4 high finite power", high.details)
	...
	_check(common_offset.success and _finite(common_offset.details), "V-R41-R4 common-offset finite solution accepted", common_offset)
	if common_offset.success:
		_check(absf(float(common_offset.details.edge_currents_a.r) - 2.0) < 1e-12, "V-R41-R4 common-offset current analytic", common_offset.details)
		_check(absf(float(common_offset.details.boundary_power_w)/2e307 - 1.0) < 1e-12, "V-R41-R4 common-offset power analytic", common_offset.details)
	...
	_check(compensated.success and _finite(compensated.details), "V-R41-R4 compensated finite balance accepted", compensated)
	if compensated.success:
		_check(absf(float(compensated.details.port_currents_a.hub) / (-i) - 1.0) < 1e-12, "V-R41-R4 compensated hub balance analytic", compensated.details.port_currents_a)
	...
	_check(not rhs_over.success and String(rhs_over.error_code)=="R3_NUMERIC_ENVELOPE", "V-R41-R4 RHS overflow fails closed", rhs_over)
	var current_over := Graph.solve_resistive(_two_node(1.0), {"a":1e308,"b":-1e308})
	_check(not current_over.success and String(current_over.error_code)=="R3_NUMERIC_ENVELOPE", "V-R41-R4 voltage/current overflow fails closed", current_over)
	var power_over := Graph.solve_resistive(_two_node(1e-100), {"a":1e200,"b":0.0})
	_check(not power_over.success and String(power_over.error_code)=="R3_NUMERIC_ENVELOPE", "V-R41-R4 finite current but power overflow fails closed", power_over)
	var cond_over := Graph.solve_resistive(_two_node(_f64_le("e807000000000000")), {"a":1.0,"b":0.0})
	_check(not cond_over.success and String(cond_over.error_code)=="R3_NUMERIC_ENVELOPE", "V-R41-R4 reciprocal overflow fails closed", cond_over)

	# Diagnostic integrity is independently checked with a sentinel primary error.
	var primary := {"success":false,"error_code":"INDEPENDENT_PRIMARY_SENTINEL","details":{"stage":"verifier-r4"}}
	var wrapped := Perf._matrix_case_failure_for_diagnostics(2000, primary)
	_check(not wrapped.success, "V-R41-R4 diagnostic wrapper remains failure", wrapped)
	_check(String(wrapped.get("matrix_hash","forged"))=="", "V-R41-R4 failed matrix hash explicit empty", wrapped)
	_check(String(wrapped.details.case.error_code)=="INDEPENDENT_PRIMARY_SENTINEL", "V-R41-R4 primary error survives wrapper", wrapped)
	_check(int(wrapped.details.part_count)==2000, "V-R41-R4 failing scale survives wrapper", wrapped)

	print("FABRIC_R4_1_INDEPENDENT_VERIFIER_R4_ASSERTIONS=%d FAILURES=%d" % [assertions, failures.size()])
	if failures.is_empty():
		print("FABRIC-R4.1-INDEPENDENT-VERIFIER-R4: PASS")
		quit(0)
	else:
		print("FABRIC_R4_1_INDEPENDENT_VERIFIER_R4_FAILURES="+JSON.stringify(failures))
		quit(1)
