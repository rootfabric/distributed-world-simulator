...
func _init() -> void:
	...
	var full_initial := FullSolver.initial_state(full["model"])
	var rom_initial := RomRuntime.initial_state(descriptor)
	assert(bool(full_initial.get("success", false)))
	...
	for _index in range(400):
		var full_step := FullSolver.step(full["model"], full_state, flows, 0.005)
