...
func _test_manifest_identity(manifest: Dictionary) -> void:
	...
	_assert(int(manifest.get("document_revision", 0)) == 8, "A2 document revision mismatch")
...
func _test_assessment_debt_and_gates(manifest: Dictionary) -> void:
	...
	_assert(not bool(assessment.get("dedicated_server_restart_recovery_candidate", true)), "M6 recovery must no longer be candidate after acceptance")
	_assert(bool(assessment.get("dedicated_server_restart_recovery_proven", false)), "M6 accepted recovery evidence missing")
	_assert(bool(assessment.get("single_server_multiplayer_architecture_candidate", false)), "A3 architecture candidate evidence missing")
	...
	_assert(String(debt_by_id.get("A2-D04", {}).get("status", "")) == "closed", "A2-D04 must be closed by accepted M6")
...
func _test_roadmap_alignment(roadmap: Dictionary) -> void:
	_assert(String(roadmap.get("project_checkpoint", "")) == "v16.10.6-architecture-a3-single-server-multiplayer", "Roadmap current checkpoint mismatch")
	_assert(String(roadmap.get("runtime_base_checkpoint", "")) == "v16.10.5-persistence-m6-dedicated-recovery", "Roadmap runtime base mismatch")
	...
	_assert(String(phases.get("M2", {}).get("status", "")) == "accepted", "M2 roadmap status must be accepted")
