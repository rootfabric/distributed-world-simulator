...
func _test_manifest_identity(manifest: Dictionary) -> void:
	...
	_assert(int(manifest.get("document_revision", 0)) == 7, "A2 document revision mismatch")
...
func _test_assessment_debt_and_gates(manifest: Dictionary) -> void:
	...
	_assert(bool(assessment.get("dedicated_server_restart_recovery_candidate", false)), "M6 dedicated recovery candidate evidence missing")
	_assert(not bool(assessment.get("dedicated_server_restart_recovery_proven", true)), "M6 must remain candidate until local double-precision acceptance")
	...
	_assert(String(debt_by_id.get("A2-D04", {}).get("status", "")) == "candidate", "A2-D04 must be a candidate closure until M6 acceptance")
...
func _test_roadmap_alignment(roadmap: Dictionary) -> void:
	_assert(String(roadmap.get("project_checkpoint", "")) == "v16.10.5-persistence-m6-dedicated-recovery", "Roadmap current checkpoint mismatch")
	_assert(String(roadmap.get("runtime_base_checkpoint", "")) == "v16.10.4-testing-m5-graphical-multiplayer-acceptance", "Roadmap runtime base mismatch")
	...
	_assert(String(phases.get("M6", {}).get("status", "")) == "candidate", "M6 roadmap status must be candidate")
...
func _test_regression_runner_coverage() -> void:
	...
	for runner in [network_runner, world_runner]:
		...
		_assert(runner.contains("v16.10.5-persistence-m6-dedicated-recovery"), "Regression runner checkpoint is stale")
