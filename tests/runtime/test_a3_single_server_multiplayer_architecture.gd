...
func _test_manifest_and_auditor(manifest: Dictionary) -> void:
	...
	_assert(int(manifest.get("document_revision", 0)) == 1, "A3 manifest revision mismatch")
	...
	_assert(String(manifest.get("status", "")) == "candidate", "A3 delivery must remain candidate until independent acceptance")
...
func _test_roadmaps_and_debt() -> void:
	...
	_assert(String(roadmap.get("current_implementation_stage", "")) == "A3", "Network roadmap current stage mismatch")
	...
	_assert(String(phases.get("A3", {}).get("status", "")) == "candidate", "Network roadmap A3 candidate status mismatch")
	_assert(String(phases.get("B1", {}).get("status", "")) == "deferred_until_A3_acceptance", "B1 must wait for A3 acceptance")
	...
	_assert(String(milestones.get("A3", {}).get("status", "")) == "candidate", "Strategy A3 candidate status mismatch")
