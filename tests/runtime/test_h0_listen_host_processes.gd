...
func _init() -> void:
	...
	_assert(String(host_report.get("checkpoint", "")) == "v16.8.3-network-t1-multi-peer", "Listen-host checkpoint mismatch")
	_assert(String(host_report.get("build_id", "")) == "t1-multi-peer-transport-v2", "Listen-host build id mismatch")
