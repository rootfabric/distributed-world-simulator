...
func _init() -> void:
	...
	_assert(String(host_report.get("checkpoint", "")) == "v16.9.0-simulation-s1-distributed-compute", "Listen-host checkpoint mismatch")
	_assert(String(host_report.get("build_id", "")) == "s1-distributed-compute-contracts", "Listen-host build id mismatch")
