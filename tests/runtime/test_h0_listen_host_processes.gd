...
func _init() -> void:
	...
	_assert(String(host_report.get("checkpoint", "")) == "v16.8.4-data-plane-b0-message-bus-contracts", "Listen-host checkpoint mismatch")
	_assert(String(host_report.get("build_id", "")) == "b0-transport-independent-message-bus-contracts", "Listen-host build id mismatch")
