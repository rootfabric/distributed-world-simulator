...
func _init() -> void:
	...
	_assert(String(host_report.get("checkpoint", "")) == "v16.8.5-domain-m0-aggregate-transactions", "Listen-host checkpoint mismatch")
	_assert(String(host_report.get("build_id", "")) == "m0-multi-aggregate-transactions-outbox", "Listen-host build id mismatch")
