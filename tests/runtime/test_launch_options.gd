...
func _init() -> void:
	...
	_assert(String(default_descriptor.get("checkpoint", "")) == "v16.5.2-foundation-network-n1", "Default descriptor checkpoint is stale")
	_assert(String(default_descriptor.get("build_id", "")) == "n1-reconnect-replay-bounded-cache", "Default descriptor build id is stale")
	_assert(SimulatorAppScript.FOUNDATION_CHECKPOINT == "v16.5.2-foundation-network-n1", "Simulator checkpoint is stale")
	_assert(SimulatorAppScript.FOUNDATION_BUILD_ID == "n1-reconnect-replay-bounded-cache", "Simulator build id is stale")
	_assert(LunarAppScript.PROJECT_VERSION == "16.5.2-foundation-network-n1", "Lunar project version is stale")
	_assert(LunarAppScript.BUILD_ID == "n1-reconnect-replay-bounded-cache", "Lunar build id is stale")
