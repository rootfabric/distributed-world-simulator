...
func _init() -> void:
	...
	_assert(String(default_descriptor.get("checkpoint", "")) == "v16.8.0-runtime-h0-listen-host", "Default descriptor checkpoint is stale")
	_assert(String(default_descriptor.get("build_id", "")) == "h0-single-process-network-first-host", "Default descriptor build id is stale")
	_assert(SimulatorAppScript.FOUNDATION_CHECKPOINT == "v16.8.0-runtime-h0-listen-host", "Simulator checkpoint is stale")
	_assert(SimulatorAppScript.FOUNDATION_BUILD_ID == "h0-single-process-network-first-host", "Simulator build id is stale")
	_assert(LunarAppScript.PROJECT_VERSION == "16.8.0-runtime-h0-listen-host", "Lunar project version is stale")
	_assert(LunarAppScript.BUILD_ID == "h0-single-process-network-first-host", "Lunar build id is stale")
