...
func _init() -> void:
	...
	_assert(String(default_descriptor.get("checkpoint", "")) == "v16.4.2-network-transport-boundary", "Default descriptor checkpoint is stale")
	_assert(String(default_descriptor.get("build_id", "")) == "n1-transport-lifecycle-boundary", "Default descriptor build id is stale")
