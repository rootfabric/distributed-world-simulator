...
func _init() -> void:
	...
	_assert(String(final_server.get("checkpoint", "")) == "v16.10.6.1-testing-m7-playable-networked-playground", "M7 server reports playable checkpoint")
	_assert(String(ar.get("runtime_report", {}).get("checkpoint", "")) == "v16.10.6.1-testing-m7-playable-networked-playground", "M7 client A reports playable checkpoint")
	_assert(String(br.get("runtime_report", {}).get("checkpoint", "")) == "v16.10.6.1-testing-m7-playable-networked-playground", "M7 client B reports playable checkpoint")
	...
	_assert(bool(ar.get("world_report", {}).get("network_prediction_mode", false)), "M7 A uses normal predicted controller")
	_assert(bool(br.get("world_report", {}).get("network_prediction_mode", false)), "M7 B uses normal predicted controller")
