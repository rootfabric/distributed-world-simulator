...
func _init() -> void:
	...
	_assert(String(final_server.get("checkpoint", "")) == "v16.12.0-network-nx2-realtime-traffic-separation", "M7 server reports NX2 runtime checkpoint")
	_assert(String(ar.get("runtime_report", {}).get("checkpoint", "")) == "v16.12.0-network-nx2-realtime-traffic-separation", "M7 client A reports NX2 runtime checkpoint")
	_assert(String(br.get("runtime_report", {}).get("checkpoint", "")) == "v16.12.0-network-nx2-realtime-traffic-separation", "M7 client B reports NX2 runtime checkpoint")
	...
	_assert(int(realtime.get("movement_snapshots_published", 0)) > 0 and int(realtime.get("movement_snapshots_published", 0)) <= applied_inputs, "NX2 server publishes bounded network-tick movement snapshots")
