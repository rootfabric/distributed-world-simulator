...
func _test_identity(strategy: Dictionary) -> void:
	...
	_assert(int(strategy.get("document_revision", 0)) == 1, "Strategy revision mismatch")
	...
	_assert(String(strategy.get("status", "")) == "candidate", "Roadmap checkpoint must remain candidate before independent acceptance")
...
func _test_sequence_and_milestones(strategy: Dictionary) -> void:
	...
	_assert(String(by_id.get("M1", {}).get("status", "")) == "next", "M1 must be next")
...
func _test_a2_alignment(strategy: Dictionary, a2: Dictionary) -> void:
	...
	_assert(int(a2.get("document_revision", 0)) == 2, "A2 manifest revision mismatch")
...
func _test_network_roadmap_alignment(strategy: Dictionary, roadmap: Dictionary) -> void:
	_assert(int(roadmap.get("document_revision", 0)) == 20, "Network roadmap revision mismatch")
	_assert(String(roadmap.get("project_checkpoint", "")) == String(strategy.get("checkpoint", "")), "Network roadmap checkpoint mismatch")
	...
	_assert(String(by_id.get("M1", {}).get("status", "")) == "next", "M1 roadmap status mismatch")
...
func _test_documentation_and_runners() -> void:
	...
	for runner_path in ["res://RUN_NETWORK_CONTRACT_TESTS.ps1", "res://RUN_WORLD_REGRESSION_TESTS.ps1"]:
		...
		_assert(runner.contains("v16.9.5-roadmap-single-server-multiplayer-first"), "Full runner checkpoint is stale")
