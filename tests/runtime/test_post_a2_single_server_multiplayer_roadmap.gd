...
func _test_identity(strategy: Dictionary) -> void:
	...
	_assert(int(strategy.get("document_revision", 0)) == 7, "Strategy revision mismatch")
...
func _test_sequence_and_milestones(strategy: Dictionary) -> void:
	...
	_assert(String(by_id.get("M6", {}).get("status", "")) == "candidate", "M6 candidate roadmap status mismatch")
...
func _test_a2_alignment(strategy: Dictionary, a2: Dictionary) -> void:
	...
	_assert(int(a2.get("document_revision", 0)) == 7, "A2 manifest revision mismatch")
...
func _test_network_roadmap_alignment(strategy: Dictionary, roadmap: Dictionary) -> void:
	...
	_assert(String(roadmap.get("project_checkpoint", "")) == "v16.10.5-persistence-m6-dedicated-recovery", "Network roadmap checkpoint mismatch")
	...
	_assert(String(by_id.get("M4", {}).get("status", "")) == "accepted", "M4 roadmap status mismatch")
	_assert(String(by_id.get("M5", {}).get("status", "")) == "accepted", "M5 accepted roadmap status mismatch")
	_assert(String(by_id.get("M6", {}).get("status", "")) == "candidate", "M6 network roadmap status mismatch")
...
func _test_documentation_and_runners() -> void:
	...
	for runner_path in ["res://RUN_NETWORK_CONTRACT_TESTS.ps1", "res://RUN_WORLD_REGRESSION_TESTS.ps1"]:
		...
		_assert(runner.contains("v16.10.5-persistence-m6-dedicated-recovery"), "Full runner checkpoint is stale")
