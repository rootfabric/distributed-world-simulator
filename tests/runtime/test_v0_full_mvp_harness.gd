...
func _test_state_aggregation() -> void:
	...
	_assert(Acceptance.aggregate_state(failed) == Acceptance.STATE_FAIL, "FAIL has aggregate precedence")
...
func _test_soak_guard() -> void:
	...
	growth_tracker.observe(_healthy_sample(40, 45))
	var growth_result: Dictionary = growth_tracker.finish(Acceptance.FINAL_SOAK_SECONDS)
	_assert(String(growth_result.get("state", "")) == Acceptance.STATE_FAIL, "monotonic pending/queue growth fails soak")
