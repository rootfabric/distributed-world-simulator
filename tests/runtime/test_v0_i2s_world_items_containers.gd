...
func _run() -> void:
	...
	_assert(
		_ok(canonical.setup("authority/v0-i2s", 1, {"playable_sandbox": true})),
	...
	_assert(runtime_b.has_presentation(BEACON_ID), "second presentation derives from same canonical WORLD beacon")
	...
	_assert(crate_target.is_in_group(&"world_interactable"), "crate presentation participates in canonical raycast group")
	_assert(
	...
	_assert(_ok(pickup), "pickup command succeeds through canonical Item Graph")
	...
	_assert(_item_player(after_pickup, BEACON_ID) == "a", "picked canonical identity belongs to player A")
	_assert(not runtime_a.has_presentation(BEACON_ID), "presentation is removed only after canonical pickup snapshot")
	...
	_assert(
		...
		"stale second-client pickup is rejected by canonical duplicate fencing"
	)
	_assert(runtime_b.has_presentation(BEACON_ID), "stale client keeps old presentation until canonical convergence arrives")
	_assert(
		_ok(runtime_b.accept_snapshot(canonical.create_snapshot())),
		"second client accepts authoritative post-pickup snapshot"
	)
	_assert(not runtime_b.has_presentation(BEACON_ID), "second client removes picked presentation after canonical convergence")
	...
	_assert(_ok(opened), "external container opens through canonical container.open")
	...
	_assert(
		...
		"Player | Container context uses authoritative open_containers state"
	)
	...
	_assert(_ok(moved_to_container), "inventory to external-container transfer succeeds canonically")
	var after_container_transfer := runtime_a.get_canonical_snapshot()
	_assert(
		_item_location_kind(after_container_transfer, BEACON_ID) == "CONTAINER",
		"canonical item location becomes CONTAINER"
	)
	_assert(
		_item_container(after_container_transfer, BEACON_ID) == CONTAINER_ID,
		"canonical item records exact external container identity"
	)
	_assert(
		_container_has(after_container_transfer, CONTAINER_ID, BEACON_ID),
		"canonical container slots contain transferred identity"
	)
	_assert(
		_ok(runtime_b.accept_snapshot(canonical.create_snapshot())),
		"second presentation runtime accepts authoritative container transfer snapshot"
	)
	_assert(
		_container_has(runtime_b.get_canonical_snapshot(), CONTAINER_ID, BEACON_ID),
		"second client projection converges on canonical container contents"
	)

	var moved_back: Dictionary = runtime_a.transfer_item(BEACON_ID)
	_assert(_ok(moved_back), "external container item transfers back to canonical player inventory")
	_assert(
		_item_location_kind(runtime_a.get_canonical_snapshot(), BEACON_ID) == "INVENTORY",
		"transfer back restores canonical INVENTORY location"
	)
	...
	_assert(_item_has_valid_transform(after_drop, BEACON_ID), "dropped canonical WORLD identity has authoritative spatial transform")
	_assert(runtime_a.has_presentation(BEACON_ID), "drop snapshot recreates world presentation")
	_assert(
		_ok(runtime_b.accept_snapshot(canonical.create_snapshot())),
		"second client accepts authoritative drop snapshot"
	)
	_assert(runtime_b.has_presentation(BEACON_ID), "second client recreates dropped world presentation")
	_assert(
	...
	_assert(
		_ok(non_spatial.setup("authority/v0-i2s/non-spatial", 1, {"playable_sandbox": false})),
		"non-sandbox canonical Item Graph configures"
	)
	...
	_assert(
		_ok(non_spatial_runtime.setup(
			non_spatial_world,
			"a",
			Callable(self, "_reject_unexpected_command")
		)),
	...
	_assert(
		non_spatial_runtime.get_presentation_item_ids().is_empty(),
		"I2S never invents presentation-only positions for non-spatial WORLD records"
	)
...
func _submit_canonical(
	...
	if resolved_operation_id.is_empty():
		resolved_operation_id = "operation/v0-i2s/%s/%d" % [player_id, sequence]
...
func _finish() -> void:
	print("V0-I2S canonical world items + containers: %d assertions, %d failures" % [
		assertions,
		failures.size(),
	])
