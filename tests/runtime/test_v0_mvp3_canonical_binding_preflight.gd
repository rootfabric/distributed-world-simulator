...
# Characterize the REAL canonical owner APIs, not a substitute seamless demo.
# Diagnostic success means the specified integration gaps were reproduced.
# It NEVER means that MVP3 passed. Restart contracts must remain unchanged.
...
func _run() -> void:
	...
	for id in ["a", "b"]:
		var joined: Dictionary = source.join(id, "transport/mvp3/" + id, "operation/mvp3/join-" + id)
		_check(bool(joined.get("success", false)), "canonical player joins: " + id)
		var moved: Dictionary = source.move_player(id, "transport/mvp3/" + id, 1, 1, 0.25 if id == "a" else 0.0, 0.25 if id == "b" else 0.0, "operation/mvp3/initial-" + id)
	...
	var replay: Dictionary = source.export_replay_state()
	...
	_check(durable.get("canonical_item_graph", {}).get("snapshot", {}).get("inventories", {}).has("b"), "whole aggregate includes the other player's Item Graph inventory")
	report["source_live_snapshot"] = before
	report["source_durable_state"] = durable
	report["source_replay_state"] = replay
	...
	_check(before_rebind.get("position") == source.get_player("a").get("position"), "restart retains position, not a missing spatial snapshot")
	var unbound_move: Dictionary = restored.move_player("a", "transport/mvp3/a", 1, 2, 0.25, 0.0, "operation/mvp3/unbound-move")
	_check(unbound_move.get("error_code") == "PLAYER_NOT_CONNECTED", "old live client binding cannot execute after restart restore")
	var rejoin_result: Dictionary = restored.join("a", "transport/mvp3/a", "operation/mvp3/rejoin-a")
	...
	_check(int(rebound.get("ownership_epoch", 0)) == int(source.get_player("a").get("ownership_epoch", 0)) + 1, "ordinary rejoin advances client ownership epoch")
	_check(rebound.get("player_entity_id") == before_rebind.get("player_entity_id"), "rejoin keeps entity identity; do not falsely call it a respawn")
	_check(rebound.get("position") == before_rebind.get("position"), "rejoin keeps position")
	var old_epoch_move: Dictionary = restored.move_player("a", "transport/mvp3/a", 1, 2, 0.25, 0.0, "operation/mvp3/old-client-epoch")
	_check(old_epoch_move.get("error_code") == "STALE_PLAYER_OWNERSHIP_EPOCH", "unchanged client's ownership epoch is no longer accepted")
	report["cases"]["restart_rebind"] = {"restore": restart_result, "move_before_rejoin": unbound_move, "player_before_rejoin": before_rebind, "rejoin": rejoin_result, "player_after_rejoin": rebound, "old_client_epoch_move": old_epoch_move, "note": "No network disconnect was executed by this headless characterization. It proves restart/rejoin API semantics, not transport behavior."}

	# Keep the canonical Item Graph intact and try to interpret an aggregate as
	# an actor-only payload. Re-hash the edited player sections legitimately;
	# the rejection must be semantic, not a conveniently broken checksum.
	...
	_check(not bool(actor_only_result.get("success", false)), "whole-world payload is not a valid actor-only transfer")
	_check(actor_only_result.get("error_code") == "ITEM_GRAPH_INVENTORY_PLAYER_MISSING", "actor-only rejection proves cross-domain ownership coupling")
	...
	live_in_restart["players"]["players"][0]["transport_session_id"] = "transport/mvp3/a"
	...
	_check(not bool(live_in_restart_result.get("success", false)), "cannot smuggle a live session through restart DTO")
	_check(live_in_restart_result.get("cause", {}).get("error_code", live_in_restart_result.get("details", {}).get("cause", {}).get("error_code", "")) == "DURABLE_PLAYER_SESSION_MUST_BE_DISCONNECTED", "live session is rejected by restart session contract")
	report["cases"]["live_session_in_restart_dto"] = live_in_restart_result
	_check(source.create_snapshot() == before, "invalid target/projection attempts leave actual source untouched")

	# A correct SM1 state-machine freeze is not an automatic M3 write fence.
	# The missing binding must guard every real mutation entry point.
	var coordinator = Coordinator.new()
	var coordinator_player: Dictionary = source.get_player("a")
	...
	_check(bool(coordinator.begin_transfer("transfer/mvp3/binding-probe", "authority/a", "authority/b", 1).get("success", false)), "real SM1 source freeze begins")
	var admission: Dictionary = coordinator.authorize_write("authority/a", 1)
	_check(admission.get("error_code") == "SM1_AUTHORITY_TRANSFER_WRITE_FENCED", "SM1 admission itself correctly fences source")
	var unbound_source_move: Dictionary = source.move_player("a", "transport/mvp3/a", 1, 2, 0.25, 0.0, "operation/mvp3/unbound-source-write")
	_check(bool(unbound_source_move.get("success", false)), "unbound M3 service does not automatically consult the separate coordinator")
	_check(source.get_player("a").get("position") != before["players"][0].get("position"), "unbound direct service call really mutates canonical player")
	report["cases"]["unbound_sm1_freeze"] = {"coordinator_admission": admission, "direct_service_call": unbound_source_move, "coordinator_snapshot": coordinator.snapshot(), "source_after": source.create_snapshot(), "note": "This is an intentionally UNBOUND composition, not a defect in SM1. It prevents treating coordinator-only evidence as an M3 integration proof."}

	for value in [source, different_owner, different_epoch, restored]:
		value.shutdown()
	...
	report["conclusion"] = "TESTED_RESTART_AND_UNBOUND_COORDINATOR_PATHS_DO_NOT_IMPLEMENT_LIVE_PER_PLAYER_HANDOFF"
	var output := OS.get_environment("DWS_MVP3_PREFLIGHT_OUTPUT")
	var written: Dictionary = AtomicJson.write_dictionary(output, report)
