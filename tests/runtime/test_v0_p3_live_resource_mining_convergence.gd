...
func _run() -> void:
	var port := _find_port()
	_assert(port > 0, "V0-P3 mining UDP port allocated")
	if port <= 0:
		_finish()
		return
	var root_path := ProjectSettings.globalize_path(
		"res://artifacts/test-results/v0-p3-live-mining-%d" % OS.get_process_id()
	)
	...
	_assert(server_pid > 0, "V0-P3 dedicated server launched")
	if server_pid <= 0:
		_finish()
		return
	_assert(
		_wait_log_marker(server_pid, server_log, "\"event\":\"SERVER_READY\"", READY_TIMEOUT_MS),
		"V0-P3 server reports SERVER_READY"
	)
	_assert(
		_wait_log_marker(server_pid, server_log, "\"event\":\"earth_runtime_ready\"", READY_TIMEOUT_MS),
		"V0-P3 Earth runtime ready"
	)
	...
	var role_ready := _wait_log_marker(
		server_pid,
		server_log,
		"\"runtime_role\":\"dedicated-server\"",
		READY_TIMEOUT_MS
	)
	_assert(lifecycle_ready and role_ready, "V0-P3 dedicated-server lifecycle fully ready")
	...
	_assert(actor_pid > 0, "V0-P3 actor A launched")
	var actor_ready := _wait_state(actor_file, ["ACTOR_READY", "FAILED"], CLIENT_TIMEOUT_MS)
	_assert(_passed_state(actor_ready, "ACTOR_READY"), "V0-P3 actor A reaches resource-ready state")
	...
	_assert(before_pid > 0, "V0-P3 initial B launched")
	var before := _wait_state(before_file, ["BEFORE_COMPLETE", "FAILED"], CLIENT_TIMEOUT_MS)
	_assert(_passed_state(before, "BEFORE_COMPLETE"), "V0-P3 initial B captures resource state and leaves")
	_wait_exit(before_pid, EXIT_TIMEOUT_MS)
	_assert(not OS.is_process_running(before_pid), "V0-P3 initial B exits before mining")
	...
	var before_resource_checksum := String(before_details.get("resource_checksum", ""))
	var before_resource_generation := int(before_details.get("resource_generation", -1))
	var before_item_checksum := String(before_details.get("item_graph_checksum", ""))
	_assert(
		before_session.length() > 0 and before_entity == "player/b" and before_epoch >= 1,
		"V0-P3 captures B reconnect identity provenance"
	)
	_assert(
		before_resource_checksum.length() == 64 and before_resource_generation >= 1,
		"V0-P3 captures pre-mine canonical resource state"
	)
	_assert(before_item_checksum.length() == 64, "V0-P3 captures pre-mine Item Graph checksum")
	_assert(int(before_details.get("remaining_units", -1)) == 8, "V0-P3 B confirms eight units before mining")

	_write_control(control_file, "MUTATE")
	var actor_mutated := _wait_state(actor_file, ["ACTOR_MUTATED", "FAILED"], CLIENT_TIMEOUT_MS)
	_assert(_passed_state(actor_mutated, "ACTOR_MUTATED"), "V0-P3 A mines while B is absent")
	if not _passed_state(actor_mutated, "ACTOR_MUTATED"):
		_finish()
		return
	var actor_details: Dictionary = actor_mutated.get("details", {})
	var resource_checksum := String(actor_details.get("resource_checksum", ""))
	var resource_generation := int(actor_details.get("resource_generation", -1))
	var item_checksum := String(actor_details.get("item_graph_checksum", ""))
	var output_item_id := String(actor_details.get("output_item_id", ""))
	_assert(
		resource_checksum.length() == 64 and resource_checksum != before_resource_checksum,
		"V0-P3 mining changes canonical resource checksum"
	)
	_assert(
		resource_generation == before_resource_generation + 1,
		"V0-P3 mining advances canonical resource generation once"
	)
	_assert(int(actor_details.get("remaining_units", -1)) == 7, "V0-P3 mining leaves seven units")
	_assert(item_checksum.length() == 64 and item_checksum != before_item_checksum, "V0-P3 mining changes canonical Item Graph checksum")
	_assert(output_item_id.begins_with("item/server-output/"), "V0-P3 mining produces deterministic server-output item identity")
	...
	var after_pid := _spawn_client(
		...
		control_file,
		{
			...
			"expected-resource-checksum": resource_checksum,
			"expected-resource-generation": str(resource_generation),
			"expected-output-item-id": output_item_id,
	...
	_assert(after_pid > 0, "V0-P3 reconnect B launched")
	var after_ready := _wait_state(after_file, ["RECONNECT_READY", "FAILED"], CLIENT_TIMEOUT_MS)
	_assert(_passed_state(after_ready, "RECONNECT_READY"), "V0-P3 reconnect B reconstructs resource and Item Graph state")
	if not _passed_state(after_ready, "RECONNECT_READY"):
		_finish()
		return
	var after_details: Dictionary = after_ready.get("details", {})
	_assert(String(after_details.get("resource_checksum", "")) == resource_checksum, "V0-P3 reconnect B exact resource checksum")
	_assert(int(after_details.get("resource_generation", -1)) == resource_generation, "V0-P3 reconnect B exact resource generation")
	_assert(int(after_details.get("remaining_units", -1)) == 7, "V0-P3 reconnect B exact remaining units")
	_assert(String(after_details.get("item_graph_checksum", "")) == item_checksum, "V0-P3 reconnect B exact Item Graph checksum")
	_assert(String(after_details.get("output_item_id", "")) == output_item_id, "V0-P3 reconnect B exact mined output identity")
	_assert(String(after_details.get("transport_session_id", "")) != before_session, "V0-P3 reconnect B gets new transport session")
	_assert(String(after_details.get("player_entity_id", "")) == before_entity, "V0-P3 reconnect B preserves canonical entity")
	_assert(int(after_details.get("ownership_epoch", 0)) > before_epoch, "V0-P3 reconnect B advances ownership epoch")

	var actor_reconnect := _wait_state(actor_file, ["ACTOR_RECONNECT_SEEN", "FAILED"], CLIENT_TIMEOUT_MS)
	_assert(_passed_state(actor_reconnect, "ACTOR_RECONNECT_SEEN"), "V0-P3 A remains live and observes B return")
	if not _passed_state(actor_reconnect, "ACTOR_RECONNECT_SEEN"):
		_finish()
		return
	var actor_final: Dictionary = actor_reconnect.get("details", {})
	_assert(String(actor_final.get("resource_checksum", "")) == resource_checksum, "V0-P3 reconnect itself does not mutate resource state")
	_assert(String(actor_final.get("item_graph_checksum", "")) == item_checksum, "V0-P3 reconnect itself does not mutate Item Graph")
	_assert(String(actor_final.get("output_item_id", "")) == output_item_id, "V0-P3 reconnect itself does not duplicate ore")
	_assert(
		String(actor_final.get("resource_checksum", "")) == String(after_details.get("resource_checksum", ""))
		and String(actor_final.get("item_graph_checksum", "")) == String(after_details.get("item_graph_checksum", "")),
		"V0-P3 A/B canonical resource and Item Graph replicas converge"
	)
	...
	_assert(_passed_state(after_complete, "RECONNECT_COMPLETE"), "V0-P3 reconnect B leaves cleanly")
	_assert(_passed_state(actor_complete, "ACTOR_COMPLETE"), "V0-P3 actor A remains healthy through mining/reconnect cycle")
	_wait_exit(after_pid, EXIT_TIMEOUT_MS)
	_wait_exit(actor_pid, EXIT_TIMEOUT_MS)
	_assert(not OS.is_process_running(after_pid) and not OS.is_process_running(actor_pid), "V0-P3 both client processes exit")
	...
	_assert(true, "V0-P3 dedicated server termination signal sent")
	for entry in [
...
func _spawn_client(
	...
	var args: Array[String] = [
		...
		"--path", project_root,
		"--log-file", log_file,
		"--script", "res://tools/runtime/v0_p3_live_mining_client.gd",
		"--",
		"--host=127.0.0.1",
		"--port=%d" % port,
		"--mode=%s" % mode,
...
func _spawn(executable: String, args: Array[String], user_root: String) -> int:
	...
	for path in [
		user_root,
		user_root.path_join("data"),
		user_root.path_join("config"),
		user_root.path_join("cache"),
	]:
...
func _assert_clean(path: String, label: String) -> void:
	...
	for marker in [
		...
		"V0_P3_RECONNECT_SERVER_DISCONNECTED",
	]:
		if content.contains(marker):
			clean = false
	_assert(clean, "%s log contains no mining/reconnect/parser/runtime failure" % label)
...
func _find_port() -> int:
	for candidate in range(48820, 48920):
...
func _finish() -> void:
	...
	if failures.is_empty():
		print("V0-P3 live resource mining convergence: %d assertions, 0 failures" % assertions)
		quit(0)
		return
	for failure in failures:
		push_error(failure)
	print(
		"V0-P3 live resource mining convergence: %d assertions, %d failures"
