...
func _test_catalog_and_earth_projection() -> void:
	...
	var node := service.get_node(NODE_ID)
...
func _test_mine_replay_and_rejection_purity() -> void:
	...
	var replay_resource_before := service.create_snapshot()
	var replay_item_before := graph.create_snapshot()
	...
	var conflict_resource_before := service.create_snapshot()
	var conflict_item_before := graph.create_snapshot()
