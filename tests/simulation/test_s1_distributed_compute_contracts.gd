...
func _test_budget_and_worker_descriptors() -> void:
	...
	var zero := BudgetScript.create(0, 1, 1)
	_assert_error(BudgetScript.validate(zero), "INVALID_EXECUTION_BUDGET", "Zero operation budget accepted")
...
func _test_job_contract() -> void:
	...
	var changed_checksum := job.duplicate(true); changed_checksum["to_tick"] = 21
	_assert_error(JobScript.validate(changed_checksum), "SIMULATION_JOB_DETERMINISM_FINGERPRINT_MISMATCH", "Changed job tick with stale fingerprint accepted")
	var bad_checksum := job.duplicate(true); bad_checksum["checksum"] = "0".repeat(64)
	_assert_error(JobScript.validate(bad_checksum), "SIMULATION_JOB_CHECKSUM_MISMATCH", "Invalid job checksum accepted")
	var unsorted_inputs := job.duplicate(true); unsorted_inputs["input_references"].reverse(); unsorted_inputs["determinism_fingerprint"] = FingerprintScript.create(unsorted_inputs["input_references"], RULE_HASH, "growth-v1", 17, 1, 20); unsorted_inputs["checksum"] = JobScript.compute_checksum(unsorted_inputs)
	_assert_error(JobScript.validate(unsorted_inputs), "SIMULATION_JOB_INPUTS_NOT_CANONICAL", "Unsorted job inputs accepted")
	var wrong_authority := job.duplicate(true); wrong_authority["input_references"][0]["authority_epoch"] = 4; wrong_authority["determinism_fingerprint"] = FingerprintScript.create(wrong_authority["input_references"], RULE_HASH, "growth-v1", 17, 1, 20); wrong_authority["checksum"] = JobScript.compute_checksum(wrong_authority)
	...
	var changed_fingerprint: Dictionary = job["determinism_fingerprint"].duplicate(true); changed_fingerprint["deterministic_seed"] = 18
...
func _test_proposal_and_result_contracts() -> void:
	...
	var job := _job()
	var proposal := ProposalScript.create("proposal/growth/1", String(job["job_id"]), 1, "worker/test/growth-1", "capability/vegetation-growth", String(job["determinism_fingerprint"]["fingerprint"]), RULE_HASH, [operation_a, operation_b], 50)
	...
	var changed_metric := proposal.duplicate(true); changed_metric["operation_count"] = 1; changed_metric["proposal_hash"] = ProposalScript.compute_proposal_hash(changed_metric); changed_metric["checksum"] = ProposalScript.compute_checksum(changed_metric)
	...
	var failure := ResultScript.failure_result("result/growth/failure", String(job["job_id"]), 1, "worker/test/growth-1", "TRANSIENT_WORKER_FAILURE", true)
	_assert_ok(ResultScript.validate(failure), "Valid failed result rejected")
	var invalid_success := result.duplicate(true); invalid_success["retryable"] = true; invalid_success["result_hash"] = ResultScript.compute_result_hash(invalid_success); invalid_success["checksum"] = ResultScript.compute_checksum(invalid_success)
...
func _test_project_wiring() -> void:
	...
	_assert(architecture.contains("worker вычисляет") and architecture.contains("M0"), "S1 architecture does not document authority/worker boundary")
	var roadmap = JSON.parse_string(roadmap_text)
	_assert(roadmap is Dictionary, "Roadmap JSON invalid")
	if roadmap is Dictionary:
		_assert(String(roadmap.get("project_checkpoint", "")) == "v16.9.0-simulation-s1-distributed-compute", "Roadmap checkpoint stale")
		var statuses: Dictionary = {}
		for phase in roadmap.get("phases", []): statuses[String(phase.get("id", ""))] = String(phase.get("status", ""))
...
func _finish() -> void:
	if failures.is_empty():
		print("S1 distributed compute contracts: PASS (%d assertions)" % assertions); quit(0); return
	for failure in failures: push_error(failure)
	print("S1 distributed compute contracts: FAIL (%d failures, %d assertions)" % [failures.size(), assertions]); quit(1)
