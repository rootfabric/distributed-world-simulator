...
const CURRENT_ROADMAP_CHECKPOINT := "v16.10.4-testing-m5-graphical-multiplayer-acceptance"
...
func _test_project_wiring() -> void:
	...
	_assert(network_runner.contains("Foundation N0 through M5 graphical multiplayer acceptance"), "Network runner final status does not include the current M5 graphical acceptance checkpoint")
