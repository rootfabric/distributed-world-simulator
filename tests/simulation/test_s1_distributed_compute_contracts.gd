...
const CURRENT_ROADMAP_CHECKPOINT := "v16.10.5-persistence-m6-dedicated-recovery"
...
func _test_project_wiring() -> void:
	...
	_assert(network_runner.contains("Foundation N0 through M6 dedicated persistence and recovery"), "Network runner final status does not include the current M5 graphical acceptance checkpoint")
