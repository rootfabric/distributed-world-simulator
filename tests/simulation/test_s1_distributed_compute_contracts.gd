...
const CURRENT_ROADMAP_CHECKPOINT := "v16.10.0-runtime-m1-unified-networked-gameplay-core"
...
func _test_project_wiring() -> void:
	...
	_assert(network_runner.contains("Foundation N0 through M1 unified networked gameplay core"), "Network runner final status does not include the current M1 runtime checkpoint")
