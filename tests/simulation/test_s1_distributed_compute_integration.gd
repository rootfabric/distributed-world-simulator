...
func _test_queue_worker_authority_commit_and_replay() -> void:
	...
	var job := _create_job(runtime, _budget())
	_assert_ok(job, "Job creation failed")
	var job_value: Dictionary = job["details"]["job"]
	var submit: Dictionary = runtime["queue_bridge"].submit(job_value)
	...
	_assert(String(claim["details"]["job"]["checksum"]) == String(job_value["checksum"]), "Queue bridge changed simulation job")
	...
	var before: Dictionary = runtime["transaction"].get_state_report(); _assert_ok(before, "Pre-commit state report failed")
	var accepted: Dictionary = runtime["authority"].accept_result(job_value, result)
	...
	var after: Dictionary = runtime["transaction"].get_state_report(); _assert_ok(after, "Post-commit state report failed")
	...
	var replay: Dictionary = runtime["authority"].accept_result(job_value, result)
	...
	var changed_proposal := ProposalScript.create(String(result["proposal"]["proposal_id"]), String(job_value["job_id"]), 1, "worker/test/growth-1", "capability/vegetation-growth", String(job_value["determinism_fingerprint"]["fingerprint"]), RULE_HASH, [result["proposal"]["operations"][0], changed_operation], 50)
	var changed_result := ResultScript.success_result(String(result["result_id"]), changed_proposal)
	_assert_error(runtime["authority"].accept_result(job_value, changed_result), "COMPUTE_RESULT_CONFLICT", "Changed result reused accepted job attempt")


func _test_input_projection_and_determinism() -> void:
	var runtime := _new_runtime("determinism")
	var job_result := _create_job(runtime, _budget()); _assert_ok(job_result, "Determinism job creation failed")
	var job: Dictionary = job_result["details"]["job"]
	var original_checksum: String = String(job["checksum"])
	var first: Dictionary = runtime["worker"].execute(job); _assert_ok(first, "First deterministic worker run failed")
	var second: Dictionary = runtime["worker"].execute(job); _assert_ok(second, "Second deterministic worker run failed")
...
func _test_stale_proposal_fencing() -> void:
	...
	var job_result := _create_job(runtime, _budget()); _assert_ok(job_result, "Stale job creation failed")
	var job: Dictionary = job_result["details"]["job"]
	var worker_output: Dictionary = runtime["worker"].execute(job); _assert_ok(worker_output, "Stale worker execution failed")
	...
	_assert_error(runtime["authority"].accept_result(job, worker_output["details"]["result"]), "STALE_COMPUTE_INPUT_CHECKSUM", "Stale proposal was accepted")
...
func _test_malicious_write_and_budget_fencing() -> void:
	...
	var job_result := _create_job(runtime, _budget()); _assert_ok(job_result, "Fence job creation failed")
	var job: Dictionary = job_result["details"]["job"]
	var valid_output: Dictionary = runtime["worker"].execute(job); _assert_ok(valid_output, "Valid worker output failed")
	...
	var malicious_proposal := ProposalScript.create("proposal/malicious/write", String(job["job_id"]), 1, "worker/test/growth-1", "capability/vegetation-growth", String(job["determinism_fingerprint"]["fingerprint"]), RULE_HASH, [valid_result["proposal"]["operations"][0], malicious_operation], 50)
	var malicious_result := ResultScript.success_result("result/malicious/write", malicious_proposal)
	_assert_error(runtime["authority"].accept_result(job, malicious_result), "UNDECLARED_COMPUTE_WRITE", "Authority accepted undeclared write")
...
func _test_worker_capability_fencing() -> void:
	...
	var job_result := _create_job(runtime, _budget()); _assert_ok(job_result, "Capability job creation failed")
	var job: Dictionary = job_result["details"]["job"].duplicate(true)
	job["required_capability_id"] = "capability/weather"
	job["checksum"] = preload("res://scripts/simulation/compute/simulation_job_envelope.gd").compute_checksum(job)
	var invalid = runtime["worker"].execute(job)
	...
	var unregistered_result: Dictionary = runtime["worker"].execute(job_result["details"]["job"])
	_assert_ok(unregistered_result, "Worker result creation failed")
	var empty_registry := WorkerRegistryScript.new()
	var alternate_authority := AuthorityScript.new(); _assert_ok(alternate_authority.configure(runtime["transaction"], runtime["registry"], empty_registry), "Alternate authority configure failed")
	_assert_error(alternate_authority.accept_result(job_result["details"]["job"], unregistered_result["details"]["result"]), "COMPUTE_WORKER_NOT_FOUND", "Unregistered worker result accepted")


func _new_runtime(suffix: String) -> Dictionary:
	var registry := RegistryScript.new(); _assert_ok(registry.setup(), "Aggregate registry setup failed")
	_assert_ok(registry.register_adapter(AdapterScript.new()), "Transaction adapter registration failed")
	var repository := RepositoryScript.new(); _assert_ok(repository.configure(_root.path_join(suffix)), "Transaction repository configure failed")
	var invariant_registry := InvariantRegistryScript.new(); _assert_ok(invariant_registry.setup(), "Invariant registry setup failed")
	var transaction := TransactionCoordinatorScript.new(); _assert_ok(transaction.configure(registry, repository, invariant_registry), "Transaction coordinator configure failed")
	_assert_ok(transaction.bootstrap(_base_snapshots()), "Transaction bootstrap failed")
	var job_factory := JobFactoryScript.new(); _assert_ok(job_factory.configure(transaction), "Job factory configure failed")
	var worker_registry := WorkerRegistryScript.new(); _assert_ok(worker_registry.register_worker(_worker_descriptor()), "Worker registration failed")
	var handler := HandlerScript.new()
	var worker := LocalWorkerScript.new(); _assert_ok(worker.configure(_worker_descriptor(), handler), "Local worker configure failed")
	var authority := AuthorityScript.new(); _assert_ok(authority.configure(transaction, registry, worker_registry), "Compute authority configure failed")
	var queue_adapter := JobQueueAdapterScript.new("adapter/s1-in-memory-job-queue", 16)
	var queue_bridge := JobQueueBridgeScript.new(); _assert_ok(queue_bridge.configure(queue_adapter), "Queue bridge configure failed")
	return {"registry": registry, "repository": repository, "transaction": transaction, "job_factory": job_factory, "worker_registry": worker_registry, "handler": handler, "worker": worker, "authority": authority, "queue_adapter": queue_adapter, "queue_bridge": queue_bridge}
...
func _finish() -> void:
	if failures.is_empty():
		print("S1 distributed compute integration: PASS (%d assertions)" % assertions); quit(0); return
	for failure in failures: push_error(failure)
	print("S1 distributed compute integration: FAIL (%d failures, %d assertions)" % [failures.size(), assertions]); quit(1)
