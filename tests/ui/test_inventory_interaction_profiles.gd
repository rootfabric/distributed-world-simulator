...
func _test_runtime_profiles() -> void:
	...
	_assert(Array(screen.player_panel.current_model.get("cells", [])).size() == player_cell_count, "7 Days profile switch must preserve the backpack model")
	_assert(screen.player_panel.get_rendered_cell_count() == player_cell_count, "7 Days profile switch must keep all backpack cells rendered")
	...
	var relation_before: Dictionary = beacon.relation.duplicate(true)
	var quantity_before := int(beacon.quantity)
	var revision_before := int(beacon.revision)
	...
	_assert(screen.transfer_session.is_active(), "7 Days RMB click must create virtual carry session")
	_assert(screen.transfer_session.remaining_quantity == 2, "7 Days RMB click must carry ceil(half) of three")
	_assert(beacon.relation == relation_before and int(beacon.quantity) == quantity_before and int(beacon.revision) == revision_before, "Starting virtual carry must not mutate Item Graph")
	_assert(screen.carry_preview.visible, "Virtual carry must be visible at cursor")
	screen._on_interaction_requested("CARRY_HALF_OR_PLACE_ONE", {
		"target_container_id": "missing-container",
		"target_slot_index": 0,
		"target_item_id": "",
	})
	_assert(screen.transfer_session.is_active() and screen.transfer_session.remaining_quantity == 2, "Rejected target must preserve virtual carry quantity")
	_assert(beacon.relation == relation_before and int(beacon.quantity) == quantity_before and int(beacon.revision) == revision_before, "Rejected target must not mutate Item Graph")
	...
	_assert(screen.transfer_session.is_active() and screen.transfer_session.remaining_quantity == 1, "RMB target click must place one and keep remaining virtual quantity")
	_assert(int(beacon.quantity) == quantity_before - 1, "First committed target must remove exactly one source unit")
	...
	_assert(not screen.transfer_session.is_active(), "Second RMB target click must exhaust virtual carry")
	_assert(not screen.carry_preview.visible, "Carry preview must disappear after commit")
	_assert(int(beacon.quantity) == quantity_before - 2, "Two committed one-unit placements must leave exact remainder")
	_assert(_total_quantity(controller, "survey_beacon") == total_before, "Virtual carry commits must conserve total quantity")

	screen.refresh()
	beacon_cell = screen.player_panel.find_cell_by_item_id(beacon.instance_id)
	source_payload = beacon_cell.view_data.duplicate(true)
	source_payload["icon_texture"] = beacon_cell.icon_texture
	relation_before = beacon.relation.duplicate(true)
	quantity_before = int(beacon.quantity)
	revision_before = int(beacon.revision)
	screen._on_interaction_requested("CARRY_ALL_OR_PLACE_ALL", source_payload)
	_assert(screen.transfer_session.is_active(), "7 Days LMB click must carry remaining full stack")
	screen._cancel_transfer_session(true)
	_assert(not screen.transfer_session.is_active(), "Escape-equivalent cancel must clear virtual carry")
	_assert(beacon.relation == relation_before and int(beacon.quantity) == quantity_before and int(beacon.revision) == revision_before, "Cancelled virtual carry must leave Item Graph and revision unchanged")

	screen._on_interaction_requested("CARRY_ALL_OR_PLACE_ALL", source_payload)
	screen._apply_interaction_profile(rust_profile, false, false)
	_assert(not screen.transfer_session.is_active(), "Changing profile must cancel pending virtual carry")
