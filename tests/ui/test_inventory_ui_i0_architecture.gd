...
func _run() -> void:
	...
	_assert(not inventory_ui.persistent_hotbar.role_label.visible and not inventory_ui.persistent_hotbar.title_label.visible and not inventory_ui.persistent_hotbar.metadata_label.visible and not inventory_ui.persistent_hotbar.drop_hint_label.visible, "Persistent hotbar must not show duplicate textual labels")
	var hotbar_style = inventory_ui.persistent_hotbar.get_theme_stylebox("panel")
	_assert(hotbar_style is StyleBoxFlat and (hotbar_style as StyleBoxFlat).bg_color.a < 0.7, "Persistent hotbar background must be translucent")
