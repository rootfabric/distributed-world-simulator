...
func _run() -> void:
	...
	var select_press := InputEventMouseButton.new()
	select_press.button_index = MOUSE_BUTTON_LEFT
	select_press.pressed = true
	beacon_cell._gui_input(select_press)
	_assert(screen.player_panel.find_cell_by_item_id(beacon.instance_id) == beacon_cell, "LMB selection must not rebuild the source backpack cell before drag starts")
	_assert(screen.inspector.current_item_id == beacon.instance_id, "LMB selection must update the inspector without rebuilding inventory")
	var backpack_drag_payload: Dictionary = beacon_cell.build_drag_payload(false)
	_assert(String(backpack_drag_payload.get("item_id", "")) == beacon.instance_id, "Selected backpack cell must remain able to provide drag payload")
	var persistent_hotbar_target = ui.persistent_hotbar.grid.get_child(1)
	_assert(persistent_hotbar_target._can_drop_data(Vector2.ZERO, backpack_drag_payload), "Persistent hotbar must accept a compatible drag from the backpack")
	persistent_hotbar_target._drop_data(Vector2.ZERO, backpack_drag_payload)
	_assert(String(beacon.relation.get("container_id", "")) == controller.player_hotbar_id and int(beacon.relation.get("slot_index", -1)) == 1, "Dropping backpack item on persistent hotbar must move it into the selected slot")
	_assert_success(controller.move_item_to_container(beacon.instance_id, controller.player_inventory_id), "Backpack drag fixture must return from hotbar for remaining checks")
	screen.refresh()
	beacon_cell = screen.player_panel.find_cell_by_item_id(beacon.instance_id)
	_assert(beacon_cell != null, "Returned item must remain draggable in the backpack")
	...
	_assert(Relations.kind_of(beacon.relation) == Relations.WORLD, "Dropping a drag outside inventory must move the whole stack to WORLD")
	_assert(_count_world_items(controller, "survey_beacon") == 1, "Outside drag drop must create a world item")
	_assert(beacon_count_before_outside_drop == 3, "Outside drag test must use the complete source stack")
	screen.refresh()
	beacon = _find_item(controller, "survey_beacon", "", Relations.WORLD)
	_assert(beacon != null, "Dropped beacon stack must remain addressable after UI refresh")
	var restore_result: Dictionary = controller.move_item_to_container(beacon.instance_id, controller.player_inventory_id)
	_assert_success(restore_result, "Outside-drop fixture must be restored to continue inventory transfer checks")
	screen.refresh()
