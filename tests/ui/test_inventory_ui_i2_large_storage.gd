...
func _run() -> void:
	...
	_assert(not bool(filtered_large.get("virtualized", true)), "Filtering to 65 batteries must disable virtualization")
	_assert(int(filtered_large.get("rendered_cell_count", 0)) == 65, "Filtered large storage must render all matching aggregates")
	_assert(screen.external_panel.get_pool_size() == view_model.VIRTUAL_PAGE_SIZE, "Filtering must reuse pool without allocating 130 cells")
