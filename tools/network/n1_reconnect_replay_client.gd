...
const SessionScript = preload("res://scripts/network/session/n1_reconnect_replay_client_session.gd")
...
func _initialize() -> void:
	...
	print("N1_RECONNECT_CLIENT_CONNECTING port=%d" % int(_options["port"]))
...
func _finish_failure(error_code: String, details: Dictionary = {}) -> void:
	...
	_finished = true
	push_error("N1 reconnect client failed: %s" % error_code)
	quit(1)
