...
const SupportScript = preload("res://tools/network/n1_reconnect_replay_process_support.gd")
const SessionScript = preload("res://scripts/network/session/n1_reconnect_replay_server_session.gd")
...
func _initialize() -> void:
	...
	SupportScript.write_json(String(_options["result_file"]), {
		"schema": "planet_simulator.n1_reconnect_replay_process_state.v1",
		"mode": "server", "state": "LISTENING", "passed": false, "port": int(_options["port"]),
	})
	print("N1_RECONNECT_SERVER_LISTENING port=%d" % int(_options["port"]))


func _process(_delta: float) -> bool:
	if _finished or _session == null: return false
...
func _finish_from_session() -> void:
	...
	_finished = true
	print("N1_RECONNECT_SERVER_RESULT %s" % JSON.stringify(report))
...
func _finish_failure(error_code: String, details: Dictionary = {}) -> void:
	...
	if not path.is_empty(): SupportScript.write_json(path, report)
	_finished = true
	push_error("N1 reconnect server failed: %s" % error_code)
	quit(1)
