...
const SessionScript = preload("res://scripts/network/session/n1_snapshot_client_session.gd")
...
func _initialize() -> void:
	...
	print("N1_CLIENT_CONNECTING port=%d" % int(_options["port"]))
...
func _finish_from_session() -> void:
	...
	_finished = true
	print("N1_CLIENT_RESULT %s" % JSON.stringify(report))
	quit(_exit_code)
...
func _finish_failure(error_code: String, details: Dictionary = {}) -> void:
	...
	_exit_code = 1
	_finished = true
	push_error("N1 snapshot client failed: %s" % error_code)
	quit(_exit_code)
