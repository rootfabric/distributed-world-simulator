...
const SupportScript = preload("res://tools/network/n1_snapshot_process_support.gd")
const SessionScript = preload("res://scripts/network/session/n1_snapshot_server_session.gd")
...
func _initialize() -> void:
	...
	SupportScript.write_json(String(_options["result_file"]), {
		"schema": "planet_simulator.n1_snapshot_process_state.v1",
		"mode": "server",
		"state": "LISTENING",
		"passed": false,
		"port": int(_options["port"]),
	})
	print("N1_SERVER_LISTENING port=%d" % int(_options["port"]))
...
func _process(_delta: float) -> bool:
	...
	if not bool(polled.get("success", false)) and _session.is_terminal():
...
func _finish_from_session() -> void:
	...
	_finished = true
	print("N1_SERVER_RESULT %s" % JSON.stringify(report))
	quit(_exit_code)
...
func _finish_failure(error_code: String, details: Dictionary = {}) -> void:
	...
	_exit_code = 1
	_finished = true
	push_error("N1 snapshot server failed: %s" % error_code)
	quit(_exit_code)
