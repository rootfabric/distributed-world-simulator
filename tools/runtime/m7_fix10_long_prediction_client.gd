...
# FIX10 prediction-only acceptance/visual diagnostic client.
#
# Long acceptance drives movement automatically through the normal playground
# Input -> advance_local_prediction path. Diagnostic mode is deliberately manual:
# it captures the mouse and never injects movement/yaw so human visual testing
# observes the production input path without the harness fighting the player.
...
func _run_phase() -> void:
	...
	_assert(bool(initial_report.get("network_playground_enabled", false)), "FIX10 stress network playground active")
	_assert(bool(initial_report.get("network_prediction_mode", false)), "FIX10 stress prediction active")
	_assert(bool(initial_report.get("m7_state_sync_enabled", false)), "FIX10 stress state sync remains enabled")
	_assert(
		String(initial_report.get("m7_interpolation_mode", "")) == "CLIENT_PREDICTION_RECONCILIATION",
		"FIX10 stress uses prediction/reconciliation presentation"
	)
	...
	_assert(remote_ready, "FIX10 stress sees the second client")
	if not remote_ready:
	...
	if _fix10_diagnostic_only:
		# The SceneTree worker bypasses SimulatorApp, so no application-level click
		# handler exists to capture the cursor. Capture explicitly for the visual
		# diagnostic and leave every movement/yaw input to the human tester.
		playground.set_runtime_mouse_capture(true)
		while not _fix10_deadline_reached():
			await process_frame
	else:
		var item_ok: bool = false
		if client_id == "a":
			item_ok = await _fix10_item_probe_a()
		else:
			item_ok = await _fix10_item_probe_b()
		...
		if client_id == "a":
			waypoints.append(Vector3(4.0, 1.2, 2.0))
			waypoints.append(Vector3(4.0, 1.2, -4.0))
			waypoints.append(Vector3(-4.0, 1.2, -4.0))
			waypoints.append(Vector3(-4.0, 1.2, 2.0))
		else:
			waypoints.append(Vector3(-3.0, 1.2, 3.0))
			waypoints.append(Vector3(5.0, 1.2, 3.0))
			waypoints.append(Vector3(5.0, 1.2, -3.0))
			waypoints.append(Vector3(-3.0, 1.2, -3.0))
	...
	_assert(bool(final_world_report.get("m7_state_sync_enabled", false)), "FIX10 stress never disabled prediction sync")
	if _fix10_diagnostic_only:
	...
	_assert(
		...
		"FIX10 stress has no prediction reconcile failures"
	)
	_assert(
		"b" in client.get_remote_player_ids() if client_id == "a" else "a" in client.get_remote_player_ids(),
		"FIX10 stress remote peer remains visible"
	)
	if not failures.is_empty():
...
func _fix10_progress_details() -> Dictionary:
	return {
		...
		"manual_diagnostic_input": _fix10_diagnostic_only,
