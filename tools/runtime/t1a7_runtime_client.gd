...
var _client_id: String = "client/t1a7/a"
...
func _init() -> void:
	...
	_client_id = String(args.get("client-id", "client/t1a7/a"))
...
func _poll_action() -> void:
	...
	_last_action_result = _client.execute_construction_runtime_blocking(
		...
		action,
		String(command.get("operation_id", "operation/t1a7/%s/action/%d" % [_client_id.sha256_text().left(8), serial])),
...
func _write_current(state: String) -> void:
	_write({
		"schema": "planet_simulator.t1a7_runtime_client_report.v1",
		"state": state,
