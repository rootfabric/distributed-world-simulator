...
const ServerScript = preload("res://scripts/labs/t1/t1a7/t1a7_m3_runtime_server_adapter.gd")
...
func _init() -> void:
	...
	var setup: Dictionary = _server.setup({
		...
		"authority_owner_id": "simulation/t1a7/dedicated",
		"authority_epoch": 1,
		"result_file": String(args.get("m3-result-file", "")),
		"t1a6_m0_root": String(args.get("m0-root", "user://t1a7-server-m0")),
...
func _write_current(state: String) -> void:
	_write({
		"schema": "planet_simulator.t1a7_runtime_server_report.v1",
		"state": state,
		"passed": state == "READY",
