...
func _saturated_entry() -> Dictionary:
	...
	entry.state.resource_ledger.assimilated.material_mg = B.MAX_STOCK
	entry.state.resource_ledger.growth_transferred.material_mg = B.MAX_STOCK
	return entry if LS.validate(entry.state, blueprint).is_empty() else {}
...
func _headroom_clips_without_lifecycle_failure() -> void:
	...
	_check(result.success, "saturated_a2_population_step_does_not_overflow")
