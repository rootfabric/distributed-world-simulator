...
func _init() -> void:
	_development_cannot_occupy_current_starvation_window()
	print("EVO_ARCH2_A5_RM26 assertions=%d failed=%d" % [passed, failed])
...
func _development_cannot_occupy_current_starvation_window() -> void:
	...
	_check(state.reproduction_count == 0 and LS.validate(state, blueprint).is_empty(), "funded_state_valid_without_reproduction")

	var fake_starvation: int = state.age_ticks - maxi(0, int(state.development.grant_seq) - 1)
	_check(fake_starvation > 0 and fake_starvation < blueprint.life_history.survival.starvation_limit_ticks, "adversarial_starvation_window_is_alive_valid")
	var tampered: Dictionary = state.duplicate(true)
	tampered.starvation_ticks = fake_starvation
	var paid_prefix_ticks: int = tampered.age_ticks - tampered.starvation_ticks
	_check(tampered.development.grant_seq > paid_prefix_ticks, "development_enters_claimed_starvation_window")
	_check(LS.validate(tampered, blueprint) == "LIFE_DEVELOPMENT_CAUSALITY", "starvation_window_splice_rejected")
	_check(LS.serialize(tampered, blueprint).is_empty(), "starvation_window_splice_not_serializable")
	_check(LS.deserialize(_encoded_state_file(blueprint, tampered)).is_empty(), "starvation_window_splice_deserialize_rejected")

	var boundary: Dictionary = state.duplicate(true)
	boundary.starvation_ticks = boundary.age_ticks - int(boundary.development.grant_seq)
	_check(boundary.starvation_ticks >= 0 and LS.validate(boundary, blueprint).is_empty(), "development_at_paid_prefix_boundary_accepted")