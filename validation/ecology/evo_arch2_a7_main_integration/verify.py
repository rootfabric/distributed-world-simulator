...
EXTERNAL_DEP = "scripts/research/ecology/plant_development_traits_extension_evo7_v1.gd"
...
PREFIXES = ("config/ecology/evo-arch2-a7-main-integration-", "config/ecology/evo-arch2-a7-protocol.v1.json",
            "docs/research/ecology/EVO_ARCH2_A7_MAIN_INTEGRATION", "scripts/research/ecology/v2/",
            "scripts/labs/ecology/arch2_a7_", "scenes/labs/ecology/arch2_a7_",
            "tests/research/ecology/v2/", "validation/ecology/evo_arch2_a5/",
...
def allowed(path: str) -> bool:
    return path == EXTERNAL_DEP or any(path == p or path.startswith(p) for p in PREFIXES)
...
def main() -> int:
    ...
    args = ap.parse_args()
    os.chdir(ROOT)
    out = ROOT / "artifacts/a7-main-integration/exact"; out.mkdir(parents=True, exist_ok=True)
    env = dict(os.environ, GODOT_SILENCE_ROOT_WARNING="1")
    summary = {"subject_head":args.head,"subject_tree":args.tree,"base_main":BASE_MAIN,
               "accepted_transfer_source":ACCEPTED_A7,"verdict":"FAIL","checks":[],"repeat_pairs":[],
               "graphical_required":True,"main_owned_files_modified":False}
    ...
    def run(name: str, cmd: list[str], marker: str="", assertions: int=0,
        ...
        print(f"PASS {name} {marker}", flush=True); return log
    ...
    def twice(name: str, path: str, marker: str, n: int) -> None:
        a=run(name+"-1",gd(path),marker,n); b=run(name+"-2",gd(path),marker,n)
    ...
    try:
        ...
        records=git("diff","--name-status",BASE_MAIN,args.head).splitlines(); req(bool(records),"EMPTY_DIFF")
        additions=[]
        for rec in records:
            parts=rec.split("\t"); req(len(parts)==2 and parts[0]=="A","NOT_ADDITION_ONLY:"+rec)
            path=parts[1]; req(allowed(path),"OUT_OF_SCOPE_PATH:"+path); additions.append(path)
        forbidden=("scripts/ecology/production/","scripts/network/","scripts/simulation/","config/control/","config/architecture/")
        req(not any(p=="project.godot" or p.startswith(forbidden) or p.startswith(".github/") for p in additions),
            "MAIN_OWNERSHIP_VIOLATION")
        ...
        req(pins["base_main"]==BASE_MAIN and pins["accepted_a7"]==ACCEPTED_A7,"TRANSFER_MANIFEST_IDENTITY")
        for path,want in {**pins["subtrees"],**pins["blobs"]}.items():
            req(git("rev-parse","HEAD:"+path)==want,"TRANSFER_IDENTITY:"+path)
            req(gp("cat-file","-e",BASE_MAIN+":"+path).returncode!=0,"TRANSFER_PATH_EXISTED_ON_MAIN:"+path)
        summary["transfer_identity"]="PASS"; summary["base_absence_gate"]="PASS_BY_GIT_EXIT_STATUS"
        ...
        sources += [ROOT/EXTERNAL_DEP, ROOT/"scenes/labs/ecology/arch2_a7_observatory.tscn"]
        refs=0
        for source in sources:
            text=source.read_text(encoding="utf-8-sig")
            for pattern in RES_PATTERNS:
                for m in pattern.finditer(text):
                    refs += 1; target=ROOT/m.group(1)
                    req(target.exists(),f"TRANSITIVE_RES_PATH_MISSING:{source.relative_to(ROOT)}->{m.group(1)}")
        summary["res_path_closure"]={"result":"PASS","sources":len(sources),"references":refs}
        ...
        suites=[("a7-protocol","arch2_a7_protocol_guard.gd","EVO_ARCH2_A7_PROTOCOL assertions=106 failed=0",106),
        ...
        for name,file,marker,n in suites: twice(name,"tests/research/ecology/v2/"+file,marker,n)

        for stage in ("a7","a6"):
            for rep in (1,2):
                shutil.rmtree(ROOT/f"artifacts/{stage}/restart",ignore_errors=True)
                for phase in ("write","read"):
                    run(f"{stage}-restart-{phase}-{rep}",gd(f"tests/research/ecology/v2/arch2_{stage}_restart.gd","--",phase),
                        f"EVO_ARCH2_{stage.upper()}_RESTART phase={phase} failed=0")
            for phase in ("write","read"):
                a=out/f"{stage}-restart-{phase}-1.log"; b=out/f"{stage}-restart-{phase}-2.log"
                req(a.read_bytes()==b.read_bytes(),f"RESTART_REPEAT:{stage}:{phase}"); summary["repeat_pairs"].append(f"{stage}-restart-{phase}")
        ...
        for number,n in RM.items():
            ...
            twice(f"rm{number}",files[0].relative_to(ROOT).as_posix(),f"EVO_ARCH2_A5_RM{number} assertions={n} failed=0",n)

        glog=run("a7-graphical",["xvfb-run","-a","-s","-screen 0 1600x1100x24",str(args.godot),
        ...
        png=ROOT/"artifacts/a7/observatory.png"; report=ROOT/"artifacts/a7/capture-sources.json"
        req(png.is_file() and report.is_file(),"GRAPHICAL_EVIDENCE_MISSING")
        summary.update(viewport_sha256=sha(png),viewport_source_report_sha256=sha(report),graphical_log_sha256=sha(glog))

        hlog=run("main-harness-regression",[sys.executable,"-m","unittest","discover","-s",str(ROOT/"tests/harness"),"-p","test_*.py","-v"],
        ...
        cdir=ROOT/"artifacts/a7-main-integration/control"; cdir.mkdir(parents=True,exist_ok=True); controls=[]
        for name,script,extra,artifact in [("standard","project_control.py",["--no-fetch","--no-fail-on-red"],"project-control-report.json"),
            ...
            src=ROOT/"artifacts/control"/artifact; req(src.is_file(),"CONTROL_REPORT_MISSING:"+name)
            health=json.loads(src.read_text()).get("overall_health","UNAVAILABLE"); req(health!="RED","CONTROL_RED:"+name)
            shutil.copyfile(src,cdir/artifact); shutil.copyfile(log,cdir/(name+".log"))
            controls.append({"name":name,"overall_health":health,"report_sha256":sha(src),"log_sha256":sha(log)})
        ...
        summary["assertion_executions"]=sum(c["assertions"] for c in summary["checks"]); summary["verdict"]="PASS"
