...
def crash_worker(root: str, anchor: dict, stage: str) -> None:
    def fault(at: str) -> None:
        if at == stage:
            os._exit(91)
    S.SnapshotStore(root, fault=fault).commit(anchor, b'fixture:new', byte_admission)
    os._exit(92)


def race_worker(root: str, anchor: dict, value: bytes, ready, results) -> None:
    ready.wait(timeout=10)
    try:
        head = S.SnapshotStore(root).commit(anchor, value, byte_admission)
...
class StoreTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.anchor = S.genesis_anchor()

    def current(self):
        return self.store.load(self.anchor)

    def put(self, data: bytes = b'fixture:old') -> dict:
        head = self.store.commit(self.anchor, data, byte_admission)
        self.anchor = head
        return head

    def test_initialize_and_reopen(self):
        head, data = S.SnapshotStore(self.root).load(S.genesis_anchor())
        self.assertEqual(head, S.genesis_anchor())
    ...
    def test_commit_and_content_address(self):
        ...
        self.assertEqual(S.SnapshotStore(self.root).load(head), (head, b'fixture:old'))
    ...
    def test_stale_cas_changes_nothing(self):
        ...
        with self.assertRaises(S.Conflict):
            self.store.commit(S.genesis_anchor(), b'fixture:new', byte_admission)
        self.assertEqual(self.current(), (head, b'fixture:old'))
        ...
        for admit in (None, lambda _: False, lambda _: 1, lambda _: 'PASS'):
            with self.subTest(admit=repr(admit)):
                with self.assertRaisesRegex(S.StoreError, 'SEMANTIC_ADMISSION_REQUIRED'):
                    self.store.commit(self.anchor, b'fixture:old', admit)
        self.assertEqual(self.current()[0], S.genesis_anchor())

    def test_admission_exception_does_not_write(self):
        def reject(_):
            raise ValueError('semantic failure')
        with self.assertRaises(ValueError):
            self.store.commit(self.anchor, b'fixture:old', reject)
        ...
        for data in (b'', b'x' * (S.MAX_SNAPSHOT_BYTES + 1), 'text'):
            with self.subTest(size=len(data)):
                with self.assertRaisesRegex(S.StoreError, 'SNAPSHOT_BYTE_BUDGET'):
                    self.store.commit(self.anchor, data, byte_admission)
        self.assertEqual(self.current()[0], S.genesis_anchor())

    def test_invalid_expected_anchor(self):
        bad = [None, '', {'tip': S.ZERO},
               {'tip': 'X' * 64, 'sequence': 0, 'snapshot_sha256': None},
               {'tip': S.ZERO, 'sequence': 1, 'snapshot_sha256': S.ZERO},
               {'tip': S.ZERO, 'sequence': False, 'snapshot_sha256': None}]
        for anchor in bad:
            with self.subTest(anchor=anchor):
                with self.assertRaisesRegex(S.StoreError, 'EXTERNAL_DURABLE_ANCHOR'):
                    self.store.commit(anchor, b'fixture:old', byte_admission)
    ...
    def test_two_real_writers_one_wins(self):
        ...
        results = ctx.Queue()
        children = [ctx.Process(target=race_worker, args=(str(self.root), head, value, barrier, results))
        ...
        after, data = self.store.load(head)  # descendant of the last acknowledged head is recoverable
        self.anchor = after
    ...
    def test_crash_at_every_durable_boundary(self):
        ...
        for stage in STAGES:
            with self.subTest(stage=stage):
                ...
                acknowledged = store.commit(S.genesis_anchor(), b'fixture:old', byte_admission)
                child = ctx.Process(target=crash_worker, args=(str(root), acknowledged, stage))
                child.start(); child.join(15)
                if child.is_alive():
                    child.kill(); child.join()
                self.assertEqual(child.exitcode, 91)
                # The caller still has only the old durable acknowledgement. load()
                # may return it or a valid descendant after an ambiguous pointer ack.
                recovered, data = S.SnapshotStore(root).load(acknowledged)
                ...
                final = store.commit(recovered, b'fixture:new', byte_admission)
                self.assertEqual(final['sequence'], 2)

    def test_public_load_requires_external_anchor(self):
        with self.assertRaisesRegex(S.StoreError, 'EXTERNAL_DURABLE_ANCHOR_REQUIRED'):
            self.store.load(None)

    def test_acknowledged_commit_rejects_zero_pointer_rollback(self):
        acknowledged = self.put()
        (self.root / 'CURRENT').write_text(S.ZERO + '\n')
        with self.assertRaisesRegex(S.StoreError, 'DURABLE_ANCHOR_ROLLBACK_OR_FORK'):
            self.store.load(acknowledged)

    def test_acknowledged_commit_rejects_previous_valid_tip(self):
        previous = self.put()
        acknowledged = self.put(b'fixture:new')
        (self.root / 'CURRENT').write_text(previous['tip'] + '\n')
        with self.assertRaisesRegex(S.StoreError, 'DURABLE_ANCHOR_ROLLBACK_OR_FORK'):
            self.store.load(acknowledged)

    def test_acknowledged_commit_rejects_alternate_valid_fork(self):
        previous = self.put()
        acknowledged = self.put(b'fixture:new')
        alternate = b'fixture:alternate'
        blob_hash = S.digest(alternate)
        (self.root / 'blobs' / (blob_hash + '.bin')).write_bytes(alternate)
        record = {'schema': S.RECORD_SCHEMA, 'sequence': 2, 'previous': previous['tip'],
                  'snapshot_sha256': blob_hash, 'snapshot_bytes': len(alternate)}
        encoded = S._json(record); fork_tip = S.digest(encoded)
        (self.root / 'records' / (fork_tip + '.json')).write_bytes(encoded)
        (self.root / 'CURRENT').write_text(fork_tip + '\n')
        with self.assertRaisesRegex(S.StoreError, 'DURABLE_ANCHOR_ROLLBACK_OR_FORK'):
            self.store.load(acknowledged)

    def test_malformed_load_anchor_fails_closed(self):
        self.put()
        bad = [None, {}, {'tip': S.ZERO, 'sequence': 0, 'snapshot_sha256': S.ZERO},
               {'tip': self.anchor['tip'], 'sequence': '1', 'snapshot_sha256': self.anchor['snapshot_sha256']}]
        for anchor in bad:
            with self.subTest(anchor=anchor):
                with self.assertRaisesRegex(S.StoreError, 'EXTERNAL_DURABLE_ANCHOR'):
                    self.store.load(anchor)
    ...
    def test_rehashed_truncated_chain_rejected(self):
        ...
        raw = S._json(record); new_tip = S.digest(raw)
        (p.parent / (new_tip + '.json')).write_bytes(raw)
        (self.root / 'CURRENT').write_text(new_tip + '\n')
    ...
    def test_disk_budget_stops_before_publication(self):
        ...
        with self.assertRaisesRegex(S.StoreError, 'STORE_CAPACITY_BUDGET'):
            self.put(b'fixture:new')
        self.assertEqual(self.current()[0], head)
    ...
    def test_immutable_collision_cannot_replace_bytes(self):
        ...
        old_tip = self.current()[0]['tip']
        with self.assertRaises((S.StoreError, OSError)):
            self.put(b'fixture:new')
        self.assertEqual(self.current()[0]['tip'], old_tip)
